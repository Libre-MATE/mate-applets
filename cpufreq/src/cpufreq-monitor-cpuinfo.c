...
G_DEFINE_TYPE(CPUFreqMonitorCPUInfo, cpufreq_monitor_cpuinfo,
              CPUFREQ_TYPE_MONITOR)

static void cpufreq_monitor_cpuinfo_init(CPUFreqMonitorCPUInfo *monitor) {}

static void cpufreq_monitor_cpuinfo_class_init(
    CPUFreqMonitorCPUInfoClass *klass) {
...
static gboolean cpufreq_monitor_cpuinfo_run(CPUFreqMonitor *monitor) {
  ...
  g_object_set(G_OBJECT(monitor), "governor", governor, "frequency", cur_freq,
               "max-frequency", max_freq, NULL);
