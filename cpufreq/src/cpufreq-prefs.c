...
static void cpufreq_prefs_set_property(GObject *object, guint prop_id,
                                       const GValue *value, GParamSpec *pspec);

static void cpufreq_prefs_get_property(GObject *object, guint prop_id,
...
static void cpufreq_prefs_class_init(CPUFreqPrefsClass *klass) {
  ...
  g_object_class_install_property(
      g_object_class, PROP_CPU,
      g_param_spec_uint("cpu", "CPU", "The monitored cpu", 0, G_MAXUINT, 0,
                        G_PARAM_READWRITE));
  g_object_class_install_property(
      g_object_class, PROP_SHOW_MODE,
      g_param_spec_enum("show-mode", "ShowMode", "The applet show mode",
                        CPUFREQ_TYPE_SHOW_MODE, CPUFREQ_MODE_BOTH,
                        G_PARAM_READWRITE));
  g_object_class_install_property(
      g_object_class, PROP_SHOW_TEXT_MODE,
      g_param_spec_enum("show-text-mode", "ShowTextMode",
                        "The applet show text mode",
                        CPUFREQ_TYPE_SHOW_TEXT_MODE,
                        CPUFREQ_MODE_TEXT_FREQUENCY_UNIT, G_PARAM_READWRITE));
  ...
  switch (prop_id) {
    ...
    case PROP_SHOW_MODE: {
      ...
      if (prefs->priv->show_mode != mode) {
        ...
        g_settings_set_int(prefs->priv->settings, "show-mode", mode);
      }
      break;
    }
    case PROP_SHOW_TEXT_MODE: {
      ...
      if (prefs->priv->show_text_mode != mode) {
        ...
        g_settings_set_int(prefs->priv->settings, "show-text-mode", mode);
      }
      break;
    }
    default:
...
static void cpufreq_prefs_setup(CPUFreqPrefs *prefs) {
  g_assert(G_IS_SETTINGS(prefs->priv->settings));

  prefs->priv->cpu = g_settings_get_int(prefs->priv->settings, "cpu");
  prefs->priv->show_mode =
      g_settings_get_int(prefs->priv->settings, "show-mode");
  prefs->priv->show_text_mode =
      g_settings_get_int(prefs->priv->settings, "show-text-mode");
}
...
CPUFreqShowMode cpufreq_prefs_get_show_mode(CPUFreqPrefs *prefs) {
  g_return_val_if_fail(CPUFREQ_IS_PREFS(prefs), CPUFREQ_MODE_BOTH);
...
/* Preferences Dialog */
static void cpufreq_prefs_dialog_show_freq_toggled(GtkWidget *show_freq,
                                                   CPUFreqPrefs *prefs) {
  CPUFreqShowTextMode show_text_mode;
  ...
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(show_freq))) {
    ...
    g_object_set(G_OBJECT(prefs), "show-text-mode", show_text_mode, NULL);
  }
}

static void cpufreq_prefs_dialog_show_unit_toggled(GtkWidget *show_unit,
                                                   CPUFreqPrefs *prefs) {
  CPUFreqShowTextMode show_text_mode;
...
static void cpufreq_prefs_dialog_show_perc_toggled(GtkWidget *show_perc,
                                                   CPUFreqPrefs *prefs) {
  CPUFreqShowTextMode show_text_mode;
...
static void cpufreq_prefs_dialog_cpu_number_changed(GtkWidget *cpu_combo,
                                                    CPUFreqPrefs *prefs) {
  gint cpu;
  ...
  if (cpu >= 0) {
    g_object_set(G_OBJECT(prefs), "cpu", cpu, NULL);
  }
}

static void cpufreq_prefs_dialog_show_mode_changed(GtkWidget *show_mode_combo,
                                                   CPUFreqPrefs *prefs) {
  CPUFreqShowMode show_mode;

  show_mode = gtk_combo_box_get_active(GTK_COMBO_BOX(show_mode_combo));
  g_object_set(G_OBJECT(prefs), "show-mode", show_mode, NULL);
}

static void cpufreq_prefs_dialog_response_cb(CPUFreqPrefs *prefs, gint response,
                                             GtkDialog *dialog) {
  GError *error = NULL;
...
static void cpufreq_prefs_dialog_update_visibility(CPUFreqPrefs *prefs) {
...
static void cpufreq_prefs_dialog_update_sensitivity(CPUFreqPrefs *prefs) {
  ...
  if (prefs->priv->show_mode != CPUFREQ_MODE_GRAPHIC) {
    ...
    key_writable =
        g_settings_is_writable(prefs->priv->settings, "show-text-mode");

    gtk_widget_set_sensitive(prefs->priv->show_freq, (TRUE && key_writable));
    gtk_widget_set_sensitive(prefs->priv->show_perc, (TRUE && key_writable));

    if (prefs->priv->show_text_mode == CPUFREQ_MODE_TEXT_PERCENTAGE)
      gtk_widget_set_sensitive(prefs->priv->show_unit, FALSE);
    else
      gtk_widget_set_sensitive(prefs->priv->show_unit, (TRUE && key_writable));
  } else {
...
static void cpufreq_prefs_dialog_update(CPUFreqPrefs *prefs) {
  if (cpufreq_utils_get_n_cpus() > 1) {
...
static void cpufreq_prefs_dialog_cpu_combo_setup(CPUFreqPrefs *prefs) {
  GtkListStore *model;
  GtkTreeIter iter;
  ...
  for (i = 0; i < n_cpus; i++) {
    ...
    gtk_list_store_set(model, &iter, 0, text_label, -1);
  ...
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(prefs->priv->cpu_combo), renderer,
                             TRUE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(prefs->priv->cpu_combo),
                                 renderer, "text", 0, NULL);
}

static void cpufreq_prefs_dialog_show_mode_combo_setup(CPUFreqPrefs *prefs) {
  GtkListStore *model;
  GtkTreeIter iter;
  GtkCellRenderer *renderer;
  ...
  gtk_list_store_append(model, &iter);
  gtk_list_store_set(model, &iter, 0, _("Graphic"), -1);

  gtk_list_store_append(model, &iter);
  gtk_list_store_set(model, &iter, 0, _("Text"), -1);

  gtk_list_store_append(model, &iter);
  gtk_list_store_set(model, &iter, 0, _("Graphic and Text"), -1);
...
static void cpufreq_prefs_dialog_create(CPUFreqPrefs *prefs) {
  GtkBuilder *builder;

  builder = gtk_builder_new_from_resource(CPUFREQ_RESOURCE_PATH
  ...
  prefs->priv->cpu_combo =
      GTK_WIDGET(gtk_builder_get_object(builder, "prefs_cpu_number"));

  prefs->priv->show_mode_combo =
      GTK_WIDGET(gtk_builder_get_object(builder, "prefs_show_mode"));

  prefs->priv->show_freq =
      GTK_WIDGET(gtk_builder_get_object(builder, "prefs_show_freq"));
  prefs->priv->show_unit =
      GTK_WIDGET(gtk_builder_get_object(builder, "prefs_show_unit"));
  prefs->priv->show_perc =
      GTK_WIDGET(gtk_builder_get_object(builder, "prefs_show_perc"));

  prefs->priv->monitor_settings_frame =
      GTK_WIDGET(gtk_builder_get_object(builder, "monitor_settings_frame"));
  ...
  g_signal_connect_swapped(prefs->priv->dialog, "response",
                           G_CALLBACK(cpufreq_prefs_dialog_response_cb), prefs);

  g_signal_connect(prefs->priv->show_freq, "toggled",
                   G_CALLBACK(cpufreq_prefs_dialog_show_freq_toggled), prefs);

  g_signal_connect(prefs->priv->show_unit, "toggled",
                   G_CALLBACK(cpufreq_prefs_dialog_show_unit_toggled), prefs);

  g_signal_connect(prefs->priv->show_perc, "toggled",
                   G_CALLBACK(cpufreq_prefs_dialog_show_perc_toggled), prefs);

  g_signal_connect(prefs->priv->cpu_combo, "changed",
                   G_CALLBACK(cpufreq_prefs_dialog_cpu_number_changed), prefs);

  g_signal_connect(prefs->priv->show_mode_combo, "changed",
                   G_CALLBACK(cpufreq_prefs_dialog_show_mode_changed), prefs);
}

void cpufreq_preferences_dialog_run(CPUFreqPrefs *prefs, GdkScreen *screen) {
