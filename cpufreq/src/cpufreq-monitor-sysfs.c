...
static GList *cpufreq_monitor_sysfs_get_available_frequencies(
    CPUFreqMonitor *monitor);
static GList *cpufreq_monitor_sysfs_get_available_governors(
    CPUFreqMonitor *monitor);
 ...
 */
const gchar *monitor_sysfs_files[] = {"scaling_max_freq",
                                      "scaling_min_freq",
                                      "scaling_governor",
                                      "cpuinfo_max_freq",
                                      "scaling_setspeed",
...
static void cpufreq_monitor_sysfs_init(CPUFreqMonitorSysfs *monitor) {}

static GObject *cpufreq_monitor_sysfs_constructor(
  ...
  object = G_OBJECT_CLASS(cpufreq_monitor_sysfs_parent_class)
               ->constructor(type, n_construct_properties, construct_params);

  g_object_get(G_OBJECT(object), "cpu", &cpu, NULL);

  path = g_strdup_printf(CPUFREQ_SYSFS_BASE_PATH, cpu,
                         monitor_sysfs_files[CPUINFO_MAX]);
  ...
  g_object_set(G_OBJECT(object), "max-frequency", max_freq, NULL);
...
static void cpufreq_monitor_sysfs_class_init(CPUFreqMonitorSysfsClass *klass) {
  ...
  monitor_class->get_available_frequencies =
      cpufreq_monitor_sysfs_get_available_frequencies;
  monitor_class->get_available_governors =
      cpufreq_monitor_sysfs_get_available_governors;
}

CPUFreqMonitor *cpufreq_monitor_sysfs_new(guint cpu) {
  CPUFreqMonitorSysfs *monitor;

  monitor = g_object_new(CPUFREQ_TYPE_MONITOR_SYSFS, "cpu", cpu, NULL);
...
static gboolean cpufreq_sysfs_cpu_is_online(guint cpu) {
  gchar *path;
  gboolean retval;
...
static gboolean cpufreq_monitor_sysfs_run(CPUFreqMonitor *monitor) {
  ...
  if (g_ascii_strcasecmp(governor, "userspace") == 0) {
    path = g_strdup_printf(CPUFREQ_SYSFS_BASE_PATH, cpu,
                           monitor_sysfs_files[SCALING_SETSPEED]);
  } else if (g_ascii_strcasecmp(governor, "powersave") == 0) {
    path = g_strdup_printf(CPUFREQ_SYSFS_BASE_PATH, cpu,
                           monitor_sysfs_files[SCALING_MIN]);
  } else if (g_ascii_strcasecmp(governor, "performance") == 0) {
    path = g_strdup_printf(CPUFREQ_SYSFS_BASE_PATH, cpu,
                           monitor_sysfs_files[SCALING_MAX]);
  ...
  g_object_set(G_OBJECT(monitor), "online", TRUE, "governor", governor,
               "frequency", atoi(frequency), NULL);
