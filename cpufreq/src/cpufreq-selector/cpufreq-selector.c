...
enum { PROP_0, PROP_CPU };

struct _CPUFreqSelectorPrivate {
  guint cpu;
};

static void cpufreq_selector_set_property(GObject *object, guint prop_id,
                                          const GValue *value,
                                          GParamSpec *spec);

static void cpufreq_selector_get_property(GObject *object, guint prop_id,
                                          GValue *value, GParamSpec *spec);

G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE(CPUFreqSelector, cpufreq_selector,
                                    G_TYPE_OBJECT)

GQuark cpufreq_selector_error_quark(void) {
...
static void cpufreq_selector_class_init(CPUFreqSelectorClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_uint("cpu", NULL, NULL, 0, G_MAXUINT, 0,
