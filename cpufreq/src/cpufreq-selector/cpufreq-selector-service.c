...
GType cpufreq_selector_service_error_get_type(void) {
...
GQuark cpufreq_selector_service_error_quark(void) {
...
static void cpufreq_selector_service_finalize(GObject *object) {
...
static void cpufreq_selector_service_class_init(
    CPUFreqSelectorServiceClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  object_class->finalize = cpufreq_selector_service_finalize;
}

static void cpufreq_selector_service_init(CPUFreqSelectorService *service) {
  service->selectors_max = -1;
}
...
CPUFreqSelectorService *cpufreq_selector_service_get_instance(void) {
  ...
  if (!service)
    service = CPUFREQ_SELECTOR_SERVICE(
        g_object_new(CPUFREQ_TYPE_SELECTOR_SERVICE, NULL));

  return service;
}

static gboolean service_shutdown(gpointer user_data) {
  g_object_unref(SELECTOR_SERVICE);

  return FALSE;
}

static void reset_killtimer(void) {
  static guint timer_id = 0;

  if (timer_id > 0) g_source_remove(timer_id);

  timer_id = g_timeout_add_seconds(30, (GSourceFunc)service_shutdown, NULL);
}
...
gboolean cpufreq_selector_service_register(CPUFreqSelectorService *service,
  ...
  if (service->system_bus) {
    g_set_error(error, CPUFREQ_SELECTOR_SERVICE_ERROR,
  ...
  if (!connection) {
    g_set_error(error, CPUFREQ_SELECTOR_SERVICE_ERROR, SERVICE_ERROR_DBUS,
                "Couldn't connect to system bus: %s", err->message);
    g_error_free(err);
  ...
  res = dbus_g_proxy_call(bus_proxy, "RequestName", &err, G_TYPE_STRING,
                          BUS_NAME, G_TYPE_UINT, DBUS_NAME_FLAG_DO_NOT_QUEUE,
  ...
  if (!res) {
    if (err) {
      g_set_error(error, CPUFREQ_SELECTOR_SERVICE_ERROR, SERVICE_ERROR_DBUS,
                  "Failed to acquire %s: %s", BUS_NAME, err->message);
      g_error_free(err);
    } else {
      g_set_error(error, CPUFREQ_SELECTOR_SERVICE_ERROR, SERVICE_ERROR_DBUS,
  ...
  dbus_g_connection_register_g_object(
      connection, "/org/mate/cpufreq_selector/selector", G_OBJECT(service));
...
static CPUFreqSelector *get_selector_for_cpu(CPUFreqSelectorService *service,
                                             guint cpu) {
  if (!service->selectors[cpu]) {
    service->selectors[cpu] = cpufreq_selector_factory_create_selector(cpu);
    if (!service->selectors[cpu]) return NULL;

    if (service->selectors_max < cpu) service->selectors_max = cpu;
  }
...
/* PolicyKit */
static gboolean cpufreq_selector_service_check_policy(
    CPUFreqSelectorService *service, DBusGMethodInvocation *context,
    GError **error) {
  ...
  result = polkit_authority_check_authorization_sync(
      service->authority, subject, "org.mate.cpufreqselector", NULL,
  ...
  if (!ret) {
    g_set_error(error, CPUFREQ_SELECTOR_SERVICE_ERROR,
                SERVICE_ERROR_NOT_AUTHORIZED, "Caller is not authorized");
  }
...
/* D-BUS interface */
gboolean cpufreq_selector_service_set_frequency(
    CPUFreqSelectorService *service, guint cpu, guint frequency,
  ...
  if (cpu > MAX_CPUS) {
    ...
    err = g_error_new(CPUFREQ_SELECTOR_SERVICE_ERROR, SERVICE_ERROR_DBUS,
                      "Error setting frequency on cpu %d: Invalid cpu", cpu);
  ...
  if (error) {
    ...
    err = g_error_new(CPUFREQ_SELECTOR_SERVICE_ERROR, SERVICE_ERROR_DBUS,
                      "Error setting frequency %d on cpu %d: %s", frequency,
                      cpu, error->message);
    dbus_g_method_return_error(context, err);
    g_error_free(err);
    g_error_free(error);
...
gboolean cpufreq_selector_service_set_governor(CPUFreqSelectorService *service,
                                               guint cpu, const gchar *governor,
                                               DBusGMethodInvocation *context) {
  CPUFreqSelector *selector;
  GError *error = NULL;
  ...
  if (cpu > MAX_CPUS) {
    ...
    err = g_error_new(CPUFREQ_SELECTOR_SERVICE_ERROR, SERVICE_ERROR_DBUS,
                      "Error setting governor on cpu %d: Invalid cpu", cpu);
    dbus_g_method_return_error(context, err);
    g_error_free(err);
  ...
  if (error) {
    ...
    err = g_error_new(CPUFREQ_SELECTOR_SERVICE_ERROR, SERVICE_ERROR_DBUS,
                      "Error setting governor %s on cpu %d: %s", governor, cpu,
                      error->message);
    dbus_g_method_return_error(context, err);
    g_error_free(err);
    g_error_free(error);
...
gboolean cpufreq_selector_service_can_set(CPUFreqSelectorService *service,
  ...
  result = polkit_authority_check_authorization_sync(
      service->authority, subject, "org.mate.cpufreqselector", NULL, 0, NULL,
      &error);
  g_object_unref(subject);
