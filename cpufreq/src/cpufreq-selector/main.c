...
static const GOptionEntry options[] = {
    ...
    {"frequency", 'f', 0, G_OPTION_ARG_INT, &frequency, "Frequency in KHz",
     NULL},
    {NULL}};

#ifdef HAVE_POLKIT
static void do_exit(GMainLoop *loop, GObject *object) {
  if (g_main_loop_is_running(loop)) g_main_loop_quit(loop);
}

static void cpufreq_selector_set_values_dbus(void) {
  ...
  proxy = dbus_g_proxy_new_for_name(connection, "org.mate.CPUFreqSelector",
  ...
  if (governor) {
    res = dbus_g_proxy_call(proxy, "SetGovernor", &error, G_TYPE_UINT, cpu,
  ...
  if (frequency != 0) {
    res = dbus_g_proxy_call(proxy, "SetFrequency", &error, G_TYPE_UINT, cpu,
                            G_TYPE_UINT, frequency, G_TYPE_INVALID,
                            G_TYPE_INVALID);
    if (!res) {
      if (error) {
...
static void cpufreq_selector_set_values(void) {
...
gint main(gint argc, gchar **argv) {
  ...
  g_object_weak_ref(G_OBJECT(SELECTOR_SERVICE), (GWeakNotify)do_exit, loop);
