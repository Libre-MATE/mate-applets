...
static void cpufreq_popup_init(CPUFreqPopup *popup) {
  ...
  gtk_ui_manager_add_ui_from_string(popup->priv->ui_manager, ui_popup, -1,
                                    NULL);
...
CPUFreqPopup *cpufreq_popup_new(void) {
  ...
  popup = CPUFREQ_POPUP(g_object_new(CPUFREQ_TYPE_POPUP, NULL));
...
void cpufreq_popup_set_monitor(CPUFreqPopup *popup, CPUFreqMonitor *monitor) {
  ...
  if (popup->priv->monitor == monitor) return;

  if (popup->priv->monitor) g_object_unref(popup->priv->monitor);
...
static void cpufreq_popup_frequencies_menu_activate(GtkAction *action,
                                                    CPUFreqPopup *popup) {
  ...
  if (!gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action))) return;
...
static void cpufreq_popup_governors_menu_activate(GtkAction *action,
                                                  CPUFreqPopup *popup) {
  CPUFreqSelector *selector;
  const gchar *name;
  guint cpu;
...
static void cpufreq_popup_menu_add_action(
    CPUFreqPopup *popup, const gchar *menu, GtkActionGroup *action_group,
    const gchar *action_name, const gchar *label, gboolean sensitive) {
  ...
  if (g_ascii_strcasecmp(menu, "Frequency") == 0) {
    ...
    popup->priv->govs_actions =
        g_slist_prepend(popup->priv->govs_actions, (gpointer)action);

    g_signal_connect(action, "activate",
                     G_CALLBACK(cpufreq_popup_governors_menu_activate), popup);
  }
...
static void frequencies_menu_create_actions(CPUFreqPopup *popup) {
  GList *available_freqs;

  available_freqs =
      cpufreq_monitor_get_available_frequencies(popup->priv->monitor);
  ...
  while (available_freqs) {
    ...
    cpufreq_popup_menu_add_action(popup, "Frequency", popup->priv->freqs_group,
                                  text, label, TRUE);
    g_free(label);
...
static void governors_menu_create_actions(CPUFreqPopup *popup) {
  GList *available_govs;

  available_govs =
      cpufreq_monitor_get_available_governors(popup->priv->monitor);
  available_govs =
      g_list_sort(available_govs, (GCompareFunc)g_ascii_strcasecmp);
  ...
  while (available_govs) {
    ...
    cpufreq_popup_menu_add_action(popup, "Governor", popup->priv->govs_group,
                                  governor, label, TRUE);
    g_free(label);
...
static void cpufreq_popup_build_ui(CPUFreqPopup *popup, GSList *actions,
  ...
  for (l = actions; l && l->data; l = g_slist_next(l)) {
    ...
    g_object_get(G_OBJECT(action), "name", &name, "label", &label, NULL);

    gtk_ui_manager_add_ui(popup->priv->ui_manager, popup->priv->merge_id,
                          menu_path, label, name, GTK_UI_MANAGER_MENUITEM,
                          FALSE);
...
static void cpufreq_popup_build_frequencies_menu(CPUFreqPopup *popup,
                                                 const gchar *path) {
  if (!popup->priv->freqs_group) {
    ...
    gtk_ui_manager_insert_action_group(popup->priv->ui_manager, action_group,
                                       0);
  }

  cpufreq_popup_build_ui(popup, popup->priv->freqs_actions, path);
}

static void cpufreq_popup_build_governors_menu(CPUFreqPopup *popup,
                                               const gchar *path) {
  if (!popup->priv->govs_group) {
    GtkActionGroup *action_group;
  ...
  cpufreq_popup_build_ui(popup, popup->priv->govs_actions, path);
}

static void cpufreq_popup_build_menu(CPUFreqPopup *popup) {
  if (popup->priv->merge_id > 0) {
    gtk_ui_manager_remove_ui(popup->priv->ui_manager, popup->priv->merge_id);
...
static void cpufreq_popup_menu_set_active_action(CPUFreqPopup *popup,
                                                 GtkActionGroup *action_group,
                                                 const gchar *prefix,
                                                 const gchar *item) {
  gchar name[128];
  ...
  /* gtk_action_group_get_action can return NULL with frequencies (userspace
   * governor) when the CPU does not actually stay locked to that exact
   * frequency but rather to a frequency range. Since
   * cpufreq_monitor_get_frequency gets the realtime frequency, this may not
   * match any named frequency and then returns NULL. Return when this happens
   * to avoid segfaults
   */
  if (action == NULL) return;

  g_signal_handlers_block_by_func(
      action, cpufreq_popup_frequencies_menu_activate, popup);
  g_signal_handlers_block_by_func(action, cpufreq_popup_governors_menu_activate,
                                  popup);
  ...
  g_signal_handlers_unblock_by_func(
      action, cpufreq_popup_frequencies_menu_activate, popup);
  g_signal_handlers_unblock_by_func(
      action, cpufreq_popup_governors_menu_activate, popup);
}

static void cpufreq_popup_menu_set_active(CPUFreqPopup *popup) {
  const gchar *governor;
  ...
  if (g_ascii_strcasecmp(governor, "userspace") == 0) {
    ...
    cpufreq_popup_menu_set_active_action(popup, popup->priv->freqs_group,
                                         "Frequency", active);
    g_free(active);
  } else {
    cpufreq_popup_menu_set_active_action(popup, popup->priv->govs_group,
                                         "Governor", governor);
  }
}
...
GtkWidget *cpufreq_popup_get_menu(CPUFreqPopup *popup) {
  ...
  if (!cpufreq_utils_selector_is_available()) return NULL;
