...
static void cpufreq_applet_help_cb(GtkAction *action, CPUFreqApplet *applet);
static void cpufreq_applet_about_cb(GtkAction *action, CPUFreqApplet *applet);

static void cpufreq_applet_pixmap_set_image(CPUFreqApplet *applet, gint perc);
...
static const GtkActionEntry cpufreq_applet_menu_actions[] = {
    {"CPUFreqAppletPreferences", "document-properties", N_("_Preferences"),
     NULL, NULL, G_CALLBACK(cpufreq_applet_preferences_cb)},
    {"CPUFreqAppletHelp", "help-browser", N_("_Help"), NULL, NULL,
     G_CALLBACK(cpufreq_applet_help_cb)},
    {"CPUFreqAppletAbout", "help-about", N_("_About"), NULL, NULL,
     G_CALLBACK(cpufreq_applet_about_cb)}};

G_DEFINE_TYPE(CPUFreqApplet, cpufreq_applet, PANEL_TYPE_APPLET)

/* Enum Types */
GType cpufreq_applet_show_mode_get_type(void) {
...
GType cpufreq_applet_show_text_mode_get_type(void) {
  static GType etype = 0;

  if (etype == 0) {
    static const GEnumValue values[] = {
        {CPUFREQ_MODE_TEXT_FREQUENCY, "CPUFREQ_MODE_TEXT_FREQUENCY",
         "mode-text-frequency"},
        {CPUFREQ_MODE_TEXT_FREQUENCY_UNIT, "CPUFREQ_MODE_TEXT_FREQUENCY_UNIT",
         "mode-text-frequency-unit"},
        {CPUFREQ_MODE_TEXT_PERCENTAGE, "CPUFREQ_MODE_TEXT_PERCENTAGE",
         "mode-text-percentage"},
        {0, NULL, NULL}};
...
static void cpufreq_applet_init(CPUFreqApplet *applet) {
  ...
  gtk_box_pack_start(GTK_BOX(applet->labels_box), applet->label, FALSE, FALSE,
                     0);

  applet->unit_label = gtk_label_new(NULL);
  gtk_box_pack_start(GTK_BOX(applet->labels_box), applet->unit_label, FALSE,
                     FALSE, 0);
}
...
static void cpufreq_applet_size_allocate(GtkWidget *widget,
  ...
  GTK_WIDGET_CLASS(cpufreq_applet_parent_class)
      ->size_allocate(widget, allocation);
}

static void cpufreq_applet_menu_popup(CPUFreqApplet *applet, guint32 time) {
  GtkWidget *menu;

  if (!cpufreq_utils_selector_is_available()) return;
...
static gboolean cpufreq_applet_button_press(GtkWidget *widget,
  ...
  return GTK_WIDGET_CLASS(cpufreq_applet_parent_class)
...
static void cpufreq_applet_help_cb(GtkAction *action, CPUFreqApplet *applet) {
  ...
  gtk_show_uri_on_window(NULL, "help:mate-cpufreq-applet",
...
static void cpufreq_applet_about_cb(GtkAction *action, CPUFreqApplet *applet) {
  static const gchar *const authors[] = {
      "Carlos Garcia Campos <carlosgc@gnome.org>", NULL};
  static const gchar *documenters[] = {
      "Carlos Garcia Campos <carlosgc@gnome.org>",
      "Davyd Madeley <davyd@madeley.id.au>", N_("MATE Documentation Team"),
      NULL};
  static const gchar *const artists[] = {"Pablo Arroyo Loma <zzioma@yahoo.es>",
                                         NULL};
  ...
  gtk_show_about_dialog(NULL, "title", _("About CPU Frequency Scaling Monitor"),
                        ...
                        "mate-cpu-frequency-applet", NULL);
}
...
static void cpufreq_applet_pixmap_set_image(CPUFreqApplet *applet, gint perc) {
  ...
  if (applet->surfaces[image] == NULL) {
    GdkPixbuf *pixbuf = gdk_pixbuf_new_from_file_at_scale(
        cpufreq_icons[image], size * scale, size * scale, TRUE, NULL);
    applet->surfaces[image] =
...
static gboolean refresh_cb(CPUFreqApplet *applet) {
  cpufreq_applet_refresh(applet);

  return FALSE;
}

static void cpufreq_applet_update_visibility(CPUFreqApplet *applet) {
  CPUFreqShowMode show_mode;
  CPUFreqShowTextMode show_text_mode;
  ...
  if (changed) {
    g_object_set(G_OBJECT(applet->label), "visible",
                 applet->show_freq || applet->show_perc, NULL);
  }
  ...
  if (show_unit != applet->show_unit) {
    ...
    g_object_set(G_OBJECT(applet->unit_label), "visible", applet->show_unit,
                 NULL);
  }
  ...
  if (show_icon != applet->show_icon) {
    ...
    g_object_set(G_OBJECT(applet->icon), "visible", applet->show_icon, NULL);
  }

  if (changed) g_idle_add((GSourceFunc)refresh_cb, applet);

  if (need_update) cpufreq_applet_update(applet, applet->monitor);
}
...
static void cpufreq_applet_update(CPUFreqApplet *applet,
  ...
  if (applet->show_freq) {
    ...
    gtk_widget_set_size_request(GTK_WIDGET(applet->label), req.width,
                                req.height);
  }
  ...
  if (applet->show_unit) {
    ...
    gtk_widget_set_size_request(GTK_WIDGET(applet->unit_label), req.width,
                                req.height);
  }
...
static void cpufreq_applet_refresh(CPUFreqApplet *applet) {
  ...
  gtk_widget_get_preferred_width(GTK_WIDGET(applet->unit_label),
                                 &unit_label_size, NULL);
...
static void cpufreq_applet_prefs_cpu_changed(CPUFreqPrefs *prefs,
                                             GParamSpec *arg1,
                                             CPUFreqApplet *applet) {
...
static void cpufreq_applet_prefs_show_mode_changed(CPUFreqPrefs *prefs,
                                                   GParamSpec *arg1,
                                                   CPUFreqApplet *applet) {
...
static void cpufreq_applet_setup(CPUFreqApplet *applet) {
  ...
  /* Preferences */
  if (applet->prefs) g_object_unref(applet->prefs);

  settings = mate_panel_applet_settings_new(MATE_PANEL_APPLET(applet),
                                            "org.mate.panel.applet.cpufreq");
  applet->prefs = cpufreq_prefs_new(settings);

  g_signal_connect(applet->prefs, "notify::cpu",
                   G_CALLBACK(cpufreq_applet_prefs_cpu_changed), applet);

  g_signal_connect(applet->prefs, "notify::show-mode",
                   G_CALLBACK(cpufreq_applet_prefs_show_mode_changed), applet);
  ...
  if (GTK_IS_ACCESSIBLE(atk_obj)) {
    ...
    atk_object_set_description(
        atk_obj, _("This utility shows the current CPU Frequency"));
  }
...
MATE_PANEL_APPLET_OUT_PROCESS_FACTORY(
    "CPUFreqAppletFactory", CPUFREQ_TYPE_APPLET, "cpufreq-applet",
    (MatePanelAppletFactoryCallback)cpufreq_applet_factory, NULL)
