...
static GList *cpufreq_monitor_libcpufreq_get_available_frequencies(
    CPUFreqMonitor *monitor);
static GList *cpufreq_monitor_libcpufreq_get_available_governors(
    CPUFreqMonitor *monitor);

G_DEFINE_TYPE(CPUFreqMonitorLibcpufreq, cpufreq_monitor_libcpufreq,
...
static void cpufreq_monitor_libcpufreq_init(CPUFreqMonitorLibcpufreq *monitor) {
}

static GObject *cpufreq_monitor_libcpufreq_constructor(
  ...
  object = G_OBJECT_CLASS(cpufreq_monitor_libcpufreq_parent_class)
...
static void cpufreq_monitor_libcpufreq_class_init(
    CPUFreqMonitorLibcpufreqClass *klass) {
  ...
  monitor_class->get_available_governors =
      cpufreq_monitor_libcpufreq_get_available_governors;
}

CPUFreqMonitor *cpufreq_monitor_libcpufreq_new(guint cpu) {
  CPUFreqMonitorLibcpufreq *monitor;

  monitor = g_object_new(CPUFREQ_TYPE_MONITOR_LIBCPUFREQ, "cpu", cpu, NULL);
...
static gboolean cpufreq_monitor_libcpufreq_run(CPUFreqMonitor *monitor) {
  ...
  g_object_set(G_OBJECT(monitor), "online", TRUE, "governor", policy->governor,
               "frequency", cpufreq_get_freq_kernel(cpu), NULL);
