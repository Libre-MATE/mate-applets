...
static void cpufreq_selector_finalize(GObject *object) {
#ifdef HAVE_POLKIT
...
#ifdef HAVE_POLKIT
typedef enum { FREQUENCY, GOVERNOR } CPUFreqSelectorCall;
...
static void selector_async_data_free(SelectorAsyncData *data) {
  if (!data) return;
...
static gboolean cpufreq_selector_connect_to_system_bus(
    CPUFreqSelector *selector, GError **error) {
  if (selector->system_bus) return TRUE;
...
static gboolean cpufreq_selector_create_proxy(CPUFreqSelector *selector,
                                              GError **error) {
  if (selector->proxy) return TRUE;

  selector->proxy = g_dbus_proxy_new_sync(
      selector->system_bus, G_DBUS_PROXY_FLAGS_NONE, NULL,
      "org.mate.CPUFreqSelector", "/org/mate/cpufreq_selector/selector",
      "org.mate.CPUFreqSelector", NULL, error);

  return (selector->proxy != NULL);
}

static void selector_setter_cb(GObject *source, GAsyncResult *result,
                               gpointer user_data) {
...
static void selector_set_frequency_async(SelectorAsyncData *data) {
  ...
  g_dbus_proxy_call(data->selector->proxy, "SetFrequency",
                    g_variant_new("(uu)", data->cpu, data->frequency),
                    G_DBUS_CALL_FLAGS_NONE, -1, NULL, selector_setter_cb, data);
}

void cpufreq_selector_set_frequency_async(CPUFreqSelector *selector, guint cpu,
                                          guint frequency) {
  guint cores;
  cores = get_nprocs();
...
static void selector_set_governor_async(SelectorAsyncData *data) {
  ...
  g_dbus_proxy_call(data->selector->proxy, "SetGovernor",
                    g_variant_new("(us)", data->cpu, data->governor),
                    G_DBUS_CALL_FLAGS_NONE, -1, NULL, selector_setter_cb, data);
}

void cpufreq_selector_set_governor_async(CPUFreqSelector *selector, guint cpu,
...
#else  /* !HAVE_POLKIT */
static void cpufreq_selector_run_command(CPUFreqSelector *selector,
                                         const gchar *args) {
  gchar *command;
  gchar *path;
  GError *error = NULL;
