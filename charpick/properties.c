...
static void set_access_namedesc(GtkWidget *widget, const gchar *name,
                                const gchar *desc) {
  AtkObject *obj;
...
void add_edit_dialog_create(charpick_data *curr_data, gchar *string,
                            gchar *title) {
  GtkWidget *dialog;
  GtkWidget *entry;
  GtkWidget *dbox;
  ...
  dialog = gtk_dialog_new_with_buttons(
      _(title), GTK_WINDOW(curr_data->propwindow),
      GTK_DIALOG_DESTROY_WITH_PARENT, "gtk-cancel", GTK_RESPONSE_CANCEL,
  ...
  if (string) gtk_entry_set_text(GTK_ENTRY(entry), string);

  curr_data->add_edit_dialog = dialog;
  curr_data->add_edit_entry = entry;
}

static void add_palette_cb(GtkDialog *dialog, int response_id,
  ...
  gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(curr_data->pref_tree), path, NULL,
                               FALSE, 0.0, 0.0);

  gtk_tree_path_free(path);
}

static void edit_palette_cb(GtkDialog *dialog, int response_id,
                            charpick_data *curr_data) {
  GtkTreeSelection *selection;
  GtkTreeIter iter;
  GtkTreeModel *model;
  ...
  if (!new || (g_ascii_strcasecmp(new, charlist) == 0)) return;
...
static void add_palette(GtkButton *buttonk, charpick_data *curr_data) {
  if (curr_data->add_edit_dialog == NULL) {
    ...
    g_signal_connect(curr_data->add_edit_dialog, "response",
                     G_CALLBACK(add_palette_cb), curr_data);
...
static void edit_palette(GtkButton *button, charpick_data *curr_data) {
  GtkTreeSelection *selection;
  GtkTreeIter iter;
  GtkTreeModel *model;
  char *charlist;
  ...
  if (curr_data->add_edit_dialog == NULL) {
    selection =
        gtk_tree_view_get_selection(GTK_TREE_VIEW(curr_data->pref_tree));
    ...
    g_signal_connect(curr_data->add_edit_dialog, "response",
                     G_CALLBACK(edit_palette_cb), curr_data);
...
static void delete_palette(GtkButton *button, charpick_data *curr_data) {
  GtkTreeSelection *selection;
  GtkTreeIter iter;
  GtkTreeIter next;
  GtkTreeModel *model;
  ...
  if (g_ascii_strcasecmp(curr_data->charlist, charlist) == 0) {
    ...
    curr_data->charlist = curr_data->chartable != NULL
                              ? g_strdup(curr_data->chartable->data)
                              : g_strdup("");
    if (g_settings_is_writable(curr_data->settings, "current-list"))
      g_settings_set_string(curr_data->settings, "current-list",
                            curr_data->charlist);
...
static GtkWidget *create_palettes_tree(charpick_data *curr_data,
                                       GtkWidget *label) {
  GtkWidget *scrolled;
  GtkWidget *tree;
  GtkListStore *model;
  ...
  set_access_namedesc(tree, _("Palettes list"),
                      _("List of available palettes"));
  g_object_unref(G_OBJECT(model));
  cell = gtk_cell_renderer_text_new();
  column =
      gtk_tree_view_column_new_with_attributes("hello", cell, "text", 0, NULL);
  ...
  while (list) {
    ...
    gtk_list_store_append(GTK_LIST_STORE(model), &iter);
    gtk_list_store_set(GTK_LIST_STORE(model), &iter, 0, charlist, 1, charlist,
                       -1);
...
static void default_chars_frame_create(charpick_data *curr_data) {
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(add_palette), curr_data);
  set_access_namedesc(button, _("Add button"), _("Click to add a new palette"));

  button =
      GTK_WIDGET(g_object_new(GTK_TYPE_BUTTON, "label", CHARPICK_STOCK_EDIT,
                              "use-stock", TRUE, "use-underline", TRUE, NULL));

  gtk_box_pack_start(GTK_BOX(vbox2), button, FALSE, FALSE, 0);
  g_signal_connect(button, "clicked", G_CALLBACK(edit_palette), curr_data);
  ...
  gtk_box_pack_start(GTK_BOX(vbox2), button, FALSE, FALSE, 0);
  g_signal_connect(button, "clicked", G_CALLBACK(delete_palette), curr_data);
...
static void phelp_cb(GtkDialog *dialog, gint tab, gpointer data) {
  GError *error = NULL;
...
void show_preferences_dialog(GtkAction *action, charpick_data *curr_data) {
  ...
  curr_data->propwindow = gtk_dialog_new_with_buttons(
      _("Character Palette Preferences"), NULL, GTK_DIALOG_DESTROY_WITH_PARENT,
      "gtk-close", GTK_RESPONSE_CLOSE, "gtk-help", GTK_RESPONSE_HELP, NULL);
  gtk_window_set_screen(GTK_WINDOW(curr_data->propwindow),
                        gtk_widget_get_screen(curr_data->applet));
  ...
  gtk_box_set_spacing(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(curr_data->propwindow))),
      2);
  gtk_dialog_set_default_response(GTK_DIALOG(curr_data->propwindow),
                                  GTK_RESPONSE_CLOSE);

  default_chars_frame_create(curr_data);
  g_signal_connect(curr_data->propwindow, "response", G_CALLBACK(response_cb),
                   curr_data);
