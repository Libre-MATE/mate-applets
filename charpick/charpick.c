...
static const gunichar def_code[] = {225, 224, 233,  237,  241, 243,
                                    189, 169, 1579, 8364, 0};
...
static const gunichar cap_a_code[] = {192, 193, 194, 195, 196,
                                      197, 198, 170, 0};
/* ccedil, cent, copy */
...
static const gunichar ZA_code[] = {7699, 7741, 7755, 7793, 7698, 7740, 7754,
                                   7792, 7749, 7748, 353,  352,  0};
/* South Africa: Afrikaans */
/* static const gchar *af_ZA_list = "áéíóúýêîôûèäëïöüÁÉÍÓÚÝÊÎÔÛÈÄËÏÖÜ"; */
static const gunichar af_ZA_code[] = {
    225, 233, 237, 243, 250, 253, 234, 238, 244, 251, 232,
    228, 235, 239, 246, 252, 193, 201, 205, 211, 218, 221,
    202, 206, 212, 219, 200, 196, 203, 207, 214, 220, 0};

static const gunichar *const chartable[] = {
    def_code,      a_code,     cap_a_code, c_code,  e_code,     cap_e_code,
    i_code,        cap_i_code, n_code,     o_code,  cap_o_code, s_code,
    t_code,        u_code,     cap_u_code, y_code,  dash_code,  quote_code,
    currency_code, one_code,   misc_code,  ZA_code, af_ZA_code};

/* sets the picked character as the selection when it gets a request */
static void charpick_selection_handler(GtkWidget *widget,
...
static gint selection_clear_cb(GtkWidget *widget, GdkEventSelection *event,
                               gpointer data) {
...
static gint toggle_button_toggled_cb(GtkToggleButton *button, gpointer data) {
  charpick_data *curr_data = data;
  gint button_index;
  gboolean toggled;
  ...
  if (toggled) {
    gunichar unichar;
    if (curr_data->last_toggle_button &&
        (button != curr_data->last_toggle_button))
      gtk_toggle_button_set_active(curr_data->last_toggle_button, FALSE);
    ...
    gtk_selection_owner_set(curr_data->invisible, GDK_SELECTION_PRIMARY,
                            GDK_CURRENT_TIME);
    gtk_selection_owner_set(curr_data->invisible, GDK_SELECTION_CLIPBOARD,
                            GDK_CURRENT_TIME);
 ...
 */
static gboolean button_press_hack(GtkWidget *widget, GdkEventButton *event,
                                  GtkWidget *applet) {
...
static void menuitem_activated(GtkMenuItem *menuitem,
                               charpick_data *curr_data) {
  gchar *string;

  string = g_object_get_data(G_OBJECT(menuitem), "string");
  if (g_ascii_strcasecmp(curr_data->charlist, string) == 0) return;
  ...
  if (g_settings_is_writable(curr_data->settings, "current-list"))
    g_settings_set_string(curr_data->settings, "current-list",
                          curr_data->charlist);
}
...
void populate_menu(charpick_data *curr_data) {
  ...
  if (curr_data->menu) gtk_widget_destroy(curr_data->menu);
  ...
  while (list) {
    ...
    g_signal_connect(menuitem, "activate", G_CALLBACK(menuitem_activated),
                     curr_data);
...
static void chooser_button_clicked(GtkButton *button,
                                   charpick_data *curr_data) {
  if (gtk_widget_get_visible(curr_data->menu))
    ...
    gtk_menu_popup_at_widget(GTK_MENU(curr_data->menu), GTK_WIDGET(button),
...
/* Force the button not to have any focus padding and let the focus
   indication be drawn on the label itself when space is tight. Taken from the
   clock applet.
   FIXME : This is an Evil Hack and should be fixed when the focus padding can
   be overridden at the gtk+ level */
...
void build_table(charpick_data *p_curr_data) {
  ...
  if (p_curr_data->box) gtk_widget_destroy(p_curr_data->box);
  ...
  if (g_list_length(p_curr_data->chartable) != 1) {
    ...
    switch (
        mate_panel_applet_get_orient(MATE_PANEL_APPLET(p_curr_data->applet))) {
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(chooser_button_clicked),
                     p_curr_data);
  ...
  for (i = 0; i < len; i++) {
    ...
    g_signal_connect(toggle_button[i], "toggled",
                     G_CALLBACK(toggle_button_toggled_cb), p_curr_data);
  ...
  for (i = 0; i < len; i++) {
    ...
    gtk_box_pack_start(GTK_BOX(row_box[index]), toggle_button[i], TRUE, TRUE,
                       0);
  }
...
static gboolean rebuild_cb(gpointer user_data) {
  charpick_data *curr_data;
...
static void queue_rebuild(charpick_data *curr_data) {
  if (curr_data->rebuild_id != 0) return;

  curr_data->rebuild_id = g_idle_add(rebuild_cb, curr_data);
  g_source_set_name_by_id(curr_data->rebuild_id, "[charpick] rebuild_cb");
}

static void applet_size_allocate(MatePanelApplet *applet,
                                 GtkAllocation *allocation, gpointer data) {
  charpick_data *curr_data = data;
  if (curr_data->panel_vertical) {
    if (curr_data->panel_size == allocation->width) return;
    curr_data->panel_size = allocation->width;
  } else {
    if (curr_data->panel_size == allocation->height) return;
...
static void applet_change_orient(MatePanelApplet *applet,
                                 MatePanelAppletOrient o, gpointer data) {
  charpick_data *curr_data = data;
  if (o == MATE_PANEL_APPLET_ORIENT_UP || o == MATE_PANEL_APPLET_ORIENT_DOWN)
...
static void about(GtkAction *action, charpick_data *curr_data) {
  static const char *const authors[] = {"Alexandre Muñiz <munizao@xprt.net>",
                                        "Kevin Vandersloot", NULL};
  ...
  gtk_show_about_dialog(NULL, "title", _("About Character Palette"), "version",
                        VERSION, "copyright",
                        ...
                        "logo-icon-name", "accessories-character-map", NULL);
}

static void help_cb(GtkAction *action, charpick_data *curr_data) {
  GError *error = NULL;

  gtk_show_uri_on_window(NULL, "help:mate-char-palette",
...
static void applet_destroy(GtkWidget *widget, gpointer data) {
  ...
  if (curr_data->about_dialog) gtk_widget_destroy(curr_data->about_dialog);
  if (curr_data->propwindow) gtk_widget_destroy(curr_data->propwindow);
  if (curr_data->box) gtk_widget_destroy(curr_data->box);
  if (curr_data->menu) gtk_widget_destroy(curr_data->menu);
  if (curr_data->invisible) gtk_widget_destroy(curr_data->invisible);
  g_free(curr_data->charlist);
  g_free(curr_data);
}

void save_chartable(charpick_data *curr_data) {
  mate_panel_applet_settings_set_glist(curr_data->settings, "chartable",
                                       curr_data->chartable);
}

static void get_chartable(charpick_data *curr_data) {
  gint i, n;
  GList *value = NULL;

  value =
      mate_panel_applet_settings_get_glist(curr_data->settings, "chartable");
  if (value) {
...
static const GtkActionEntry charpick_applet_menu_actions[] = {
    ...
    {"About", "help-about", N_("_About"), NULL, NULL, G_CALLBACK(about)}};
...
void set_atk_name_description(GtkWidget *widget, const gchar *name,
  ...
  if (GTK_IS_ACCESSIBLE(aobj) == FALSE) return;
...
static void make_applet_accessible(GtkWidget *applet) {
  set_atk_name_description(applet, _("Character Palette"),
                           _("Insert characters"));
}

static gboolean charpicker_applet_fill(MatePanelApplet *applet) {
  ...
  curr_data->settings =
      mate_panel_applet_settings_new(applet, "org.mate.panel.applet.charpick");
  curr_data->rebuild_id = 0;
  ...
  gtk_selection_add_target(curr_data->invisible, GDK_SELECTION_PRIMARY,
                           utf8_atom, 0);

  gtk_selection_add_target(curr_data->invisible, GDK_SELECTION_CLIPBOARD,
                           utf8_atom, 0);

  g_signal_connect(curr_data->invisible, "selection-get",
                   G_CALLBACK(charpick_selection_handler), curr_data);

  g_signal_connect(curr_data->invisible, "selection-clear-event",
                   G_CALLBACK(selection_clear_cb), curr_data);

  make_applet_accessible(GTK_WIDGET(applet));

  /* session save signal */
  g_signal_connect(applet, "change-orient", G_CALLBACK(applet_change_orient),
                   curr_data);

  g_signal_connect(applet, "size-allocate", G_CALLBACK(applet_size_allocate),
                   curr_data);

  g_signal_connect(applet, "destroy", G_CALLBACK(applet_destroy), curr_data);

  gtk_widget_show_all(GTK_WIDGET(applet));

  action_group = gtk_action_group_new("Charpicker Applet Actions");
  gtk_action_group_set_translation_domain(action_group, GETTEXT_PACKAGE);
  gtk_action_group_add_actions(action_group, charpick_applet_menu_actions,
                               G_N_ELEMENTS(charpick_applet_menu_actions),
                               curr_data);

  mate_panel_applet_setup_menu_from_resource(
      MATE_PANEL_APPLET(applet),
      CHARPICK_RESOURCE_PATH "charpick-applet-menu.xml", action_group);
...
static gboolean charpicker_applet_factory(MatePanelApplet *applet,
                                          const gchar *iid, gpointer data) {
  gboolean retval = FALSE;

  if (!strcmp(iid, "CharpickerApplet")) retval = charpicker_applet_fill(applet);

  return retval;
}

MATE_PANEL_APPLET_OUT_PROCESS_FACTORY("CharpickerAppletFactory",
                                      PANEL_TYPE_APPLET, "char-palette",
                                      charpicker_applet_factory, NULL)
