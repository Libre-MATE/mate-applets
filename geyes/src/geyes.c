...
static gfloat gtk_align_to_gfloat(GtkAlign align) {
  switch (align) {
...
static void calculate_pupil_xy(EyesApplet *eyes_applet, gint x, gint y,
  ...
  nx = (float)x -
       (float)MAX(allocation.width - eyes_applet->eye_width, 0) * xalign -
       eye_width_half;
  ny = (float)y -
       (float)MAX(allocation.height - eyes_applet->eye_height, 0) * yalign -
       eye_height_half;

  h = hypotf(nx, ny);
  if ((h < 0.5f) || (fabs(h) < (fabs(hypotf(eye_height_half, eye_width_half) -
...
static void draw_eye(EyesApplet *eyes_applet, gsize eye_num, gint pupil_x,
                     gint pupil_y) {
  GdkPixbuf *pixbuf;
  ...
  gdk_pixbuf_composite(eyes_applet->pupil_image, pixbuf, rect.x, rect.y,
...
static gint timer_cb(EyesApplet *eyes_applet) {
  GdkDisplay *display;
  GdkSeat *seat;
  gint x, y, dx, dy;
  ...
  for (i = 0; i < eyes_applet->num_eyes; i++) {
    if (gtk_widget_get_realized(eyes_applet->eyes[i])) {
      gdk_window_get_device_position(
          gtk_widget_get_window(eyes_applet->eyes[i]),
          gdk_seat_get_pointer(seat), &x, &y, NULL);
      gtk_widget_translate_coordinates(
          eyes_applet->eyes[i], gtk_widget_get_toplevel(eyes_applet->eyes[i]),
          0, 0, &dx, &dy);
      x -= dx;
      y -= dy;
...
static void about_cb(GtkAction *action, EyesApplet *eyes_applet) {
  static const gchar *authors[] = {"Dave Camp <campd@oit.edu>", NULL};
  ...
  gtk_show_about_dialog(
      NULL, "title", _("About Eyes"), "version", VERSION, "comments",
      _("A goofy set of eyes for the MATE "
...
static gboolean properties_load(EyesApplet *eyes_applet) {
...
void setup_eyes(EyesApplet *eyes_applet) {
  ...
  gtk_box_pack_start(GTK_BOX(eyes_applet->vbox), eyes_applet->hbox, TRUE, TRUE,
                     0);
  ...
  for (i = 0; i < eyes_applet->num_eyes; i++) {
    ...
    gtk_box_pack_start(GTK_BOX(eyes_applet->hbox), eyes_applet->eyes[i], TRUE,
                       TRUE, 0);
...
static EyesApplet *create_eyes(MatePanelApplet *applet) {
  ...
  eyes_applet->settings =
      mate_panel_applet_settings_new(applet, GEYES_SETTINGS_SCHEMA);
  gtk_container_add(GTK_CONTAINER(applet), eyes_applet->vbox);
  return eyes_applet;
}

static void destroy_cb(GObject *object, EyesApplet *eyes_applet) {
  g_return_if_fail(eyes_applet);

  g_source_remove(eyes_applet->timeout_id);

  if (eyes_applet->hbox) destroy_eyes(eyes_applet);
  eyes_applet->timeout_id = 0;

  if (eyes_applet->eye_image) g_object_unref(eyes_applet->eye_image);
  eyes_applet->eye_image = NULL;

  if (eyes_applet->pupil_image) g_object_unref(eyes_applet->pupil_image);
  ...
  if (eyes_applet->settings) g_object_unref(eyes_applet->settings);
  eyes_applet->settings = NULL;

  g_free(eyes_applet);
}

static void help_cb(GtkAction *action, EyesApplet *eyes_applet) {
  GError *error = NULL;

  gtk_show_uri_on_window(NULL, "help:mate-geyes", gtk_get_current_event_time(),
                         &error);
...
static gboolean geyes_applet_fill(MatePanelApplet *applet) {
  ...
  mate_panel_applet_setup_menu_from_resource(
      eyes_applet->applet, GEYES_RESOURCE_PATH "geyes-applet-menu.xml",
      action_group);
  ...
  set_atk_name_description(
      GTK_WIDGET(eyes_applet->applet), _("Eyes"),
      _("The eyes look in the direction of the mouse pointer"));

  g_signal_connect(eyes_applet->vbox, "destroy", G_CALLBACK(destroy_cb),
                   eyes_applet);
  ...
  if ((result = properties_load(eyes_applet)) == TRUE) setup_eyes(eyes_applet);

  return result;
}

static gboolean geyes_applet_factory(MatePanelApplet *applet, const gchar *iid,
                                     gpointer data) {
  gboolean retval = FALSE;

  theme_dirs_create();

  if (!strcmp(iid, "GeyesApplet")) retval = geyes_applet_fill(applet);
...
MATE_PANEL_APPLET_OUT_PROCESS_FACTORY("GeyesAppletFactory", PANEL_TYPE_APPLET,
                                      "geyes", geyes_applet_factory, NULL)
