...
enum { COL_THEME_DIR = 0, COL_THEME_NAME, TOTAL_COLS };

void theme_dirs_create(void) {
  static gboolean themes_created = FALSE;

  if (themes_created == TRUE) return;

  theme_directories[0] = g_build_filename(GEYES_THEMES_DIR, NULL);

  theme_directories[1] =
      g_build_filename(g_get_user_config_dir(), "mate", "geyes-themes", NULL);

  themes_created = TRUE;
}

static void parse_theme_file(EyesApplet *eyes_applet, FILE *theme_file) {
  gchar line_buf[512]; /* prolly overkill */
  gchar *token;

  if (fgets(line_buf, 512, theme_file) == NULL) g_debug("fgets error");
  ...
  while (!feof(theme_file)) {
    ...
    if (strncmp(token, "wall-thickness", strlen("wall-thickness")) == 0) {
      ...
      if (eyes_applet->num_eyes > MAX_EYES) eyes_applet->num_eyes = MAX_EYES;
    } else if (strncmp(token, "eye-pixmap", strlen("eye-pixmap")) == 0) {
      token = strtok(NULL, "\"");
      token = strtok(NULL, "\"");
      if (eyes_applet->eye_filename != NULL) g_free(eyes_applet->eye_filename);
...
int load_theme(EyesApplet *eyes_applet, const gchar *theme_dir) {
  ...
  if ((theme_file = fopen(file_name, "r")) == NULL) {
    ...
    eyes_applet->theme_dir =
        g_build_filename(GEYES_THEMES_DIR, "Default-tiny", NULL);
  ...
  if (theme_file == NULL) {
    dialog = gtk_message_dialog_new_with_markup(
        ...
        "<b>%s</b>\n\n%s", _("Can not launch the eyes applet."),
  ...
  eyes_applet->eye_image =
      gdk_pixbuf_new_from_file(eyes_applet->eye_filename, NULL);

  if (eyes_applet->pupil_image) g_object_unref(eyes_applet->pupil_image);

  eyes_applet->pupil_image =
      gdk_pixbuf_new_from_file(eyes_applet->pupil_filename, NULL);
...
static void destroy_theme(EyesApplet *eyes_applet) {
...
static void theme_selected_cb(GtkTreeSelection *selection, gpointer data) {
  ...
  g_settings_set_string(eyes_applet->settings, GEYES_SETTINGS_THEME_PATH_KEY,
                        theme);

  g_free(theme);
}

static void phelp_cb(GtkDialog *dialog) {
  GError *error = NULL;
...
static void presponse_cb(GtkDialog *dialog, gint id, gpointer data) {
  EyesApplet *eyes_applet = data;
...
void properties_cb(GtkAction *action, EyesApplet *eyes_applet) {
  ...
  if (!g_settings_is_writable(eyes_applet->settings,
  ...
  for (i = 0; i < NUM_THEME_DIRECTORIES; i++) {
    ...
    while ((dp = g_dir_read_name(dfd)) != NULL) {
      if (dp[0] != '.') {
        ...
        gtk_list_store_set(model, &iter, COL_THEME_DIR, theme_dir,
                           COL_THEME_NAME, dp, -1);
  ...
  gtk_builder_add_callback_symbols(
      builder, "on_preferences_dialog_response", G_CALLBACK(presponse_cb),
      "on_themes_treeselection_changed", G_CALLBACK(theme_selected_cb), NULL);
