...
static gboolean read_cb(GIOChannel *source, GIOCondition condition,
                        gpointer user_data) {
  GTask *task;
  CommandData *data;
  ...
  error = NULL;
  status =
      g_io_channel_read_chars(source, buffer, BUFFER_SIZE, &bytes_read, &error);
...
static void command_data_free(gpointer user_data) {
  CommandData *data;
...
static gboolean ma_command_initable_init(GInitable *initable,
  ...
  if (command->command == NULL || *command->command == '\0') {
    g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA, "Empty command");
...
static void ma_command_finalize(GObject *object) {
  MaCommand *command;
...
static void ma_command_set_property(GObject *object, guint property_id,
...
static void ma_command_get_property(GObject *object, guint prop_id,
...
static void install_properties(GObjectClass *object_class) {
  command_properties[PROP_COMMAND] = g_param_spec_string(
      "command", "command", "command", NULL, G_PARAM_READWRITE);

  g_object_class_install_properties(object_class, LAST_PROP,
                                    command_properties);
}

static void ma_command_class_init(MaCommandClass *command_class) {
...
static void ma_command_init(MaCommand *command) {}
...
MaCommand *ma_command_new(const gchar *command, GError **error) {
  return g_initable_new(MA_TYPE_COMMAND, NULL, error, "command", command, NULL);
}
...
void ma_command_run_async(MaCommand *command, GCancellable *cancellable,
  ...
  if (cancellable) {
    g_signal_connect_object(cancellable, "cancelled", G_CALLBACK(cancelled_cb),
  ...
  if (!g_spawn_async_with_pipes(NULL, command->argv, NULL, spawn_flags, NULL,
                                NULL, &data->pid, NULL, &command_stdout, NULL,
                                &error)) {
    g_task_return_error(task, error);
    g_object_unref(task);
