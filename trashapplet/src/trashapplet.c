...
G_DECLARE_FINAL_TYPE(TrashApplet, trash_applet, TRASH, APPLET, MatePanelApplet)
...
static void trash_applet_do_empty(GtkAction *action, TrashApplet *applet);
static void trash_applet_show_about(GtkAction *action, TrashApplet *applet);
static void trash_applet_open_folder(GtkAction *action, TrashApplet *applet);
static void trash_applet_show_help(GtkAction *action, TrashApplet *applet);

static const GtkActionEntry trash_applet_menu_actions[] = {
    {"EmptyTrash", "edit-clear", N_("_Empty Trash"), NULL, NULL,
     G_CALLBACK(trash_applet_do_empty)},
    {"OpenTrash", "document-open", N_("_Open Trash"), NULL, NULL,
     G_CALLBACK(trash_applet_open_folder)},
    {"HelpTrash", "help-browser", N_("_Help"), NULL, NULL,
     G_CALLBACK(trash_applet_show_help)},
    {"AboutTrash", "help-about", N_("_About"), NULL, NULL,
     G_CALLBACK(trash_applet_show_about)}};

static void trash_applet_monitor_changed(TrashApplet *applet) {
  GError *error = NULL;
  GFileInfo *info;
  GIcon *icon;
  AtkObject *atk_obj;
  ...
  if (!g_icon_equal(icon, applet->icon)) {
     ...
     */
    gtk_image_set_from_gicon(GTK_IMAGE(applet->image), icon,
                             GTK_ICON_SIZE_MENU);

    if (applet->icon) g_object_unref(applet->icon);
  ...
  if (items != applet->items) {
    ...
    if (items) {
      text = g_strdup_printf(
          ngettext("%d Item in Trash", "%d Items in Trash", items), items);
    } else
...
static void trash_applet_set_icon_size(TrashApplet *applet, gint size) {
...
static void trash_applet_dispose(GObject *object) {
  ...
  if (applet->trash_monitor) g_object_unref(applet->trash_monitor);
  applet->trash_monitor = NULL;

  if (applet->trash) g_object_unref(applet->trash);
  applet->trash = NULL;

  if (applet->image) g_object_unref(applet->image);
  applet->image = NULL;
...
static void trash_applet_init(TrashApplet *applet) {
  ...
  g_signal_connect_swapped(applet->trash_monitor, "changed",
                           G_CALLBACK(trash_applet_monitor_changed), applet);

  /* setup drag and drop */
  gtk_drag_dest_set(GTK_WIDGET(applet), GTK_DEST_DEFAULT_ALL, drop_types,
                    G_N_ELEMENTS(drop_types), GDK_ACTION_MOVE);
...
static gboolean trash_applet_button_release(GtkWidget *widget,
  ...
  if (settings == NULL) settings = g_settings_new(PANEL_SCHEMA);
...
static gboolean trash_applet_key_press(GtkWidget *widget, GdkEventKey *event) {
  TrashApplet *applet = TRASH_APPLET(widget);
...
static gboolean trash_applet_drag_motion(GtkWidget *widget,
  ...
  for (target = gdk_drag_context_list_targets(context); target;
    ...
    if (!strcmp(name, "application/x-panel-icon-internal")) break;
  }
...
static void error_dialog(TrashApplet *applet, const gchar *error, ...) {
  va_list args;
  gchar *error_string;
  GtkWidget *dialog;
...
static void trash_applet_open_folder(GtkAction *action, TrashApplet *applet) {
  ...
  gtk_show_uri_on_window(NULL, "trash:", gtk_get_current_event_time(), &err);
...
static void trash_applet_show_help(GtkAction *action, TrashApplet *applet) {
  ...
  gtk_show_uri_on_window(NULL, "help:mate-trashapplet",
                         gtk_get_current_event_time(), &err);

  if (err) {
    error_dialog(applet, _("There was an error displaying help: %s"),
                 err->message);
    g_error_free(err);
  }
}
...
static void trash_applet_show_about(GtkAction *action, TrashApplet *applet) {
  ...
  static const char *documenters[] = {"Michiel Sikkes <michiel@eyesopened.nl>",
  ...
  gtk_show_about_dialog(
      ...
      _("Copyright \xc2\xa9 2004 Michiel Sikkes\n"
        "Copyright \xc2\xa9 2008 Ryan Lortie\n"
        "Copyright \xc2\xa9 2012-2021 MATE developers"),
      "comments",
      _("A MATE trash bin that lives in your panel. "
      ...
      _("translator-credits"), "logo_icon_name", "user-trash-full", NULL);
}

static gboolean confirm_delete_immediately(GtkWidget *parent_view,
                                           gint num_files, gboolean all) {
  GdkScreen *screen;
  ...
  if (all) {
    ...
    detail = g_strdup_printf(
        "None of the %d selected items can be moved to the Trash", num_files);
  } else {
    prompt =
        _("Cannot move some items to trash, do you want to delete these "
          "immediately?");
    detail = g_strdup_printf(
        "%d of the selected items cannot be moved to the Trash", num_files);
  }

  str = g_strconcat("<span weight=\"bold\" size=\"larger\">", prompt, "</span>",
                    NULL);
  ...
  gtk_dialog_add_button(GTK_DIALOG(dialog), "gtk-delete", GTK_RESPONSE_YES);
...
static void trash_applet_drag_data_received(GtkWidget *widget,
  ...
  list = g_uri_list_extract_uris(
      (gchar *)gtk_selection_data_get_data(selectiondata));
  ...
  if (untrashable) {
    if (confirm_delete_immediately(widget, g_list_length(untrashable),
                                   trashed == NULL)) {
...
static void trash_applet_class_init(TrashAppletClass *class) {
...
static gboolean trash_applet_factory(MatePanelApplet *applet, const gchar *iid,
                                     gpointer data) {
  ...
  if (!strcmp(iid, "TrashApplet")) {
    ...
    gtk_action_group_add_actions(action_group, trash_applet_menu_actions,
                                 G_N_ELEMENTS(trash_applet_menu_actions),
                                 applet);
    mate_panel_applet_setup_menu_from_resource(
        applet, GRESOURCE "trashapplet-menu.xml", action_group);
...
MATE_PANEL_APPLET_OUT_PROCESS_FACTORY("TrashAppletFactory", TRASH_TYPE_APPLET,
                                      "TrashApplet", trash_applet_factory, NULL)
