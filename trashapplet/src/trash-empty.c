...
static gboolean trash_empty_update_dialog(gpointer user_data) {
  gsize deleted, total;
  GFile *file;
  ...
  if (trash_empty_dialog) {
    ...
    text = g_strdup_printf(_("Removing item %s of %s"), index_str, total_str);
...
static gboolean trash_empty_done(gpointer user_data) {
...
static void trash_empty_maybe_schedule_update(GIOSchedulerJob *job, GFile *file,
                                              gsize deleted) {
  if (!trash_empty_update_pending) {
    ...
    g_io_scheduler_job_send_to_mainloop_async(job, trash_empty_update_dialog,
                                              NULL, NULL);
  }
}

static void trash_empty_delete_contents(GIOSchedulerJob *job,
...
static gboolean trash_empty_job(GIOSchedulerJob *job, GCancellable *cancellable,
                                gpointer user_data) {
  gsize deleted;
  GFile *trash;
  ...
  g_io_scheduler_job_send_to_mainloop_async(job, trash_empty_done, NULL, NULL);

  return FALSE;
}
/* ================ worker thread code ends here ================ */

static void trash_empty_start(GtkWidget *parent) {
  struct {
    const char *name;
    gpointer *pointer;
  } widgets[] = {{"empty_trash", (gpointer *)&trash_empty_dialog},
  ...
  builder =
      gtk_builder_new_from_resource(GRESOURCE "trashapplet-empty-progress.ui");
...
static gboolean trash_empty_require_confirmation(void) {
...
static void trash_empty_confirmation_response(GtkDialog *dialog,
                                              gint response_id,
                                              gpointer user_data) {
  if (response_id == GTK_RESPONSE_YES) trash_empty_start(GTK_WIDGET(dialog));
 ...
 */
static void trash_empty_show_confirmation_dialog(GtkWidget *parent) {
  GtkWidget *dialog;
  GtkWidget *button;
  GdkScreen *screen;
