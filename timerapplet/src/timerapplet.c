...
#define GET_SPIN_BUTTON(x) \
  (GTK_SPIN_BUTTON(gtk_builder_get_object(builder, (x))))
...
static const GtkActionEntry applet_menu_actions[] = {
    {"Start", "media-playback-start", N_("_Start timer"), NULL, NULL,
     G_CALLBACK(timer_start_callback)},
    {"Pause", "media-playback-pause", N_("P_ause timer"), NULL, NULL,
     G_CALLBACK(timer_pause_callback)},
    {"Stop", "media-playback-stop", N_("S_top timer"), NULL, NULL,
     G_CALLBACK(timer_stop_callback)},
    {"Reset", "edit-undo", N_("R_eset"), NULL, NULL,
     G_CALLBACK(timer_reset_callback)},
    {"Preferences", "document-properties", N_("_Preferences"), NULL, NULL,
     G_CALLBACK(timer_preferences_callback)},
    {"About", "help-about", N_("_About"), NULL, NULL,
     G_CALLBACK(timer_about_callback)}};

static char *ui =
    "<menuitem name='Item 1' action='Start' />"
...
static void timer_applet_destroy(MatePanelApplet *applet_widget,
                                 TimerApplet *applet) {
  g_assert(applet);
...
/* timer management */
static gboolean timer_callback(TimerApplet *applet) {
  AtkObject *atk_obj;
  ...
  if (!applet->active) {
    ...
    if (!applet->pause) applet->elapsed += STEP;
    ...
    if (remaining <= 0) {
      ...
      if (hours > 0)
        tooltip = g_strdup_printf("%s (%02d:%02d:%02d)", name, hours, minutes,
                                  seconds);
      else
  ...
  gtk_action_set_sensitive(
      gtk_action_group_get_action(applet->action_group, "Start"),
      !applet->active || applet->pause);
  gtk_action_set_sensitive(
      gtk_action_group_get_action(applet->action_group, "Pause"),
      applet->active && !applet->pause);
  gtk_action_set_sensitive(
      gtk_action_group_get_action(applet->action_group, "Stop"),
      applet->active);
  gtk_action_set_sensitive(
      gtk_action_group_get_action(applet->action_group, "Reset"),
      !applet->active && !applet->pause && applet->elapsed);
  gtk_action_set_sensitive(
      gtk_action_group_get_action(applet->action_group, "Preferences"),
      !applet->active && !applet->pause);
...
static void timer_spin_button_value_changed(GtkSpinButton *spinbutton,
                                            TimerApplet *applet) {
  gint duration;
...
static void timer_preferences_callback(GtkAction *action, TimerApplet *applet) {
  ...
  builder = gtk_builder_new_from_resource(
      "/org/mate/mate-applets/timerapplet/timerapplet-preferences.ui");
  ...
  g_settings_bind(applet->settings, NAME_KEY, GET_WIDGET("name_entry"), "text",
                  G_SETTINGS_BIND_DEFAULT);
  g_settings_bind(applet->settings, SHOW_NOTIFICATION_KEY,
                  GET_WIDGET("show_notification_popup_check"), "active",
                  G_SETTINGS_BIND_DEFAULT);
  g_settings_bind(applet->settings, SHOW_DIALOG_KEY,
                  GET_WIDGET("show_dialog_check"), "active",
                  G_SETTINGS_BIND_DEFAULT);

  /* signals */
  gtk_builder_add_callback_symbols(builder, "on_hours_spinbutton_value_changed",
                                   G_CALLBACK(timer_spin_button_value_changed),
                                   "on_minutes_spinbutton_value_changed",
                                   G_CALLBACK(timer_spin_button_value_changed),
...
static gboolean timer_applet_click(TimerApplet *applet) {
...
static void timer_settings_changed(GSettings *settings, gchar *key,
                                   TimerApplet *applet) {
  timer_callback(applet);
}

static gboolean timer_applet_fill(MatePanelApplet *applet_widget) {
  TimerApplet *applet;
  AtkObject *atk_obj;
  ...
  if (!notify_is_initted()) notify_init("timer-applet");
  ...
  applet->settings =
      mate_panel_applet_settings_new(applet_widget, TIMER_SCHEMA);
  applet->timeout_id = 0;
  applet->active = FALSE;
  applet->pause = FALSE;

  applet->box = GTK_BOX(gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0));
  applet->image = GTK_IMAGE(
      gtk_image_new_from_icon_name(APPLET_ICON, GTK_ICON_SIZE_BUTTON));
  applet->pause_image = GTK_IMAGE(gtk_image_new_from_icon_name(
  ...
  gtk_box_pack_start(applet->box, GTK_WIDGET(applet->image), TRUE, TRUE, 0);
  gtk_box_pack_start(applet->box, GTK_WIDGET(applet->pause_image), TRUE, TRUE,
                     0);
  gtk_box_pack_start(applet->box, GTK_WIDGET(applet->label), TRUE, TRUE, 3);

  gtk_container_add(GTK_CONTAINER(applet_widget), GTK_WIDGET(applet->box));

  gtk_widget_show_all(GTK_WIDGET(applet->applet));
  gtk_widget_hide(GTK_WIDGET(applet->pause_image));

  g_signal_connect(applet->applet, "destroy", G_CALLBACK(timer_applet_destroy),
                   applet);
...
static gboolean timer_factory(MatePanelApplet *applet, const char *iid,
                              gpointer data) {
  gboolean retval = FALSE;

  if (!g_strcmp0(iid, "TimerApplet")) retval = timer_applet_fill(applet);

  return retval;
}

/* needed by mate-panel applet library */
MATE_PANEL_APPLET_OUT_PROCESS_FACTORY("TimerAppletFactory", PANEL_TYPE_APPLET,
                                      "Timer applet", timer_factory, NULL)
