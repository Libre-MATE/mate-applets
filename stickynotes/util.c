...
static Atom xstuff_atom_get(const char *atom_name) {
  ...
  if (!atom_hash)
    atom_hash = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
...
int xstuff_get_current_workspace(GtkWindow *window) {
  ...
  result = XGetWindowProperty(xdisplay, root_window,
                              xstuff_atom_get("_NET_CURRENT_DESKTOP"), 0,
                              G_MAXLONG, False, XA_CARDINAL, &type, &format,
                              &nitems, &bytes_after, (gpointer)&num);
...
void xstuff_change_workspace(GtkWindow *window, int new_space) {
  XEvent xev;
  Window xwindow;
  Display *gdk_display;
  Screen *screen;
