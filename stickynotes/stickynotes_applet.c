...
static const GtkActionEntry stickynotes_applet_menu_actions[] = {
    {"new_note", "document-new", N_("_New Note"), NULL, NULL,
     G_CALLBACK(menu_new_note_cb)},
    {"hide_notes", NULL, N_("Hi_de Notes"), NULL, NULL,
     G_CALLBACK(menu_hide_notes_cb)},
    {"destroy_all", "edit-delete", N_("_Delete Notes"), NULL, NULL,
     G_CALLBACK(menu_destroy_all_cb)},
    {"preferences", "document-properties", N_("_Preferences"), NULL, NULL,
     G_CALLBACK(menu_preferences_cb)},
    {"help", "help-browser", N_("_Help"), NULL, NULL, G_CALLBACK(menu_help_cb)},
...
static const GtkToggleActionEntry stickynotes_applet_menu_toggle_actions[] = {
    {"lock", NULL, N_("_Lock Notes"), NULL, NULL,
     G_CALLBACK(menu_toggle_lock_cb), FALSE}};

/* Sticky Notes applet factory */
static gboolean stickynotes_applet_factory(MatePanelApplet *mate_panel_applet,
                                           const gchar *iid, gpointer data) {
  if (!strcmp(iid, "StickyNotesApplet")) {
    if (!stickynotes) stickynotes_applet_init(mate_panel_applet);

    mate_panel_applet_set_flags(mate_panel_applet,
                                MATE_PANEL_APPLET_EXPAND_MINOR);

    /* Add applet to linked list of all applets */
    stickynotes->applets = g_list_append(
        stickynotes->applets, stickynotes_applet_new(mate_panel_applet));
...
MATE_PANEL_APPLET_OUT_PROCESS_FACTORY("StickyNotesAppletFactory",
                                      PANEL_TYPE_APPLET, "stickynotes_applet",
                                      stickynotes_applet_factory, NULL)

/* colorshift a surface */
static void stickynotes_make_prelight_icon(cairo_surface_t *dest,
                                           cairo_surface_t *src, int shift) {
  gint i, j;
...
static void icon_theme_changed(GtkIconTheme *icon_theme, gpointer user_data) {
  gtk_icon_theme_append_search_path(icon_theme,
                                    PKG_DATA_DIR G_DIR_SEPARATOR_S "icons");
}

static void stickynotes_applet_init_icons(void) {
...
static void stickynotes_destroy(GtkWidget *widget, gpointer user_dta) {
...
void stickynotes_applet_init(MatePanelApplet *mate_panel_applet) {
  ...
  stickynotes->icon_normal = gtk_icon_theme_load_surface(
      gtk_icon_theme_get_default(), "mate-sticky-notes-applet", size, scale,
      NULL, 0, NULL);

  stickynotes->icon_prelight = cairo_surface_create_similar(
...
void stickynotes_applet_init_prefs(void) {
  ...
  gtk_builder_add_from_resource(stickynotes->builder,
                                GRESOURCE "sticky-notes-preferences.ui", NULL);

  stickynotes->w_prefs = GTK_WIDGET(
      gtk_builder_get_object(stickynotes->builder, "preferences_dialog"));

  stickynotes->w_prefs_width = gtk_spin_button_get_adjustment(GTK_SPIN_BUTTON(
      gtk_builder_get_object(stickynotes->builder, "width_spin")));
  stickynotes->w_prefs_height = gtk_spin_button_get_adjustment(GTK_SPIN_BUTTON(
      gtk_builder_get_object(stickynotes->builder, "height_spin")));
  stickynotes->w_prefs_color =
      GTK_WIDGET(gtk_builder_get_object(stickynotes->builder, "default_color"));
  stickynotes->w_prefs_font_color = GTK_WIDGET(
      gtk_builder_get_object(stickynotes->builder, "prefs_font_color"));
  ...
  stickynotes->w_prefs_font =
      GTK_WIDGET(gtk_builder_get_object(stickynotes->builder, "default_font"));
  ...
  stickynotes->w_prefs_sticky =
      GTK_WIDGET(&GTK_CHECK_BUTTON(gtk_builder_get_object(stickynotes->builder,
                                                          "sticky_check"))
                      ->toggle_button);
  stickynotes->w_prefs_force = GTK_WIDGET(
      &GTK_CHECK_BUTTON(
...

  {
    ...
    gtk_size_group_add_widget(group, GTK_WIDGET(gtk_builder_get_object(
                                         stickynotes->builder, "width_label")));
    gtk_size_group_add_widget(
        group, GTK_WIDGET(gtk_builder_get_object(stickynotes->builder,
                                                 "height_label")));
    gtk_size_group_add_widget(
        group, GTK_WIDGET(gtk_builder_get_object(stickynotes->builder,
                                                 "prefs_color_label")));
  ...
  if (!g_settings_is_writable(stickynotes->settings, "default-width")) {
    gtk_widget_set_sensitive(
        GTK_WIDGET(gtk_builder_get_object(stickynotes->builder, "width_label")),
        FALSE);
    gtk_widget_set_sensitive(
        GTK_WIDGET(gtk_builder_get_object(stickynotes->builder, "width_spin")),
        FALSE);
  }
  if (!g_settings_is_writable(stickynotes->settings, "default-height")) {
    gtk_widget_set_sensitive(GTK_WIDGET(gtk_builder_get_object(
                             ...
                             FALSE);
    gtk_widget_set_sensitive(
        GTK_WIDGET(gtk_builder_get_object(stickynotes->builder, "height_spin")),
        FALSE);
  }
...
StickyNotesApplet *stickynotes_applet_new(MatePanelApplet *mate_panel_applet) {
  ...
  gtk_action_group_add_actions(
      applet->action_group, stickynotes_applet_menu_actions,
      G_N_ELEMENTS(stickynotes_applet_menu_actions), applet);
  gtk_action_group_add_toggle_actions(
      applet->action_group, stickynotes_applet_menu_toggle_actions,
      G_N_ELEMENTS(stickynotes_applet_menu_toggle_actions), applet);
  mate_panel_applet_setup_menu_from_resource(
      mate_panel_applet, GRESOURCE "menu.xml", applet->action_group);
  ...
  g_signal_connect(applet->w_applet, "destroy", G_CALLBACK(applet_destroy_cb),
                   applet);
  g_signal_connect(applet->w_applet, "destroy", G_CALLBACK(stickynotes_destroy),
                   NULL);
...
void stickynotes_applet_update_icon(StickyNotesApplet *applet) {
  ...
  if (applet->prelighted)
    surface = cairo_surface_create_similar(
        ...
        cairo_surface_get_content(stickynotes->icon_prelight), size, size);
  else
    surface = cairo_surface_create_similar(
        stickynotes->icon_normal,
        cairo_surface_get_content(stickynotes->icon_normal), size, size);

  cr = cairo_create(surface);
  cairo_set_source_surface(cr,
                           applet->prelighted ? stickynotes->icon_prelight
                                              : stickynotes->icon_normal,
                           0, 0);
  cairo_paint(cr);
...
void stickynotes_applet_update_prefs(void) {
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(stickynotes->w_prefs_sys_color), sys_color);
...
void stickynotes_applet_update_menus(void) {
  ...
  gboolean locked_writable =
      g_settings_is_writable(stickynotes->settings, "locked");
  ...
  for (l = stickynotes->applets; l != NULL; l = l->next) {
    ...
    GtkAction *action =
        gtk_action_group_get_action(applet->action_group, "lock");

    g_object_set(action, "active", locked, "sensitive", locked_writable, NULL);
...
void stickynotes_applet_update_tooltips(void) {
  ...
  for (l = stickynotes->applets; l; l = l->next) {
    ...
    if (applet->menu_tip)
      gtk_label_set_text(
          GTK_LABEL(gtk_bin_get_child(GTK_BIN(applet->menu_tip))), no_notes);
  }
...
void stickynotes_applet_panel_icon_get_geometry(int *x, int *y, int *width,
                                                int *height) {
  GtkWidget *widget;
