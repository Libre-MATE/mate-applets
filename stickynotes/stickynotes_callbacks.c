...
gboolean stickynote_resize_cb(GtkWidget *widget, GdkEventButton *event,
                              StickyNote *note) {
  if (event->type == GDK_BUTTON_PRESS && event->button == 1) {
    if (widget == note->w_resize_se)
      gtk_window_begin_resize_drag(
          GTK_WINDOW(note->w_window), GDK_WINDOW_EDGE_SOUTH_EAST, event->button,
          (gint)event->x_root, (gint)event->y_root, event->time);
    else /* if (widget == note->w_resize_sw) */
      gtk_window_begin_resize_drag(
          GTK_WINDOW(note->w_window), GDK_WINDOW_EDGE_SOUTH_WEST, event->button,
          (gint)event->x_root, (gint)event->y_root, event->time);
  } else
    return FALSE;
...
gboolean stickynote_move_cb(GtkWidget *widget, GdkEventButton *event,
                            StickyNote *note) {
  if (event->type == GDK_BUTTON_PRESS && event->button == 1)
...
gboolean stickynote_configure_cb(GtkWidget *widget, GdkEventConfigure *event,
                                 StickyNote *note) {
  note->x = event->x;
  note->y = event->y;
  note->w = event->width;
...
gboolean stickynote_delete_cb(GtkWidget *widget, GdkEvent *event,
                              StickyNote *note) {
  stickynotes_remove(note);
...
gboolean stickynote_show_popup_menu(GtkWidget *widget, GdkEventButton *event,
...
gboolean stickynote_keypress_cb(GtkWidget *widget, GdkEventKey *event,
                                GtkMenu *popup_menu) {
  ...
  if ((event->keyval == GDK_KEY_F10 &&
    ...
    if (!focus_widget) focus_widget = widget;

    gtk_menu_popup_at_widget(popup_menu, focus_widget, GDK_GRAVITY_SOUTH_WEST,
...
void popup_toggle_lock_cb(GtkCheckMenuItem *item, StickyNote *note) {
  stickynote_set_locked(note, gtk_check_menu_item_get_active(item));
}
