...
static void set_icon_geometry(GdkWindow *window, int x, int y, int width,
                              int height) {
  gulong data[4];
  Display *dpy;
  ...
  XChangeProperty(dpy, GDK_WINDOW_XID(window),
                  gdk_x11_get_xatom_by_name_for_display(
                      gdk_window_get_display(window), "_NET_WM_ICON_GEOMETRY"),
                  XA_CARDINAL, 32, PropModeReplace, (guchar *)&data, 4);
}

/* Called when a timeout occurs.  */
static gboolean timeout_happened(gpointer data) {
...
static void buffer_changed(GtkTextBuffer *buffer, StickyNote *note) {
...
static StickyNote *stickynote_new_aux(GdkScreen *screen, gint x, gint y, gint w,
                                      gint h) {
  StickyNote *note;
  GtkBuilder *builder;
  ...
  gtk_builder_add_from_resource(builder, GRESOURCE "sticky-notes-note.ui",
                                NULL);
  gtk_builder_add_from_resource(builder, GRESOURCE "sticky-notes-properties.ui",
                                NULL);

  note->w_window =
      GTK_WIDGET(gtk_builder_get_object(builder, "stickynote_window"));
  ...
  note->w_scroller =
      GTK_WIDGET(gtk_builder_get_object(builder, "body_scroller"));
  ...
  note->img_lock = GTK_IMAGE(gtk_builder_get_object(builder, "lock_img"));
  note->img_close = GTK_IMAGE(gtk_builder_get_object(builder, "close_img"));
  note->img_resize_se =
  ...
  gtk_widget_set_direction(
      GTK_WIDGET(gtk_builder_get_object(builder, "resize_bar")),
      GTK_TEXT_DIR_LTR);

  note->w_menu = GTK_WIDGET(gtk_builder_get_object(builder, "stickynote_menu"));
  note->w_lock_toggle_item =
      GTK_CHECK_MENU_ITEM(gtk_builder_get_object(builder, "popup_toggle_lock"));

  note->w_properties =
      GTK_WIDGET(gtk_builder_get_object(builder, "stickynote_properties"));
  ...
  note->w_font_color_label =
      GTK_WIDGET(gtk_builder_get_object(builder, "font_color_label"));
  note->w_font = GTK_WIDGET(gtk_builder_get_object(builder, "note_font"));
  note->w_font_label =
      GTK_WIDGET(gtk_builder_get_object(builder, "font_label"));
  note->w_def_color = GTK_WIDGET(
      &GTK_CHECK_BUTTON(gtk_builder_get_object(builder, "def_color_check"))
           ->toggle_button);
  note->w_def_font = GTK_WIDGET(
      &GTK_CHECK_BUTTON(gtk_builder_get_object(builder, "def_font_check"))
           ->toggle_button);
  ...
  if (w == 0 || h == 0)
    ...
    gtk_window_resize(GTK_WINDOW(note->w_window), note->w, note->h);

  if (x != -1 && y != -1)
    gtk_window_move(GTK_WINDOW(note->w_window), note->x, note->y);

  /* Set the button images */
  gtk_image_set_from_icon_name(note->img_close, STICKYNOTES_STOCK_CLOSE,
                               GTK_ICON_SIZE_MENU);
  gtk_image_set_pixel_size(note->img_close, STICKYNOTES_ICON_SIZE);

  gtk_image_set_from_icon_name(note->img_resize_se, STICKYNOTES_STOCK_RESIZE_SE,
                               GTK_ICON_SIZE_MENU);
  gtk_image_set_pixel_size(note->img_resize_se, STICKYNOTES_ICON_SIZE);

  gtk_image_set_from_icon_name(note->img_resize_sw, STICKYNOTES_STOCK_RESIZE_SW,
                               GTK_ICON_SIZE_MENU);
  ...
  gtk_dialog_set_default_response(GTK_DIALOG(note->w_properties),
                                  GTK_RESPONSE_CLOSE);
  g_signal_connect(note->w_properties, "response", G_CALLBACK(response_cb),
                   note);
  ...
  g_signal_connect(note->w_close, "clicked", G_CALLBACK(stickynote_close_cb),
                   note);
  ...
  g_signal_connect(note->w_color, "color-set", G_CALLBACK(properties_color_cb),
                   note);
  ...
  g_signal_connect(note->w_font, "font-set", G_CALLBACK(properties_font_cb),
                   note);
  ...
  g_signal_connect_after(note->w_window, "key-press-event",
                         G_CALLBACK(stickynote_keypress_cb), note->w_menu);
...
void stickynote_change_properties(StickyNote *note) {
  ...
  if (note->font_color)
    ...
    color_str =
        g_settings_get_string(stickynotes->settings, "default-font-color");
  }
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(note->w_def_font),
                               note->font == NULL);
  if (note->font)
    gtk_font_button_set_font_name(GTK_FONT_BUTTON(note->w_font), note->font);
...
gboolean stickynote_get_empty(const StickyNote *note) {
  return gtk_text_buffer_get_char_count(
             gtk_text_view_get_buffer(GTK_TEXT_VIEW(note->w_body))) == 0;
}
...
void stickynote_set_title(StickyNote *note, const gchar *title) {
  ...
  if (!title) {
    ...
    if (!date_format) date_format = g_strdup("%x");
...
void stickynote_set_color(StickyNote *note, const gchar *color_str,
                          const gchar *font_color_str, gboolean save) {
  char *color_str_actual, *font_color_str_actual;
  gboolean force_default, use_system_color;

  if (save) {
    if (note->color) g_free(note->color);
    if (note->font_color) g_free(note->font_color);

    note->color = color_str ? g_strdup(color_str) : NULL;
    note->font_color = font_color_str ? g_strdup(font_color_str) : NULL;

    gtk_widget_set_sensitive(note->w_color_label, note->color != NULL);
    gtk_widget_set_sensitive(note->w_font_color_label,
                             note->font_color != NULL);
    gtk_widget_set_sensitive(note->w_color, note->color != NULL);
    gtk_widget_set_sensitive(note->w_font_color, note->color != NULL);
  }

  force_default =
      g_settings_get_boolean(stickynotes->settings, "force-default");
  use_system_color =
      g_settings_get_boolean(stickynotes->settings, "use-system-color");
  ...
  if (!color_str || force_default) {
    if (use_system_color)
      ...
      color_str_actual =
          g_settings_get_string(stickynotes->settings, "default-color");
  } else
  ...
  if (color_str_actual) {
    ...
    gtk_widget_override_background_color(note->w_window, GTK_STATE_FLAG_NORMAL,
                                         &colors[0]);
    gtk_widget_override_background_color(note->w_body, GTK_STATE_FLAG_NORMAL,
                                         &colors[0]);
    gtk_widget_override_background_color(note->w_lock, GTK_STATE_FLAG_NORMAL,
                                         &colors[0]);
    gtk_widget_override_background_color(note->w_close, GTK_STATE_FLAG_NORMAL,
                                         &colors[0]);
    gtk_widget_override_background_color(note->w_resize_se,
                                         GTK_STATE_FLAG_NORMAL, &colors[0]);
  ...
  if (color_str_actual) g_free(color_str_actual);
  if (font_color_str_actual) g_free(font_color_str_actual);
}

/* Set the sticky note font */
void stickynote_set_font(StickyNote *note, const gchar *font_str,
                         gboolean save) {
...
void stickynote_set_locked(StickyNote *note, gboolean locked) {
  ...
  if (locked) {
    gtk_image_set_from_icon_name(note->img_lock, STICKYNOTES_STOCK_LOCKED,
                                 GTK_ICON_SIZE_MENU);
    gtk_widget_set_tooltip_text(note->w_lock, _("This note is locked."));
  } else {
    gtk_image_set_from_icon_name(note->img_lock, STICKYNOTES_STOCK_UNLOCKED,
                                 GTK_ICON_SIZE_MENU);
  ...
  gtk_check_menu_item_set_active(note->w_lock_toggle_item, locked);
...
void stickynote_set_visible(StickyNote *note, gboolean visible) {
  if (visible) {
    ...
    if (g_settings_get_boolean(stickynotes->settings, "sticky"))
      ...
      xstuff_change_workspace(GTK_WINDOW(note->w_window), note->workspace - 1);
  } else {
    /* Hide sticky note */
    int x, y, width, height;

    stickynotes_applet_panel_icon_get_geometry(&x, &y, &width, &height);
    set_icon_geometry(gtk_widget_get_window(GTK_WIDGET(note->w_window)), x, y,
                      width, height);
...
void stickynotes_remove(StickyNote *note) {
  ...
  builder = gtk_builder_new();
  gtk_builder_add_from_resource(builder, GRESOURCE "sticky-notes-delete.ui",
                                NULL);
...
gboolean stickynotes_save_now(void) {
  ...
  for (i = 0; i < g_list_length(stickynotes->notes); i++) {
    ...
    if (!g_settings_get_boolean(stickynotes->settings, "sticky") && wnck_win)
      note->workspace =
          1 + wnck_workspace_get_number(wnck_window_get_workspace(wnck_win));
    else
      note->workspace = 0;
    ...
    {
      xmlNodePtr node =
          xmlNewTextChild(root, NULL, XML_CHAR("note"), XML_CHAR(body));
      ...
      if (note->font_color)
        xmlNewProp(node, XML_CHAR("font_color"), XML_CHAR(note->font_color));

      if (note->font) xmlNewProp(node, XML_CHAR("font"), XML_CHAR(note->font));

      if (note->locked) xmlNewProp(node, XML_CHAR("locked"), XML_CHAR("true"));
...
void stickynotes_load(GdkScreen *screen) {
  ...
  gchar *file = g_build_filename(g_get_user_config_dir(), "mate",
                                 "stickynotes-applet.xml", NULL);
  ...
  if (g_file_test(file, G_FILE_TEST_EXISTS)) {
    ...
    file = g_build_filename(g_get_home_dir(), ".mate2", "stickynotes_applet",
                            NULL);
  ...
  while (node) {
    if (!xmlStrcmp(node->name, (const xmlChar *)"note")) {
      ...
      /* Retrieve and set title of the note */
      {
        ...
        if (title) stickynote_set_title(note, title);
      ...
      /* Retrieve and set the font of the note */
      {
        ...
        if (font_str) stickynote_set_font(note, font_str, TRUE);
       ...
       * by default unlocked */
      {
        ...
        if (locked) stickynote_set_locked(note, !strcmp(locked, "true"));
        g_free(locked);
      }
    }
