...
static gboolean get_desktop_window(Window *window) {
  ...
  if (gdk_property_get(
          root_window, gdk_atom_intern("CAJA_DESKTOP_WINDOW_ID", FALSE),
          gdk_x11_xatom_to_atom(XA_WINDOW), 0, 4, FALSE, &type_returned,
          &format_returned, &length_returned, (guchar **)&desktop_window)) {
    *window = *desktop_window;
...
static void popup_add_note(StickyNotesApplet *applet, GtkWidget *item) {
  stickynotes_add(gtk_widget_get_screen(applet->w_applet));
}

static void stickynote_show_notes(gboolean visible) {
  StickyNote *note;
  GList *l;
...
static void stickynote_toggle_notes_visible(void) {
  stickynote_show_notes(!stickynotes->visible);
}

/* Applet Callback : Mouse button press on the applet. */
gboolean applet_button_cb(GtkWidget *widget, GdkEventButton *event,
...
gboolean applet_key_cb(GtkWidget *widget, GdkEventKey *event,
                       StickyNotesApplet *applet) {
  switch (event->keyval) {
...
gboolean applet_cross_cb(GtkWidget *widget, GdkEventCrossing *event,
                         StickyNotesApplet *applet) {
  applet->prelighted =
...
gboolean applet_focus_cb(GtkWidget *widget, GdkEventFocus *event,
                         StickyNotesApplet *applet) {
  stickynotes_applet_update_icon(applet);
...
static GdkFilterReturn desktop_window_event_filter(GdkXEvent *xevent,
  ...
  gboolean desktop_hide =
      g_settings_get_boolean(stickynotes->settings, "desktop-hide");
  if (desktop_hide && (((XEvent *)xevent)->xany.type == PropertyNotify) &&
      (((XEvent *)xevent)->xproperty.atom ==
    ...
    if (actual_type == None) {
      ...
      if (actual_type != None) {
        ...
        window = gdk_x11_window_foreign_new_for_display(
            gdk_window_get_display(window), desktop_window);
      }
    }
  }
...
void applet_change_orient_cb(MatePanelApplet *mate_panel_applet,
...
void applet_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation,
                             StickyNotesApplet *applet) {
  if ((applet->panel_orient == MATE_PANEL_APPLET_ORIENT_UP) ||
      (applet->panel_orient == MATE_PANEL_APPLET_ORIENT_DOWN)) {
    if (applet->panel_size == allocation->height) return;
    applet->panel_size = allocation->height;
  } else {
    if (applet->panel_size == allocation->width) return;
...
void applet_destroy_cb(MatePanelApplet *mate_panel_applet,
                       StickyNotesApplet *applet) {
  GList *notes;
  ...
  if (applet->action_group) g_object_unref(applet->action_group);
...
static void destroy_all_response_cb(GtkDialog *dialog, gint id,
                                    StickyNotesApplet *applet) {
...
void menu_destroy_all_cb(GtkAction *action, StickyNotesApplet *applet) {
  ...
  gtk_builder_add_from_resource(builder, GRESOURCE "sticky-notes-delete-all.ui",
                                NULL);
  ...
  applet->destroy_all_dialog =
      GTK_WIDGET(gtk_builder_get_object(builder, "delete_all_dialog"));

  g_object_unref(builder);

  g_signal_connect(applet->destroy_all_dialog, "response",
                   G_CALLBACK(destroy_all_response_cb), applet);
...
void menu_help_cb(GtkAction *action, StickyNotesApplet *applet) {
  ...
  gtk_show_uri_on_window(NULL, "help:mate-stickynotes-applet",
...
void menu_about_cb(GtkAction *action, StickyNotesApplet *applet) {
  static const gchar *authors[] = {"Loban A Rahman <loban@earthling.net>",
  ...
  gtk_show_about_dialog(NULL, "title", _("About Sticky Notes"), "version",
                        VERSION, "copyright",
                        ...
                        "logo-icon-name", "mate-sticky-notes-applet", NULL);
}
...
void preferences_save_cb(gpointer data) {
  ...
  sys_color = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(stickynotes->w_prefs_sys_color));
  sys_font = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(stickynotes->w_prefs_sys_font));
  sticky = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(stickynotes->w_prefs_sticky));
  force_default = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(stickynotes->w_prefs_force));
  desktop_hide = gtk_toggle_button_get_active(
  ...
  if (g_settings_is_writable(stickynotes->settings, "use-system-color"))
    g_settings_set_boolean(stickynotes->settings, "use-system-color",
                           sys_color);
  ...
  if (g_settings_is_writable(stickynotes->settings, "force-default"))
    g_settings_set_boolean(stickynotes->settings, "force-default",
                           force_default);
...
void preferences_color_cb(GtkWidget *button, gpointer data) {
  ...
  g_settings_set_string(stickynotes->settings, "default-font-color",
                        font_color_str);
...
void preferences_apply_cb(GSettings *settings, gchar *key, gpointer data) {
  ...
  else if (!strcmp(key, "use-system-color") || !strcmp(key, "default-color")) {
    for (l = stickynotes->notes; l; l = l->next) {
      note = l->data;
      stickynote_set_color(note, note->color, note->font_color, FALSE);
    }
  }

  else if (!strcmp(key, "use-system-font") || !strcmp(key, "default-font")) {
    for (l = stickynotes->notes; l; l = l->next) {
      note = l->data;
...
gboolean preferences_delete_cb(GtkWidget *widget, GdkEvent *event,
                               gpointer data) {
