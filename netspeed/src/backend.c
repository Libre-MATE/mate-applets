...
gboolean is_dummy_device(const char *device) {
  glibtop_netload netload;
  glibtop_get_netload(&netload, device);

  if (netload.if_flags & (1 << GLIBTOP_IF_FLAGS_LOOPBACK)) return TRUE;
 ...
 */
GList *get_available_devices(void) {
  glibtop_netlist buf;
  char **devices, **dev;
...
GSList *get_ip_address_list(const char *iface_name, gboolean ipv4) {
  GSList *list = NULL;
  ...
  if (getifaddrs(&ifaces) != -1) {
    ...
    for (struct ifaddrs *iface = ifaces; iface != NULL;
      ...
      if ((iface->ifa_addr->sa_family == family) &&
        ...
        if (iface->ifa_addr->sa_family == AF_INET6) {
          ...
          list = g_slist_prepend(
              list, g_strdup_printf("%s/%u (%s)", ip, netmask, scope));
        } else {
          ...
          list = g_slist_prepend(list, g_strdup_printf("%s/%u", ip, netmask));
        }
      }
    }
    if (list != NULL) list = g_slist_sort(list, (GCompareFunc)g_strcmp0);
    freeifaddrs(ifaces);
  }
  return list;
}

GSList *get_ip6_address_list(const char *iface_name) {
  GSList *list = NULL;
  struct ifaddrs *ifaces;
...
const gchar *get_default_route(void) {
  FILE *fp;
  ...
  while (!feof(fp)) {
    ...
    retval =
        sscanf(buffer, "%49s %x %x %x %u %u %u %x %u %u %u", device, &ip, &gw,
               &flags, &ref, &use, &metric, &mask, &mtu, &window, &irtt);
 ...
 */
void free_device_info(DevInfo *devinfo) {
  g_free(devinfo->name);
...
*/
static void get_ptp_info(DevInfo *devinfo) {
  int fd = -1;
  struct ifreq request = {};

  g_strlcpy(request.ifr_name, devinfo->name, sizeof request.ifr_name);

  if ((fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) return;
...
void get_device_info(const char *device, DevInfo **info) {
  DevInfo *devinfo;
  glibtop_netload netload;
  gboolean ptp = FALSE;
  ...
  *info = g_new0(DevInfo, 1);
  ...
  devinfo->running =
      (netload.if_flags & (1L << GLIBTOP_IF_FLAGS_RUNNING) ? TRUE : FALSE);
...
gboolean compare_device_info(const DevInfo *a, const DevInfo *b) {
  g_assert(a && b);
...
#ifdef HAVE_IW
void get_wireless_info(DevInfo *devinfo) {
  int fd;
  int newqual;
  ...
  if (iw_get_basic_config(fd, devinfo->name, &info.b) < 0) goto out;
  ...
  if (iw_get_stats(fd, devinfo->name, &info.stats, &info.range,
                   info.has_range) >= 0)
    info.has_stats = 1;

  if (info.has_stats) {
    if ((iw_get_range_info(fd, devinfo->name, &info.range) >= 0) &&
        (info.range.max_qual.qual > 0)) {
      newqual = 0.5f + (100.0f * info.stats.qual.qual) /
                           (1.0f * info.range.max_qual.qual);
    } else {
      newqual = info.stats.qual.qual;
    }

    newqual = CLAMP(newqual, 0, 100);
    if (devinfo->qual != newqual) devinfo->qual = newqual;
...
static int nl80211_init(struct nl80211_state *state) {
  ...
  err = 1;
  setsockopt(nl_socket_get_fd(state->sock), SOL_NETLINK, NETLINK_EXT_ACK, &err,
             sizeof(err));
...
static void parse_bitrate(struct nlattr *bitrate_attr, char *buf, int buflen) {
  ...
  if (rate > 0)
    pos += snprintf(pos, buflen - (pos - buf), _("%d.%d MBit/s"), rate / 10,
                    rate % 10);
  else
    pos += snprintf(pos, buflen - (pos - buf), _("(unknown)"));

  if (rinfo[NL80211_RATE_INFO_MCS])
    pos += snprintf(pos, buflen - (pos - buf), _(" MCS %d"),
                    nla_get_u8(rinfo[NL80211_RATE_INFO_MCS]));
  if (rinfo[NL80211_RATE_INFO_VHT_MCS])
    pos += snprintf(pos, buflen - (pos - buf), _(" VHT-MCS %d"),
...
static int station_cb(struct nl_msg *msg, void *arg) {
  ...
  if (sinfo[NL80211_STA_INFO_RX_BYTES] && sinfo[NL80211_STA_INFO_RX_PACKETS]) {
    ...
    g_debug("RX: %" G_GUINT32_FORMAT " bytes (%" G_GUINT32_FORMAT " packets)",
            rx_bytes, nla_get_u32(sinfo[NL80211_STA_INFO_RX_PACKETS]));
  }
  if (sinfo[NL80211_STA_INFO_TX_BYTES] && sinfo[NL80211_STA_INFO_TX_PACKETS]) {
    ...
    g_debug("TX: %" G_GUINT32_FORMAT " bytes (%" G_GUINT32_FORMAT " packets)",
            tx_bytes, nla_get_u32(sinfo[NL80211_STA_INFO_TX_PACKETS]));
  }
  ...
  if (sinfo[NL80211_STA_INFO_CONNECTED_TIME]) {
    devinfo->connected_time =
        nla_get_u32(sinfo[NL80211_STA_INFO_CONNECTED_TIME]);
    g_debug("connected time: %" G_GUINT32_FORMAT " seconds",
            devinfo->connected_time);
  }
...
static int iface_cb(struct nl_msg *msg, void *arg) {
  DevInfo *devinfo = (DevInfo *)arg;
  struct genlmsghdr *gnlh = nlmsg_data(nlmsg_hdr(msg));
  ...
  if (tb_msg[NL80211_ATTR_WIPHY_FREQ]) {
    ...
    pos += sprintf(pos, _("%d (%d MHz)"), ieee80211_frequency_to_channel(freq),
                   freq);
...
void get_wireless_info(DevInfo *devinfo) {
  ...
  /* Add the callback */
  nl_socket_modify_cb(nlstate.sock, NL_CB_VALID, NL_CB_CUSTOM, iface_cb,
                      devinfo);
  /* Send the message */
  ...
  if (!devinfo->running) goto cleanup;
  ...
  nla_put_u32(msg, NL80211_ATTR_IFINDEX, devidx);
  /* Add the callback */
  nl_socket_modify_cb(nlstate.sock, NL_CB_VALID, NL_CB_CUSTOM, scan_cb,
                      devinfo);
  /* Send the message */
  ret = nl_send_auto(nlstate.sock, msg);
  ...
  nla_put_u32(msg, NL80211_ATTR_IFINDEX, devidx);
  /* Add the callback */
  nl_socket_modify_cb(nlstate.sock, NL_CB_VALID, NL_CB_CUSTOM, station_cb,
                      devinfo);
  /* Send the message */
  ret = nl_send_auto(nlstate.sock, msg);
