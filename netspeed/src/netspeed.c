...
#define GET_COLOR_CHOOSER(x) \
  (GTK_COLOR_CHOOSER(gtk_builder_get_object(builder, (x))))
#define GET_DRAWING_AREA(x) \
  (GTK_DRAWING_AREA(gtk_builder_get_object(builder, (x))))
...
static const char *wireless_quality_icon[] = {
    "mate-netspeed-wireless-25", "mate-netspeed-wireless-50",
    "mate-netspeed-wireless-75", "mate-netspeed-wireless-100"};
...
static void update_tooltip(NetspeedApplet *netspeed);
 ...
 */
static void add_markup_fgcolor(char **string, const char *color) {
  char *tmp = *string;
 ...
 */
static void change_icons(NetspeedApplet *netspeed) {
  ...
  /* FIXME: Not all network icons include a high enough resolution, so to make
   * them all consistent, we cap them at 48px.*/
  ...
  if (netspeed->show_icon && netspeed->change_icon && netspeed->devinfo) {
    ...
    dev = gtk_icon_theme_load_surface(netspeed->icon_theme,
                                      dev_type_icon[DEV_UNKNOWN], icon_size,
  ...
  in_arrow = gtk_icon_theme_load_surface(netspeed->icon_theme, IN_ICON, 16,
                                         icon_scale, NULL, 0, NULL);

  out_arrow = gtk_icon_theme_load_surface(netspeed->icon_theme, OUT_ICON, 16,
                                          icon_scale, NULL, 0, NULL);
  ...
  if (netspeed->devinfo && netspeed->devinfo->running) {
    ...
    copy = cairo_surface_create_similar(
        dev, cairo_surface_get_content(dev),
    ...
    down = gtk_icon_theme_load_surface(netspeed->icon_theme, ERROR_ICON,
                                       icon_size, icon_scale, NULL, 0, NULL);
 ...
 * or just the sum
 */
static void applet_change_size_or_orient(MatePanelApplet *applet, int arg1,
  ...
  gtk_box_pack_start(GTK_BOX(netspeed->in_box), netspeed->in_pix, FALSE, FALSE,
                     0);
  gtk_box_pack_start(GTK_BOX(netspeed->in_box), netspeed->in_label, TRUE, TRUE,
                     0);
  gtk_box_pack_start(GTK_BOX(netspeed->out_box), netspeed->out_pix, FALSE,
                     FALSE, 0);
  gtk_box_pack_start(GTK_BOX(netspeed->out_box), netspeed->out_label, TRUE,
                     TRUE, 0);
  gtk_box_pack_start(GTK_BOX(netspeed->sum_box), netspeed->sum_label, TRUE,
                     TRUE, 0);
  gtk_box_pack_start(GTK_BOX(netspeed->box), netspeed->pix_box, FALSE, FALSE,
                     0);
  ...
  if (netspeed->show_sum) {
    gtk_box_pack_start(GTK_BOX(netspeed->speed_box), netspeed->sum_box, TRUE,
                       TRUE, 0);
  } else {
    gtk_box_pack_start(GTK_BOX(netspeed->speed_box), netspeed->in_box, TRUE,
                       TRUE, 0);
    gtk_box_pack_start(GTK_BOX(netspeed->speed_box), netspeed->out_box, TRUE,
                       TRUE, 0);
  }
  gtk_box_pack_start(GTK_BOX(netspeed->box), netspeed->speed_box, TRUE, TRUE,
                     0);
 ...
 */
static void change_quality_icon(NetspeedApplet *netspeed) {
  if (netspeed->devinfo->type == DEV_WIRELESS && netspeed->devinfo->up &&
      netspeed->show_quality_icon) {
...
static void update_quality_icon(NetspeedApplet *netspeed) {
  ...
  q /= 25;
  q = MIN(q,
          3); /* q out of range would crash when accessing qual_surfaces[q] */
  gtk_image_set_from_surface(GTK_IMAGE(netspeed->qual_pix),
                             netspeed->qual_surfaces[q]);
}

static void init_quality_surfaces(NetspeedApplet *netspeed) {
  int i;
  ...
  for (i = 0; i < 4; i++) {
    ...
    surface = gtk_icon_theme_load_surface(netspeed->icon_theme,
                                          wireless_quality_icon[i], icon_size,
                                          icon_scale, NULL, 0, NULL);

    if (surface) {
      cairo_t *cr;

      netspeed->qual_surfaces[i] = cairo_surface_create_similar(
...
static void icon_theme_changed_cb(GtkIconTheme *icon_theme,
                                  NetspeedApplet *netspeed) {
...
static void format_transfer_rate(gchar *out, double bytes, gboolean bits) {
  const char *format;
  const char *unit;
  ...
  if (bytes < IEC_KIBI_DBL) {
    format = "%.0f %s";
    unit = bits ? /* translators: bits (short) */ N_("bit/s")
                : /* translators: Bytes (short) */ N_("B/s");
  } else if (bytes < IEC_MEBI_DBL) {
    format = (bytes < (100.0 * IEC_KIBI_DBL)) ? "%.1f %s" : "%.0f %s";
    bytes /= IEC_KIBI_DBL;
    unit = bits ? /* translators: kibibits (short) */ N_("Kibit/s")
                : /* translators: Kibibytes (short) */ N_("KiB/s");
  } else if (bytes < IEC_GIBI_DBL) {
    format = "%.1f %s";
    bytes /= IEC_MEBI_DBL;
    unit = bits ? /* translators: Mebibit (short) */ N_("Mibit/s")
                : /* translators: Mebibyte (short) */ N_("MiB/s");
  } else {
    format = "%.1f %s";
    bytes /= IEC_GIBI_DBL;
    unit = bits ? /* translators: Gibibit (short) */ N_("Gibit/s")
                : /* translators: Gibibyte (short) */ N_("GiB/s");
  }
 ...
 */
static char *bps_to_string(double bytes, gboolean bits) {
 ...
 */
static void redraw_graph(NetspeedApplet *netspeed, cairo_t *cr) {
  ...
  offset = 0;
  for (i = (netspeed->index_graph + 1) % GRAPH_VALUES;
       netspeed->in_graph[i] < 0; i = (i + 1) % GRAPH_VALUES)
    offset++;
  for (i = offset + 1; i < GRAPH_VALUES; i++) {
    ...
    in_points[i].y =
        h - 6 - (int)((h - 8) * netspeed->in_graph[index] / max_val);
    out_points[i].y =
        h - 6 - (int)((h - 8) * netspeed->out_graph[index] / max_val);
  }
  in_points[offset].x = out_points[offset].x =
      ((w - 6) * offset) / GRAPH_VALUES + 4;
...
static gboolean set_applet_devinfo(NetspeedApplet *netspeed,
                                   const char *iface) {
  DevInfo *info;
...
static void search_for_up_if(NetspeedApplet *netspeed) {
  ...
  if (default_route != NULL) {
    if (set_applet_devinfo(netspeed, default_route)) return;
  }

  devices = get_available_devices();
  for (tmp = devices; tmp; tmp = g_list_next(tmp)) {
    if (is_dummy_device(tmp->data)) continue;
    if (set_applet_devinfo(netspeed, tmp->data)) break;
  }
  g_list_free_full(devices, g_free);
}

static char *format_time(guint32 sec) {
  char *res;
  char *m, *s;
  int seconds;
  int minutes;
  ...
  if (sec < 60)
    return g_strdup_printf(ngettext("%'" G_GUINT32_FORMAT " second",
                                    "%'" G_GUINT32_FORMAT " seconds", sec),
                           sec);
...
static char *mac_addr_n2a(const unsigned char *hw) {
  if (hw[6] || hw[7]) {
    return g_strdup_printf("%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x", hw[0],
                           hw[1], hw[2], hw[3], hw[4], hw[5], hw[6], hw[7]);
  } else {
    return g_strdup_printf("%02x:%02x:%02x:%02x:%02x:%02x", hw[0], hw[1], hw[2],
                           hw[3], hw[4], hw[5]);
  }
}

static void format_ipv4(guint32 ip, char *dest) {
  inet_ntop(AF_INET, &ip, dest, INET_ADDRSTRLEN);
}

static void fill_details_dialog(NetspeedApplet *netspeed) {
  char *text;
  ...
  GSList *ipv6_address_list =
      get_ip_address_list(netspeed->devinfo->name, FALSE);
  ...
  if (netspeed->devinfo->type == DEV_WIRELESS) {
    ...
    /* _maybe_ we can add the encrypted icon between the essid and the signal
     * bar. */
    ...
    gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(netspeed->signalbar),
                                  quality);
    ...
    gtk_label_set_text(
        GTK_LABEL(netspeed->channel_text),
...
static void update_applet(NetspeedApplet *netspeed) {
  ...
  if (netspeed->devinfo->running) {
    ...
    format_transfer_rate(netspeed->devinfo->rx_rate, inrate,
                         netspeed->show_bits);
    format_transfer_rate(netspeed->devinfo->tx_rate, outrate,
                         netspeed->show_bits);
    format_transfer_rate(netspeed->devinfo->sum_rate, inrate + outrate,
                         netspeed->show_bits);
  } else {
  ...
  if (netspeed->devinfo->type == DEV_WIRELESS) {
    if (netspeed->devinfo->up) update_quality_icon(netspeed);
    ...
    if (netspeed->connected_time_text) {
      ...
      text = format_time(netspeed->devinfo->connected_time);
      gtk_label_set_text(
          GTK_LABEL(netspeed->connected_time_text),
          netspeed->devinfo->connected_time > 0 ? text : _("na"));
      g_free(text);
    }
#endif
  ...
  if (netspeed->inbytes_text) {
    if (netspeed->show_bits)
      inbytes =
          g_format_size_full(netspeed->devinfo->rx * 8,
                             G_FORMAT_SIZE_IEC_UNITS | G_FORMAT_SIZE_BITS);
    else
      inbytes =
          g_format_size_full(netspeed->devinfo->rx, G_FORMAT_SIZE_IEC_UNITS);
  ...
  if (netspeed->outbytes_text) {
    if (netspeed->show_bits)
      outbytes =
          g_format_size_full(netspeed->devinfo->tx * 8,
                             G_FORMAT_SIZE_IEC_UNITS | G_FORMAT_SIZE_BITS);
    else
      outbytes =
          g_format_size_full(netspeed->devinfo->tx, G_FORMAT_SIZE_IEC_UNITS);
...
static gboolean timeout_function(NetspeedApplet *netspeed) {
  if (!netspeed) return FALSE;
  if (!netspeed->timeout_id) return FALSE;
...
void netspeed_applet_display_help(GtkWidget *dialog, const gchar *section) {
  ...
  ret = gtk_show_uri_on_window(NULL, uri, gtk_get_current_event_time(), &error);
  g_free(uri);
 ...
 */
static void help_cb(GtkAction *action, NetspeedApplet *netspeed) {
...
static void about_cb(GtkAction *action, gpointer data) {
  const char *authors[] = {"Jörgen Scheibengruber <mfcn@gmx.de>",
  ...
  gtk_show_about_dialog(
      ...
      "authors", authors, "documenters", NULL, "translator-credits",
      _("translator-credits"), "website", PACKAGE_URL, "logo-icon-name",
      LOGO_ICON, NULL);
}

static void netspeed_applet_destroy_preferences(GtkWidget *widget,
                                                gpointer user_data) {
  NetspeedApplet *netspeed;
  (void)widget;
 ...
 */
static void settings_cb(GtkAction *action, NetspeedApplet *netspeed) {
  g_assert(netspeed);
...
static gboolean da_draw(GtkWidget *widget, cairo_t *cr,
                        NetspeedApplet *netspeed) {
...
static void incolor_changed_cb(GtkColorChooser *button,
                               NetspeedApplet *netspeed) {
  GdkRGBA color;
  gchar *string;
...
static void outcolor_changed_cb(GtkColorChooser *button,
                                NetspeedApplet *netspeed) {
  GdkRGBA color;
  gchar *string;
 ...
 */
static void info_response_cb(GtkDialog *dialog, gint id,
                             NetspeedApplet *netspeed) {
 ...
 */
static void showinfo_cb(GtkAction *action, NetspeedApplet *netspeed) {
  GtkBuilder *builder;
  ...
  builder = gtk_builder_new_from_resource(NETSPEED_RESOURCE_PATH
                                          "netspeed-details.ui");
  ...
  gtk_color_chooser_set_rgba(GET_COLOR_CHOOSER("incolor_sel"),
                             &netspeed->in_color);
  gtk_color_chooser_set_rgba(GET_COLOR_CHOOSER("outcolor_sel"),
                             &netspeed->out_color);

  fill_details_dialog(netspeed);

  gtk_builder_add_callback_symbols(
      builder, "on_drawingarea_draw", G_CALLBACK(da_draw),
      "on_incolor_sel_color_set", G_CALLBACK(incolor_changed_cb),
      "on_outcolor_sel_color_set", G_CALLBACK(outcolor_changed_cb),
      "on_dialog_response", G_CALLBACK(info_response_cb), NULL);
...
static const GtkActionEntry netspeed_applet_menu_actions[] = {
    {"NetspeedAppletDetails", "dialog-information", N_("Device _Details"), NULL,
     NULL, G_CALLBACK(showinfo_cb)},
    {"NetspeedAppletProperties", "document-properties", N_("Preferences..."),
     NULL, NULL, G_CALLBACK(settings_cb)},
    {"NetspeedAppletHelp", "help-browser", N_("Help"), NULL, NULL,
     G_CALLBACK(help_cb)},
    {"NetspeedAppletAbout", "help-about", N_("About..."), NULL, NULL,
     G_CALLBACK(about_cb)}};
 ...
 */
static void label_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation,
                                   NetspeedApplet *netspeed) {
...
static gboolean netspeed_applet_button_press_event(GtkWidget *widget,
  ...
  if (event->button == 1) {
    ...
    if (netspeed->up_cmd && netspeed->down_cmd) {
      ...
      netspeed->connect_dialog = gtk_message_dialog_new(
  ...
  return GTK_WIDGET_CLASS(netspeed_applet_parent_class)
 ...
 */
static void netspeed_applet_finalize(GObject *object) {
  ...
  if (netspeed->icon_theme != NULL) {
    g_signal_handlers_disconnect_by_func(netspeed->icon_theme,
                                         icon_theme_changed_cb, netspeed);
...
static void update_tooltip(NetspeedApplet *netspeed) {
  ...
  if (!netspeed->show_tooltip) return;
  ...
  if (!netspeed->devinfo->running)
    ...
    if (netspeed->devinfo->type == DEV_WIRELESS)
      g_string_append_printf(
          ...
          netspeed->devinfo->rssi, netspeed->devinfo->rx_bitrate,
...
static gboolean netspeed_applet_enter_notify_event(GtkWidget *widget,
...
static gboolean netspeed_applet_leave_notify_event(GtkWidget *widget,
                                                   GdkEventCrossing *event) {
  NetspeedApplet *netspeed = NETSPEED_APPLET(widget);
...
static void netspeed_applet_class_init(NetspeedAppletClass *klass) {
...
static void netspeed_applet_init(NetspeedApplet *netspeed) {}

static void changeicon_settings_changed(GSettings *settings, const gchar *key,
                                        NetspeedApplet *netspeed) {
  netspeed->change_icon = g_settings_get_boolean(settings, key);
  change_icons(netspeed);
}

static void showalladdresses_settings_changed(GSettings *settings,
                                              const gchar *key,
                                              NetspeedApplet *netspeed) {
  netspeed->show_all_addresses = g_settings_get_boolean(settings, key);
}

static void showsum_settings_changed(GSettings *settings, const gchar *key,
                                     NetspeedApplet *netspeed) {
...
static void showbits_settings_changed(GSettings *settings, const gchar *key,
                                      NetspeedApplet *netspeed) {
  netspeed->show_bits = g_settings_get_boolean(settings, key);
}

static void showicon_settings_changed(GSettings *settings, const gchar *key,
                                      NetspeedApplet *netspeed) {
  netspeed->show_icon = g_settings_get_boolean(settings, key);
  change_icons(netspeed);
}

static void showqualityicon_settings_changed(GSettings *settings,
                                             const gchar *key,
                                             NetspeedApplet *netspeed) {
...
static void auto_change_device_settings_changed(GSettings *settings,
                                                const gchar *key,
                                                NetspeedApplet *netspeed) {
...
static void device_settings_changed(GSettings *settings, const gchar *key,
                                    NetspeedApplet *netspeed) {
  char *device;
 ...
 */
static gboolean netspeed_applet_factory(MatePanelApplet *applet,
  ...
  if (strcmp(iid, "NetspeedApplet")) return FALSE;
  ...
  netspeed->settings =
      mate_panel_applet_settings_new(applet, "org.mate.panel.applet.netspeed");

  /* Get stored settings from gsettings
   */
  netspeed->show_all_addresses =
      g_settings_get_boolean(netspeed->settings, "show-all-addresses");
  ...
  netspeed->show_quality_icon =
      g_settings_get_boolean(netspeed->settings, "show-quality-icon");
  netspeed->change_icon =
      g_settings_get_boolean(netspeed->settings, "change-icon");
  ...
  if (!netspeed->devinfo) get_device_info("lo", &netspeed->devinfo);
  ...
  netspeed->timeout_id =
      g_timeout_add(REFRESH_TIME, (GSourceFunc)timeout_function, netspeed);

  g_signal_connect(applet, "change-size",
                   G_CALLBACK(applet_change_size_or_orient), netspeed);
  ...
  g_signal_connect(netspeed->in_label, "size-allocate",
                   G_CALLBACK(label_size_allocate_cb), netspeed);
  ...
  g_signal_connect(netspeed->settings, "changed::auto-change-device",
                   G_CALLBACK(auto_change_device_settings_changed), netspeed);

  g_signal_connect(netspeed->settings, "changed::device",
                   G_CALLBACK(device_settings_changed), netspeed);

  g_signal_connect(netspeed->settings, "changed::show-all-addresses",
                   G_CALLBACK(showalladdresses_settings_changed), netspeed);

  g_signal_connect(netspeed->settings, "changed::show-sum",
                   G_CALLBACK(showsum_settings_changed), netspeed);

  g_signal_connect(netspeed->settings, "changed::show-bits",
                   G_CALLBACK(showbits_settings_changed), netspeed);

  g_signal_connect(netspeed->settings, "changed::change-icon",
                   G_CALLBACK(changeicon_settings_changed), netspeed);

  g_signal_connect(netspeed->settings, "changed::show-icon",
                   G_CALLBACK(showicon_settings_changed), netspeed);

  g_signal_connect(netspeed->settings, "changed::show-quality-icon",
                   G_CALLBACK(showqualityicon_settings_changed), netspeed);
  ...
  mate_panel_applet_setup_menu_from_resource(
      applet, NETSPEED_RESOURCE_PATH "netspeed-menu.xml", action_group);
  ...
  if (GTK_IS_ACCESSIBLE(atk_obj)) {
    ...
    atk_object_set_description(
        atk_obj, _("A little applet that displays some information on the "
...
MATE_PANEL_APPLET_OUT_PROCESS_FACTORY("NetspeedAppletFactory",
                                      NETSPEED_TYPE_APPLET, "NetspeedApplet",
                                      netspeed_applet_factory, NULL)
