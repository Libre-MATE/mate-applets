...
static void netspeed_preferences_init(NetspeedPreferences *preferences) {
  gtk_widget_init_template(GTK_WIDGET(preferences));
}

static void netspeed_preferences_set_property(GObject *object,
                                              guint property_id,
                                              const GValue *value,
                                              GParamSpec *pspec) {
...
static void netspeed_preferences_get_property(GObject *object,
...
static void netspeed_preferences_response(GtkDialog *dialog, gint response_id) {
...
static void on_network_device_combo_changed(GtkComboBox *combo,
                                            NetspeedPreferences *preferences) {
  gint active;
  ...
  old_auto_change_device = auto_change_device =
      g_settings_get_boolean(preferences->settings, "auto-change-device");
  if (0 == active) {
    if (auto_change_device) return;
    ...
    current_device_name =
        netspeed_applet_get_current_device_name(preferences->netspeed);
    auto_change_device = FALSE;
    selected_device_name =
        g_list_nth_data(preferences->devices, (guint)(active - 1));
    if (!g_strcmp0(selected_device_name, current_device_name)) return;
    g_settings_set_string(preferences->settings, "device",
                          selected_device_name);
  }
  if (old_auto_change_device != auto_change_device)
    g_settings_set_boolean(preferences->settings, "auto-change-device",
                           auto_change_device);
}

static void netspeed_preferences_finalize(GObject *object) {
...
static void netspeed_preferences_class_init(NetspeedPreferencesClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_NETSPEED_APPLET,
      g_param_spec_pointer("netspeed-applet", "Netspeed Applet",
                           "The Netspeed Applet",
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  gtk_widget_class_set_template_from_resource(
      widget_class, NETSPEED_RESOURCE_PATH "netspeed-preferences.ui");

  gtk_widget_class_bind_template_child(widget_class, NetspeedPreferences,
                                       network_device_combo);
  gtk_widget_class_bind_template_child(widget_class, NetspeedPreferences,
                                       show_all_addresses_checkbutton);
  gtk_widget_class_bind_template_child(widget_class, NetspeedPreferences,
                                       show_sum_checkbutton);
  gtk_widget_class_bind_template_child(widget_class, NetspeedPreferences,
                                       show_bits_checkbutton);
  gtk_widget_class_bind_template_child(widget_class, NetspeedPreferences,
                                       show_icon_checkbutton);
  gtk_widget_class_bind_template_child(widget_class, NetspeedPreferences,
                                       show_quality_icon_checkbutton);
  gtk_widget_class_bind_template_child(widget_class, NetspeedPreferences,
                                       change_icon_checkbutton);

  gtk_widget_class_bind_template_callback(widget_class,
                                          on_network_device_combo_changed);
}

static void fill_device_combo(NetspeedPreferences *preferences,
                              GSettings *settings) {
  GList *ptr;
  int i, active = -1;
  const gchar *current_device_name;
  ...
  gtk_combo_box_text_append_text(
      GTK_COMBO_BOX_TEXT(preferences->network_device_combo), _("Default"));
  ...
  for (i = 0; ptr; ptr = g_list_next(ptr)) {
    gtk_combo_box_text_append_text(
        GTK_COMBO_BOX_TEXT(preferences->network_device_combo), ptr->data);
    if (g_str_equal(ptr->data, current_device_name)) active = (i + 1);
    ++i;
  }
  if (active < 0 || auto_change_device) active = 0;
  gtk_combo_box_set_active(GTK_COMBO_BOX(preferences->network_device_combo),
                           active);
}
...
GtkWidget *netspeed_preferences_new(NetspeedApplet *netspeed) {
  ...
  preferences = g_object_new(NETSPEED_TYPE_PREFERENCES, "netspeed-applet",
                             netspeed, NULL);

  preferences->settings = settings =
      netspeed_applet_get_settings(preferences->netspeed);
  ...
  g_settings_bind(settings, "show-sum", preferences->show_sum_checkbutton,
                  "active", G_SETTINGS_BIND_DEFAULT);

  g_settings_bind(settings, "show-bits", preferences->show_bits_checkbutton,
                  "active", G_SETTINGS_BIND_DEFAULT);

  g_settings_bind(settings, "show-icon", preferences->show_icon_checkbutton,
                  "active", G_SETTINGS_BIND_DEFAULT);
  ...
  g_settings_bind(settings, "change-icon", preferences->change_icon_checkbutton,
                  "active", G_SETTINGS_BIND_DEFAULT);
