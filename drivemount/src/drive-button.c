...
enum { CMD_NONE, CMD_MOUNT_OR_PLAY, CMD_UNMOUNT, CMD_EJECT };

/* type registration boilerplate code */
G_DEFINE_TYPE(DriveButton, drive_button, GTK_TYPE_BUTTON)

static void drive_button_set_volume(DriveButton *self, GVolume *volume);
static void drive_button_set_mount(DriveButton *self, GMount *mount);
...
static void drive_button_theme_change(GtkIconTheme *icon_theme, gpointer data);

static void drive_button_class_init(DriveButtonClass *class) {
...
static void drive_button_init(DriveButton *self) {
  GtkWidget *image;
...
GtkWidget *drive_button_new(GVolume *volume) {
  ...
  if (volume != NULL) {
    ...
    g_signal_connect(gtk_icon_theme_get_default(), "changed",
                     G_CALLBACK(drive_button_theme_change), self);
  }
...
static void drive_button_dispose(GObject *object) {
  ...
  if (self->update_tag) g_source_remove(self->update_tag);
  self->update_tag = 0;
...
static gboolean drive_button_button_press(GtkWidget *widget,
  ...
  if (event->button == 1) {
    ...
    if (self->popup_menu) {
      gtk_menu_popup_at_widget(GTK_MENU(self->popup_menu), widget,
...
static gboolean drive_button_update(gpointer user_data) {
  DriveButton *self;
  GdkScreen *screen;
  ...
  gtk_widget_get_preferred_size(gtk_bin_get_child(GTK_BIN(self)), NULL,
                                &image_req);
  ...
  if (!self->volume && !self->mount) {
    ...
    icon_info = gtk_icon_theme_lookup_icon_for_scale(
        icon_theme, "media-floppy", MIN(width, height), scale,
    ...
    if (gtk_bin_get_child(GTK_BIN(self)) != NULL)
      gtk_image_set_from_surface(GTK_IMAGE(gtk_bin_get_child(GTK_BIN(self))),
                                 surface);
  ...
  icon_info = gtk_icon_theme_lookup_by_gicon_for_scale(
      icon_theme, icon, MIN(width, height), scale, GTK_ICON_LOOKUP_USE_BUILTIN);
  if (icon_info) {
    surface = gtk_icon_info_load_surface(icon_info, NULL, NULL);
    g_object_unref(icon_info);
  }
  ...
  tmp_surface = cairo_surface_create_similar(
  ...
  if (is_mounted) {
    ...
    gchar *color_string =
        g_settings_get_string(settings, "drivemount-checkmark-color");
    if (!color_string) color_string = g_strdup("#00ff00");
...
void drive_button_queue_update(DriveButton *self) {
  if (!self->update_tag) {
    self->update_tag = g_idle_add(drive_button_update, self);
  }
}

void drive_button_set_size(DriveButton *self, int icon_size) {
  g_return_if_fail(DRIVE_IS_BUTTON(self));
...
static char *escape_underscores(const char *str) {
  ...
  for (i = j = 0; str[i] != '\0'; i++, j++) {
    new_str[j] = str[i];
    if (str[i] == '_') new_str[++j] = '_';
  }
  new_str[j] = '\0';
  return new_str;
}
static GtkWidget *create_menu_item(DriveButton *self, const gchar *icon_name,
...
static void open_drive(DriveButton *self, GtkWidget *item) {
  GdkScreen *screen;
  GtkWidget *dialog;
  GError *error = NULL;
  GFile *file = NULL;
  ...
  if (!app_info) app_info = G_APP_INFO(g_desktop_app_info_new("caja.desktop"));
  ...
  if (app_info) {
    ...
    g_app_info_launch(app_info, files, G_APP_LAUNCH_CONTEXT(launch_context),
                      &error);
...
/* copied from mate-volume-manager/src/manager.c maybe there is a better way
 * than duplicating this code? */

/*
 * gvm_run_command - run the given command, replacing %d with the device node
 * and %m with the given path
 */
static void gvm_run_command(const char *device, const char *command,
                            const char *path) {
  char *argv[4];
  gchar *new_command;
  GError *error = NULL;
 ...
 * gvm_check_dvd_only)
 */
static gboolean gvm_check_dvd_only(const char *udi, const char *device,
...
static gboolean check_dvd_video(DriveButton *self) {
  GFile *file;
  ...
  udi = g_volume_get_identifier(self->volume, G_VOLUME_IDENTIFIER_KIND_HAL_UDI);
...
static gboolean check_audio_cd(DriveButton *self) {
  GFile *file;
  char *activation_uri;
  GMount *mount;
...
static void run_command(DriveButton *self, const char *command) {
  GFile *file;
...
static void mount_drive(DriveButton *self, GtkWidget *item) {
  if (self->volume) {
    GMountOperation *mount_op = gtk_mount_operation_new(NULL);
    g_volume_mount(self->volume, G_MOUNT_MOUNT_NONE, mount_op, NULL,
                   dummy_async_ready_callback, NULL);
...
static void unmount_drive(DriveButton *self, GtkWidget *item) {
  if (self->volume) {
    GMount *mount;
    ...
    g_mount_unmount_with_operation(self->mount, G_MOUNT_UNMOUNT_NONE, NULL,
                                   NULL, dummy_async_ready_callback, NULL);
  } else {
    g_return_if_reached();
  }
}
...
static void eject_drive(DriveButton *self, GtkWidget *item) {
  if (self->volume) {
    g_volume_eject_with_operation(self->volume, G_MOUNT_UNMOUNT_NONE, NULL,
                                  NULL, (GAsyncReadyCallback)eject_finish,
                                  NULL);
  } else if (self->mount) {
    g_mount_eject_with_operation(self->mount, G_MOUNT_UNMOUNT_NONE, NULL, NULL,
                                 (GAsyncReadyCallback)eject_finish, NULL);
  } else {
    g_return_if_reached();
  }
}
static void play_autoplay_media(DriveButton *self, const char *dflt) {
  run_command(self, dflt);
}

static void play_dvd(DriveButton *self, GtkWidget *item) {
  /* FIXME add an option to set this */
  play_autoplay_media(self, "totem %d");
}

static void play_cda(DriveButton *self, GtkWidget *item) {
  /* FIXME add an option to set this */
...
static void drive_button_ensure_popup(DriveButton *self) {
  ...
  if (check_dvd_video(self)) {
    item = create_menu_item(self, "media-playback-start", _("_Play DVD"),
                            G_CALLBACK(play_dvd), TRUE);
  } else if (check_audio_cd(self)) {
    item = create_menu_item(self, "media-playback-start", _("_Play CD"),
                            G_CALLBACK(play_cda), TRUE);
  } else {
  ...
  if (mounted) {
    ...
    item = create_menu_item(self, NULL, label, G_CALLBACK(unmount_drive), TRUE);
    g_free(label);
    gtk_container_add(GTK_CONTAINER(self->popup_menu), item);
  } else {
    label = g_strdup_printf(_("_Mount %s"), display_name);
    item = create_menu_item(self, NULL, label, G_CALLBACK(mount_drive), TRUE);
    g_free(label);
    gtk_container_add(GTK_CONTAINER(self->popup_menu), item);
  }
  ...
  if (ejectable) {
    ...
    item = create_menu_item(self, "media-eject", label, G_CALLBACK(eject_drive),
                            TRUE);
    g_free(label);
    gtk_container_add(GTK_CONTAINER(self->popup_menu), item);
  }
