...
static void mount_added(GVolumeMonitor *monitor, GMount *mount,
                        DriveList *self);
static void mount_changed(GVolumeMonitor *monitor, GMount *mount,
                          DriveList *self);
static void mount_removed(GVolumeMonitor *monitor, GMount *mount,
                          DriveList *self);
static void volume_added(GVolumeMonitor *monitor, GVolume *volume,
                         DriveList *self);
static void volume_changed(GVolumeMonitor *monitor, GVolume *volume,
                           DriveList *self);
static void volume_removed(GVolumeMonitor *monitor, GVolume *volume,
                           DriveList *self);
...
static void drive_list_class_init(DriveListClass *class) {
...
static void drive_list_init(DriveList *self) {
  ...
  g_signal_connect(self->settings, "changed::drivemount-checkmark-color",
                   G_CALLBACK(settings_color_changed), self);

  /* listen for drive connects/disconnects, and add
   * currently connected drives. */
  self->count = 0;
  if (!volume_monitor) volume_monitor = g_volume_monitor_get();
...
GtkWidget *drive_list_new(void) { return g_object_new(DRIVE_TYPE_LIST, NULL); }
...
static void drive_list_dispose(GObject *object) {
  ...
  g_signal_handlers_disconnect_by_func(volume_monitor, G_CALLBACK(mount_added),
                                       self);
  g_signal_handlers_disconnect_by_func(volume_monitor,
                                       G_CALLBACK(mount_changed), self);
  g_signal_handlers_disconnect_by_func(volume_monitor,
                                       G_CALLBACK(mount_removed), self);
  g_signal_handlers_disconnect_by_func(volume_monitor, G_CALLBACK(volume_added),
                                       self);
  g_signal_handlers_disconnect_by_func(volume_monitor,
  ...
  if (self->layout_tag) g_source_remove(self->layout_tag);
  self->layout_tag = 0;
...
static void list_buttons(gpointer key, gpointer value, gpointer user_data) {
...
static gboolean relayout_buttons(gpointer data) {
  ...
  if (self->count > 0) {
    ...
    for (tmp = sorted_buttons, i = 0; tmp != NULL; tmp = tmp->next, i++) {
      ...
      if (self->orientation == GTK_ORIENTATION_HORIZONTAL) {
        gtk_container_child_set(GTK_CONTAINER(self), button, "left-attach",
                                i + 1, "top-attach", 0, "width", 1, "height", 1,
                                NULL);
      } else {
        gtk_container_child_set(GTK_CONTAINER(self), button, "left-attach", 0,
                                "top-attach", i + 1, "width", 1, "height", 1,
                                NULL);
      }
    }
  } else {
    ...
    if (self->orientation == GTK_ORIENTATION_HORIZONTAL) {
      gtk_container_child_set(GTK_CONTAINER(self), self->dummy, "left-attach",
                              i + 1, "top-attach", 0, "width", 1, "height", 1,
                              NULL);
    } else {
      gtk_container_child_set(GTK_CONTAINER(self), self->dummy, "left-attach",
                              0, "top-attach", i + 1, "width", 1, "height", 1,
                              NULL);
    }
  }
  return FALSE;
...
static void mount_changed(GVolumeMonitor *monitor, GMount *mount,
  ...
  if (button) drive_button_queue_update(button);
}
...
static void add_volume(DriveList *self, GVolume *volume) {
  ...
  if (g_hash_table_lookup(self->volumes, volume) != NULL) return;
...
static void add_mount(DriveList *self, GMount *mount) {
  ...
  if (g_hash_table_lookup(self->mounts, mount) != NULL) return;
...
void drive_list_set_orientation(DriveList *self, GtkOrientation orientation) {
...
static void set_icon_size(gpointer key, gpointer value, gpointer user_data) {
...
void drive_list_set_panel_size(DriveList *self, int panel_size) {
  g_return_if_fail(DRIVE_IS_LIST(self));
...
static void set_button_relief(gpointer key, gpointer value,
                              gpointer user_data) {
