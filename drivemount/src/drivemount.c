...
static void change_orient(MatePanelApplet *applet, MatePanelAppletOrient o,
...
static void size_allocate(MatePanelApplet *applet, GdkRectangle *allocation,
                          DriveList *drive_list) {
  int size;
...
static void change_background(MatePanelApplet *applet,
                              MatePanelAppletBackgroundType type,
                              GdkColor *colour, cairo_pattern_t *pattern,
                              DriveList *drivelist) {
  switch (type) {
...
static void display_about_dialog(GtkAction *action, DriveList *drive_list) {
  const gchar *authors[] = {"James Henstridge <jamesh@canonical.com>", NULL};

  const gchar *documenters[] = {"Dan Mueth <muet@alumni.uchicago.edu>",
...
static void display_help(GtkAction *action, DriveList *drive_list) {
  GdkScreen *screen;
  GError *error = NULL;

  screen = gtk_widget_get_screen(GTK_WIDGET(drive_list));

  gtk_show_uri_on_window(NULL, "help:mate-drivemount",
...
static const GtkActionEntry applet_menu_actions[] = {
    {"Help", "help-browser", N_("_Help"), NULL, NULL, G_CALLBACK(display_help)},
    {"About", "help-about", N_("_About"), NULL, NULL,
     G_CALLBACK(display_about_dialog)}};

static gboolean applet_factory(MatePanelApplet *applet, const char *iid,
                               gpointer user_data) {
  gboolean ret = FALSE;
  ...
  if (!strcmp(iid, drivemount_iid)) {
    ...
    g_signal_connect_object(applet, "change-orient", G_CALLBACK(change_orient),
                            drive_list, 0);
    g_signal_connect_object(applet, "size-allocate", G_CALLBACK(size_allocate),
                            drive_list, 0);
    g_signal_connect(applet, "change-background", G_CALLBACK(change_background),
                     drive_list);

    /* set initial state */
    change_orient(applet, mate_panel_applet_get_orient(applet),
    ...
    gtk_action_group_add_actions(action_group, applet_menu_actions,
                                 G_N_ELEMENTS(applet_menu_actions), drive_list);
    mate_panel_applet_setup_menu_from_resource(
        applet, DRIVEMOUNT_RESOURCE_PATH "drivemount-applet-menu.xml",
        action_group);
...
MATE_PANEL_APPLET_OUT_PROCESS_FACTORY(factory_iid, PANEL_TYPE_APPLET,
                                      "Drive-Mount-Applet", applet_factory,
                                      NULL)
