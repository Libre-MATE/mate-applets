...
static void link_cb(GtkButton *button, gpointer data) {
  gtk_show_uri_on_window(NULL, "http://www.weather.com/",
...
static void override_widget_font(GtkWidget *widget,
  ...
  if (!provider_added) provider = gtk_css_provider_new();

  gtk_widget_set_name(GTK_WIDGET(widget), "MateWeatherAppletTextView");
  css = g_strdup_printf("#MateWeatherAppletTextView { %s %s %s %s }", family,
                        weight, style, size);
...
void mateweather_dialog_update(MateWeatherDialog *dialog) {
  ...
  if (dialog->applet->mateweather_info == NULL) return;

  info = dialog->applet->mateweather_info;

  /* Update icon */
  icon_name = weather_info_get_icon_name(info);
  gtk_image_set_from_icon_name(GTK_IMAGE(dialog->cond_image), icon_name,
                               GTK_ICON_SIZE_DIALOG);

  /* Update current condition fields and forecast */
  gtk_label_set_text(GTK_LABEL(dialog->cond_location),
                     weather_info_get_location_name(info));
  gtk_label_set_text(GTK_LABEL(dialog->cond_update),
                     weather_info_get_update(info));
  gtk_label_set_text(GTK_LABEL(dialog->cond_cond),
  ...
  gtk_label_set_text(GTK_LABEL(dialog->cond_apparent),
                     weather_info_get_apparent(info));
  gtk_label_set_text(GTK_LABEL(dialog->cond_dew), weather_info_get_dew(info));
  gtk_label_set_text(GTK_LABEL(dialog->cond_humidity),
                     weather_info_get_humidity(info));
  gtk_label_set_text(GTK_LABEL(dialog->cond_wind), weather_info_get_wind(info));
  gtk_label_set_text(GTK_LABEL(dialog->cond_pressure),
                     weather_info_get_pressure(info));
  gtk_label_set_text(GTK_LABEL(dialog->cond_vis),
                     weather_info_get_visibility(info));
  gtk_label_set_text(GTK_LABEL(dialog->cond_sunrise),
                     weather_info_get_sunrise(info));
  gtk_label_set_text(GTK_LABEL(dialog->cond_sunset),
  ...
  if (dialog->applet->mateweather_pref.location->zone_valid) {
    ...
    forecast =
        g_strdup(weather_info_get_forecast(dialog->applet->mateweather_info));
    if (forecast) {
      ...
      gtk_text_buffer_set_text(
          buffer, _("Forecast not currently available for this location."), -1);
    }
    gtk_widget_show(
        gtk_notebook_get_nth_page(GTK_NOTEBOOK(dialog->weather_notebook), 1));
  } else {
    gtk_widget_hide(
        gtk_notebook_get_nth_page(GTK_NOTEBOOK(dialog->weather_notebook), 1));
  }
  ...
  if (dialog->applet->mateweather_pref.radar_enabled) {
    ...
    gtk_widget_show(
        gtk_notebook_get_nth_page(GTK_NOTEBOOK(dialog->weather_notebook), 2));
...
static void mateweather_dialog_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
...
static void mateweather_dialog_get_property(GObject *object, guint prop_id,
...
static void mateweather_dialog_init(MateWeatherDialog *dialog) {
  gtk_widget_init_template(GTK_WIDGET(dialog));
}

static GObject *mateweather_dialog_constructor(
...
GtkWidget *mateweather_dialog_new(MateWeatherApplet *applet) {
  return g_object_new(MATEWEATHER_TYPE_DIALOG, "mateweather-applet", applet,
                      NULL);
}
...
static void mateweather_dialog_class_init(MateWeatherDialogClass *klass) {
  ...
  gtk_widget_class_set_template_from_resource(
      widget_class, WEATHER_RESOURCE_PATH "mateweather-dialog.ui");

  gtk_widget_class_bind_template_child(widget_class, MateWeatherDialog,
                                       weather_notebook);
  gtk_widget_class_bind_template_child(widget_class, MateWeatherDialog,
                                       cond_location);
  gtk_widget_class_bind_template_child(widget_class, MateWeatherDialog,
