...
static void about_cb(GtkAction *action, MateWeatherApplet *gw_applet) {
  mateweather_about_run(gw_applet);
}

static void help_cb(GtkAction *action, MateWeatherApplet *gw_applet) {
  GError *error = NULL;

  gtk_show_uri_on_window(NULL, "help:mateweather", gtk_get_current_event_time(),
                         &error);
...
static void pref_cb(GtkAction *action, MateWeatherApplet *gw_applet) {
...
static void details_cb(GtkAction *action, MateWeatherApplet *gw_applet) {
...
static void update_cb(GtkAction *action, MateWeatherApplet *gw_applet) {
  mateweather_update(gw_applet);
}

static const GtkActionEntry weather_applet_menu_actions[] = {
    {"Details", NULL, N_("_Details"), NULL, NULL, G_CALLBACK(details_cb)},
    {"Update", "view-refresh", N_("_Update"), NULL, NULL,
     G_CALLBACK(update_cb)},
    {"Props", "document-properties", N_("_Preferences"), NULL, NULL,
     G_CALLBACK(pref_cb)},
...
static void place_widgets(MateWeatherApplet *gw_applet) {
  ...
  if ((gw_applet->mateweather_info) &&
    ...
    gw_applet->image =
        gtk_image_new_from_icon_name("weather-storm", GTK_ICON_SIZE_BUTTON);
    ;
  }
  ...
  /* Pack the box */
  if (gw_applet->box) gtk_widget_destroy(gw_applet->box);
...
static void change_orient_cb(MatePanelApplet *w, MatePanelAppletOrient o,
                             gpointer data) {
...
static void size_allocate_cb(MatePanelApplet *w, GtkAllocation *allocation,
                             gpointer data) {
  guint size;
  MateWeatherApplet *gw_applet = (MateWeatherApplet *)data;
  ...
  if (gw_applet->size == size) return;

  gw_applet->size = size;
  place_widgets(gw_applet);
}

static gboolean clicked_cb(GtkWidget *widget, GdkEventButton *ev,
                           gpointer data) {
  MateWeatherApplet *gw_applet = data;

  if ((ev == NULL) || (ev->button != 1)) return FALSE;
...
static void applet_destroy(GtkWidget *widget, MateWeatherApplet *gw_applet) {
  GNetworkMonitor *monitor;

  if (gw_applet->pref_dialog) gtk_widget_destroy(gw_applet->pref_dialog);

  if (gw_applet->details_dialog) gtk_widget_destroy(gw_applet->details_dialog);
...
void mateweather_applet_create(MateWeatherApplet *gw_applet) {
  ...
  mate_panel_applet_set_flags(gw_applet->applet,
  ...
  g_signal_connect(gw_applet->applet, "destroy", G_CALLBACK(applet_destroy),
                   gw_applet);
  ...
  gtk_action_group_add_actions(action_group, weather_applet_menu_actions,
                               G_N_ELEMENTS(weather_applet_menu_actions),
                               gw_applet);
  mate_panel_applet_setup_menu_from_resource(
      gw_applet->applet, WEATHER_RESOURCE_PATH "mateweather-applet-menu.xml",
      action_group);
...
static void update_finish(WeatherInfo *info, gpointer data) {
  ...
  /* Update timer */
  if (gw_applet->timeout_tag > 0) g_source_remove(gw_applet->timeout_tag);
  if (gw_applet->mateweather_pref.update_enabled) {
    ...
    if ((info != NULL) &&
        ((nxtSunEvent = weather_info_next_sun_event(info)) >= 0)) {
  ...
  if ((info != NULL) && weather_info_is_valid(info)) {
    ...
    if (gw_applet->mateweather_pref.show_notifications) {
      ...
      if (!notify_is_initted()) notify_init(_("Weather Forecast"));
      ...
      if (notify_is_initted()) {
        ...
        /* Show notification */
        message =
            g_strdup_printf("%s: %s", weather_info_get_location_name(info),
        ...
        if (icon == NULL) icon = "stock-unknown";
    ...
    if (gw_fault_counter >= MAX_CONSECUTIVE_FAULTS) {
      ...
      /* Update dialog -- if one is present */
      if (gw_applet->details_dialog)
        mateweather_dialog_update(
            MATEWEATHER_DIALOG(gw_applet->details_dialog));
...
void mateweather_update(MateWeatherApplet *gw_applet) {
  ...
  prefs.type =
      gw_applet->mateweather_pref.detailed ? FORECAST_ZONE : FORECAST_STATE;
  ...
  if (gw_applet->mateweather_info &&
      weather_location_equal(
          weather_info_get_location(gw_applet->mateweather_info),
          gw_applet->mateweather_pref.location)) {
    weather_info_update(gw_applet->mateweather_info, &prefs, update_finish,
                        gw_applet);
  } else {
    weather_info_free(gw_applet->mateweather_info);
    gw_applet->mateweather_info = weather_info_new(
        gw_applet->mateweather_pref.location, &prefs, update_finish, gw_applet);
  }
}
