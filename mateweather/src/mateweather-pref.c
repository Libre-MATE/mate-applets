...
static void add_atk_relation(GtkWidget* widget1, GtkWidget* widget2,
...
void set_access_namedesc(GtkWidget* widget, const gchar* name,
                         const gchar* desc) {
...
static void mateweather_pref_set_accessibility(MateWeatherPref* pref) {
  ...
  add_atk_relation(pref->priv->basic_update_btn, pref->priv->basic_update_spin,
                   ATK_RELATION_CONTROLLER_FOR);
  add_atk_relation(pref->priv->basic_radar_btn, pref->priv->basic_radar_url_btn,
                   ATK_RELATION_CONTROLLER_FOR);
  add_atk_relation(pref->priv->basic_radar_btn,
                   pref->priv->basic_radar_url_entry,
                   ATK_RELATION_CONTROLLER_FOR);

  add_atk_relation(pref->priv->basic_update_spin, pref->priv->basic_update_btn,
                   ATK_RELATION_CONTROLLED_BY);
  add_atk_relation(pref->priv->basic_radar_url_btn, pref->priv->basic_radar_btn,
                   ATK_RELATION_CONTROLLED_BY);
  add_atk_relation(pref->priv->basic_radar_url_entry,
                   pref->priv->basic_radar_btn, ATK_RELATION_CONTROLLED_BY);

  /* Accessible Name and Description for the components in Preference Dialog */
  set_access_namedesc(pref->priv->tree, _("Location view"),
                      _("Select Location from the list"));
  set_access_namedesc(pref->priv->basic_update_spin, _("Update spin button"),
                      _("Spinbutton for updating"));
  set_access_namedesc(pref->priv->basic_radar_url_entry, _("Address Entry"),
                      _("Enter the URL"));
}
...
static gboolean update_dialog(MateWeatherPref* pref) {
  ...
  gtk_combo_box_set_active(
      GTK_COMBO_BOX(pref->priv->basic_temp_combo),
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(pref->priv->basic_radar_url_btn),
...
#ifdef HAVE_LIBNOTIFY
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(pref->priv->basic_show_notifications_btn),
      gw_applet->mateweather_pref.show_notifications);
#endif
  return TRUE;
}

static void on_row_selected(GtkTreeSelection* selection,
  ...
  gw_applet->mateweather_pref.location =
      weather_location_new(loc->name, loc->code, loc->zone, loc->radar,
                           loc->coordinates, NULL, NULL);

  mateweather_update(gw_applet);
}

static gboolean compare_location(GtkTreeModel* model, GtkTreePath* path,
  ...
  if (!weather_location_equal(loc,
                              pref->priv->applet->mateweather_pref.location)) {
    return FALSE;
  }
...
static void load_locations(MateWeatherPref* pref) {
  ...
  column = gtk_tree_view_column_new_with_attributes(
      "not used", cell_renderer, "text", MATEWEATHER_XML_COL_LOC, NULL);
  ...
  if (!pref->priv->model) {
    GtkWidget* d =
        gtk_message_dialog_new(NULL, 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK,
                               _("Failed to load the Locations XML database.  "
  ...
  if (pref->priv->applet->mateweather_pref.location) {
    ...
    gtk_tree_model_foreach(GTK_TREE_MODEL(pref->priv->model), compare_location,
                           pref);
  }
}

static void on_show_notifications_toggled(GtkToggleButton* button,
                                          MateWeatherPref* pref) {
  MateWeatherApplet* gw_applet = pref->priv->applet;
...
static void on_auto_update_toggled(GtkToggleButton* button,
                                   MateWeatherPref* pref) {
  MateWeatherApplet* gw_applet = pref->priv->applet;
  gboolean toggled;
  gint nxtSunEvent;
  ...
  if (gw_applet->mateweather_pref.update_enabled) {
    gw_applet->timeout_tag = g_timeout_add_seconds(
        (guint)gw_applet->mateweather_pref.update_interval, timeout_cb,
        gw_applet);

    nxtSunEvent = weather_info_next_sun_event(gw_applet->mateweather_info);
    if (nxtSunEvent >= 0) {
      gw_applet->suncalc_timeout_tag = g_timeout_add_seconds(
          (guint)nxtSunEvent, suncalc_timeout_cb, gw_applet);
    }
  }
}

static void on_temp_combo_changed(GtkComboBox* combo, MateWeatherPref* pref) {
  MateWeatherApplet* gw_applet = pref->priv->applet;
...
static void on_speed_combo_changed(GtkComboBox* combo, MateWeatherPref* pref) {
  MateWeatherApplet* gw_applet = pref->priv->applet;
...
static void on_pres_combo_changed(GtkComboBox* combo, MateWeatherPref* pref) {
  MateWeatherApplet* gw_applet = pref->priv->applet;
...
static void on_dist_combo_changed(GtkComboBox* combo, MateWeatherPref* pref) {
  MateWeatherApplet* gw_applet = pref->priv->applet;
...
static void on_radar_toggled(GtkToggleButton* button, MateWeatherPref* pref) {
  MateWeatherApplet* gw_applet = pref->priv->applet;
  gboolean toggled;
  ...
  if (toggled == FALSE || gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
                              pref->priv->basic_radar_url_btn)) == TRUE) {
    soft_set_sensitive(pref->priv->basic_radar_url_hbox, toggled);
  }
}

static void on_use_radar_url_toggled(GtkToggleButton* button,
                                     MateWeatherPref* pref) {
  MateWeatherApplet* gw_applet = pref->priv->applet;
  gboolean toggled;
...
static gboolean on_radar_url_changed(GtkWidget* widget, GdkEventFocus* event,
                                     MateWeatherPref* pref) {
  MateWeatherApplet* gw_applet = pref->priv->applet;
  gchar* text;
  ...
  g_settings_set_string(gw_applet->settings, "radar",
                        gw_applet->mateweather_pref.radar);

  return FALSE;
}

static void on_update_interval_changed(GtkSpinButton* button,
                                       MateWeatherPref* pref) {
  MateWeatherApplet* gw_applet = pref->priv->applet;

  gw_applet->mateweather_pref.update_interval =
      gtk_spin_button_get_value_as_int(button) * 60;
  g_settings_set_int(gw_applet->settings, "auto-update-interval",
                     gw_applet->mateweather_pref.update_interval);
...
static gboolean free_data(GtkTreeModel* model, GtkTreePath* path,
...
static gboolean find_location(GtkTreeModel* model, GtkTreeIter* iter,
                              const gchar* location, gboolean go_parent) {
  ...
  if ((len = strlen(location)) == 0) return FALSE;
...
static void on_find_next_clicked(GtkButton* button, MateWeatherPref* pref) {
  GtkTreeView* tree;
  GtkTreeModel* model;
  GtkEntry* entry;
...
static void help_cb(GtkDialog* dialog, MateWeatherPref* pref) {
  ...
  current_page =
      gtk_notebook_get_current_page(GTK_NOTEBOOK(pref->priv->notebook));
  uri = g_strdup_printf("help:mateweather/mateweather-prefs#mateweather-%s",
                        (current_page == 0) ? "metric" : "change-location");
  gtk_show_uri_on_window(GTK_WINDOW(dialog), uri, gtk_get_current_event_time(),
                         &error);
  g_free(uri);
...
static void on_response(GtkDialog* dialog, gint id, MateWeatherPref* pref) {
...
static void mateweather_pref_create(MateWeatherPref* pref) {
  ...
  gtk_dialog_add_buttons(GTK_DIALOG(pref), "gtk-close", GTK_RESPONSE_CLOSE,
                         "gtk-help", GTK_RESPONSE_HELP, NULL);
  ...
  pref->priv->basic_update_btn =
      gtk_check_button_new_with_mnemonic(_("_Automatically update every:"));
  ...
  gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(temp_combo),
                                 _("Fahrenheit"));
  gtk_widget_show(temp_combo);

  if (!g_settings_is_writable(pref->priv->applet->settings,
                              "temperature-unit")) {
...
#ifdef RADARMAP
  pref->priv->basic_radar_btn =
      gtk_check_button_new_with_mnemonic(_("Enable _radar map"));
  ...
  pref->priv->basic_radar_url_btn = gtk_check_button_new_with_mnemonic(
      _("Use _custom address for radar map"));
  gtk_widget_show(pref->priv->basic_radar_url_btn);
  gtk_box_pack_start(GTK_BOX(radar_toggle_hbox),
                     pref->priv->basic_radar_url_btn, FALSE, FALSE, 0);

  if (!g_settings_is_writable(pref->priv->applet->settings,
                              "use-custom-radar-url")) {
    hard_set_sensitive(pref->priv->basic_radar_url_btn, FALSE);
  }

  pref->priv->basic_radar_url_hbox =
      gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 12);
  gtk_widget_show(pref->priv->basic_radar_url_hbox);

  label = gtk_label_new("    ");
  gtk_widget_show(label);
  gtk_box_pack_start(GTK_BOX(pref->priv->basic_radar_url_hbox), label, FALSE,
                     FALSE, 0);
  ...
  pref->priv->basic_show_notifications_btn =
      gtk_check_button_new_with_mnemonic(_("Show _notifications"));
  ...
  pref_basic_update_lbl =
      gtk_label_new_with_mnemonic(_("_Automatically update every:"));
  ...
  pref->priv->basic_update_spin =
      gtk_spin_button_new(pref_basic_update_spin_adj, 1, 0);
  gtk_widget_show(pref->priv->basic_update_spin);

  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(pref->priv->basic_update_spin),
                              TRUE);
  gtk_spin_button_set_update_policy(
      GTK_SPIN_BUTTON(pref->priv->basic_update_spin), GTK_UPDATE_IF_VALID);
  ...
  gtk_box_pack_start(GTK_BOX(pref_basic_update_hbox),
                     pref->priv->basic_update_btn, FALSE, TRUE, 0);
  gtk_box_pack_start(GTK_BOX(pref_basic_update_hbox), value_hbox, FALSE, FALSE,
                     0);
  gtk_box_pack_start(GTK_BOX(value_hbox), pref->priv->basic_update_spin, FALSE,
                     FALSE, 0);
  gtk_box_pack_start(GTK_BOX(value_hbox), pref_basic_update_sec_lbl, FALSE,
                     FALSE, 0);
   ...
   */
  gtk_box_pack_start(GTK_BOX(vbox), pref->priv->basic_show_notifications_btn,
                     TRUE, TRUE, 0);
#endif
  ...
  gtk_notebook_set_tab_label(
      GTK_NOTEBOOK(pref->priv->notebook),
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(pref->priv->notebook), 0),
      pref_basic_note_lbl);
  ...
  gtk_label_set_mnemonic_widget(GTK_LABEL(tree_label),
                                GTK_WIDGET(pref->priv->tree));
  ...
  gtk_box_pack_start(GTK_BOX(pref_find_hbox), pref->priv->find_entry, TRUE,
                     TRUE, 0);
  gtk_box_pack_start(GTK_BOX(pref_find_hbox), pref->priv->find_next_btn, FALSE,
                     FALSE, 0);
  ...
  gtk_notebook_set_tab_label(
      GTK_NOTEBOOK(pref->priv->notebook),
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(pref->priv->notebook), 1),
      pref_loc_note_lbl);

  mateweather_pref_set_accessibility(pref);
  gtk_label_set_mnemonic_widget(GTK_LABEL(pref_basic_update_sec_lbl),
                                pref->priv->basic_update_spin);
  ...
  /* signals */
  g_signal_connect(temp_combo, "changed", G_CALLBACK(on_temp_combo_changed),
                   pref);
  g_signal_connect(speed_combo, "changed", G_CALLBACK(on_speed_combo_changed),
                   pref);
  g_signal_connect(dist_combo, "changed", G_CALLBACK(on_dist_combo_changed),
                   pref);
  g_signal_connect(pres_combo, "changed", G_CALLBACK(on_pres_combo_changed),
                   pref);
  g_signal_connect(pref->priv->basic_update_btn, "toggled",
                   G_CALLBACK(on_auto_update_toggled), pref);
#ifdef RADARMAP
  g_signal_connect(pref->priv->basic_radar_btn, "toggled",
                   G_CALLBACK(on_radar_toggled), pref);
  g_signal_connect(pref->priv->basic_radar_url_btn, "toggled",
                   G_CALLBACK(on_use_radar_url_toggled), pref);
  g_signal_connect(pref->priv->basic_radar_url_entry, "focus-out-event",
                   G_CALLBACK(on_radar_url_changed), pref);
#endif /* RADARMAP */
#ifdef HAVE_LIBNOTIFY
  g_signal_connect(pref->priv->basic_show_notifications_btn, "toggled",
                   G_CALLBACK(on_show_notifications_toggled), pref);
#endif /* HAVE_LIBNOTIFY */
  g_signal_connect(pref->priv->find_next_btn, "clicked",
                   G_CALLBACK(on_find_next_clicked), pref);
  g_signal_connect(pref->priv->find_entry, "changed",
                   G_CALLBACK(find_entry_changed), pref);
  g_signal_connect(selection, "changed", G_CALLBACK(on_row_selected), pref);
  g_signal_connect(pref->priv->basic_update_spin, "value-changed",
                   G_CALLBACK(on_update_interval_changed), pref);
  g_signal_connect(pref, "response", G_CALLBACK(on_response), pref);
}

static void mateweather_pref_set_property(GObject* object, guint prop_id,
                                          const GValue* value,
                                          GParamSpec* pspec) {
...
static void mateweather_pref_get_property(GObject* object, guint prop_id,
...
static GObject* mateweather_pref_constructor(
    GType type, guint n_construct_params,
...
GtkWidget* mateweather_pref_new(MateWeatherApplet* applet) {
  return g_object_new(MATEWEATHER_TYPE_PREF, "mateweather-applet", applet,
                      NULL);
}
...
static void mateweather_pref_class_init(MateWeatherPrefClass* klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_MATEWEATHER_APPLET,
      g_param_spec_pointer("mateweather-applet", "MateWeather Applet",
