...
static void about_cb(GtkAction* action, AccessxStatusApplet* sapplet) {
  static const gchar* authors[] = {"Calum Benson <calum.benson@sun.com>",
                                   "Bill Haneman <bill.haneman@sun.com>", NULL};
  ...
  gtk_show_about_dialog(
      NULL, "title", _("About AccessX Status"), "version", VERSION, "comments",
...
static void help_cb(GtkAction* action, AccessxStatusApplet* sapplet) {
  ...
  gtk_show_uri_on_window(NULL, "help:mate-accessx-status",
  ...
  if (error) {
    ...
    GtkWidget* dialog = gtk_message_dialog_new(
        ...
        GTK_BUTTONS_CLOSE,
        _("There was an error launching the help viewer: %s"), error->message);
...
static void dialog_cb(GtkAction* action, AccessxStatusApplet* sapplet) {
  ...
  appinfo = g_app_info_create_from_commandline(
      ...
      _("Open the keyboard preferences dialog"), G_APP_INFO_CREATE_NONE,
      &error);
  ...
  if (!error) {
    ...
    g_app_info_launch(appinfo, NULL, G_APP_LAUNCH_CONTEXT(launch_context),
                      &error);
...
static const GtkActionEntry accessx_status_applet_menu_actions[] = {
    {"Dialog", "document-properties", N_("_Keyboard Accessibility Preferences"),
...
static XkbDescPtr accessx_status_applet_get_xkb_desc(
  ...
  if (sapplet->xkb == NULL) {
    ...
    display = XkbOpenDisplay(display_name, &xkb_base_event_type, &ir, NULL,
                             NULL, &reason_return);
    g_assert(display);
...
static gboolean accessx_status_applet_xkb_select(AccessxStatusApplet* sapplet) {
  ...
  if (retval) {
    retval = XkbSelectEvents(display, XkbUseCoreKbd, XkbAllEventsMask,
                             XkbAllEventsMask);
...
static void accessx_status_applet_init_modifiers(AccessxStatusApplet* sapplet) {
  ...
  unsigned int meta_mask =
      XkbKeysymToModifiers(sapplet->xkb_display, XK_Meta_L);
  ...
  alt_gr_mask =
      XkbKeysymToModifiers(sapplet->xkb_display, XK_Mode_switch) |
...
static gboolean timer_reset_slowkeys_image(AccessxStatusApplet* sapplet) {
  ...
  cairo_surface_t* surface = gtk_icon_theme_load_surface(
      icon_theme, SLOWKEYS_IDLE_ICON, icon_size, icon_scale, NULL, 0, NULL);
...
static gboolean timer_reset_bouncekeys_image(AccessxStatusApplet* sapplet) {
  ...
  gint icon_scale = gtk_widget_get_scale_factor(GTK_WIDGET(sapplet->applet));
  cairo_surface_t* surface = gtk_icon_theme_load_surface(
      icon_theme, BOUNCEKEYS_ICON, icon_size, icon_scale, NULL, 0, NULL);
...
static GdkPixbuf* accessx_status_applet_get_glyph_pixbuf(GtkWidget* widget,
                                                         GdkPixbuf* base,
                                                         GdkRGBA* fg,
                                                         gchar* glyphstring) {
...
static cairo_surface_t* accessx_status_applet_altgraph_image(
    AccessxStatusApplet* sapplet, GtkStateFlags state) {
  ...
  icon_base = gtk_icon_theme_load_icon_for_scale(
      icon_theme, icon_name, icon_size, icon_scale, 0, NULL);
   ...
   * in your locale.
   */
  glyph_pixbuf = accessx_status_applet_get_glyph_pixbuf(
      GTK_WIDGET(sapplet->applet), pixbuf, &fg, ("æ"));
  gdk_pixbuf_composite(glyph_pixbuf, pixbuf, 0, 0,
                       gdk_pixbuf_get_width(glyph_pixbuf),
                       gdk_pixbuf_get_height(glyph_pixbuf), 0., 0., 1.0, 1.0,
...
static cairo_surface_t* accessx_status_applet_slowkeys_image(
    AccessxStatusApplet* sapplet, XkbAccessXNotifyEvent* event) {
  ...
  if (event != NULL) {
    ...
    switch (event->detail) {
      ...
      case XkbAXN_SKReject:
        ...
        g_timeout_add_full(G_PRIORITY_HIGH_IDLE, MAX(event->sk_delay, 150),
                           (GSourceFunc)timer_reset_slowkeys_image, sapplet,
                           NULL);
        break;
  ...
  ret_pixbuf = gtk_icon_theme_load_icon_for_scale(
      icon_theme, icon_name, icon_size, icon_scale, 0, NULL);
  ...
  if (!is_idle) {
    ...
    if (event && window) {
      KeySym keysym =
          XkbKeycodeToKeysym(GDK_WINDOW_XDISPLAY(window), event->keycode, 0, 0);
    ...
    glyph_pixbuf = accessx_status_applet_get_glyph_pixbuf(
        GTK_WIDGET(sapplet->applet), ret_pixbuf, &fg, glyphstring);
...
static cairo_surface_t* accessx_status_applet_bouncekeys_image(
    AccessxStatusApplet* sapplet, XkbAccessXNotifyEvent* event) {
  GdkRGBA fg;
  ...
  if (event != NULL) {
    switch (event->detail) {
      ...
      case XkbAXN_BKReject:
        ...
        g_timeout_add_full(
            G_PRIORITY_HIGH_IDLE, MAX(event->debounce_delay, 150),
            (GSourceFunc)timer_reset_bouncekeys_image, sapplet, NULL);
        break;
      default:
        icon_name = ACCESSX_BASE_ICON;
        break;
  ...
  tmp_pixbuf = gtk_icon_theme_load_icon_for_scale(
      icon_theme, icon_name, icon_size, icon_scale, 0, NULL);
  ...
  if (tmp_pixbuf) {
    ...
    g_object_unref(tmp_pixbuf);
    glyph_pixbuf = accessx_status_applet_get_glyph_pixbuf(
        GTK_WIDGET(sapplet->applet), icon_base, &fg, glyphstring);

    gdk_pixbuf_composite(glyph_pixbuf, icon_base, 2, 2,
                         gdk_pixbuf_get_width(glyph_pixbuf) - 2,
                         gdk_pixbuf_get_height(glyph_pixbuf) - 2, -2., -2., 1.0,
                         1.0, GDK_INTERP_NEAREST, 96);

    gdk_pixbuf_composite(glyph_pixbuf, icon_base, 1, 1,
                         gdk_pixbuf_get_width(glyph_pixbuf) - 1,
                         gdk_pixbuf_get_height(glyph_pixbuf) - 1, 1., 1., 1.0,
                         1.0, GDK_INTERP_NEAREST, alpha);
...
static cairo_surface_t* accessx_status_applet_mousekeys_image(
    AccessxStatusApplet* sapplet, XkbStateNotifyEvent* event) {
  ...
  gint icon_scale = gtk_widget_get_scale_factor(GTK_WIDGET(sapplet->applet));
  tmp_pixbuf = gtk_icon_theme_load_icon_for_scale(
      icon_theme, MOUSEKEYS_BASE_ICON, icon_size, icon_scale, 0, NULL);
  ...
  if (mouse_pixbuf && event && event->ptr_buttons) {
    ...
    for (i = 0; i < G_N_ELEMENTS(button_icons); ++i) {
      if (event->ptr_buttons & button_icons[i].mask) {
        button_pixbuf = gtk_icon_theme_load_icon_for_scale(
            icon_theme, button_icons[i].icon_name, icon_size, icon_scale, 0,
            NULL);

        gdk_pixbuf_composite(button_pixbuf, mouse_pixbuf, 0, 0,
                             gdk_pixbuf_get_width(button_pixbuf),
                             gdk_pixbuf_get_height(button_pixbuf), 0.0, 0.0,
  ...
  dot_pixbuf = gtk_icon_theme_load_icon_for_scale(
      icon_theme, which_dot, icon_size, icon_scale, 0, NULL);

  gdk_pixbuf_composite(dot_pixbuf, mouse_pixbuf, 0, 0,
                       gdk_pixbuf_get_width(dot_pixbuf),
                       gdk_pixbuf_get_height(dot_pixbuf), 0.0, 0.0, 1.0, 1.0,
                       GDK_INTERP_NEAREST, 255);

  surface =
      gdk_cairo_surface_create_from_pixbuf(mouse_pixbuf, icon_scale, NULL);
...
static void accessx_status_applet_set_state_icon(AccessxStatusApplet* sapplet,
  ...
  if (surface == NULL && icon_name != NULL) {
    ...
    icon_size = mate_panel_applet_get_size(sapplet->applet) - ICON_PADDING;
    icon_scale = gtk_widget_get_scale_factor(GTK_WIDGET(sapplet->applet));
    surface = gtk_icon_theme_load_surface(icon_theme, icon_name, icon_size,
...
static void accessx_status_applet_update(AccessxStatusApplet* sapplet,
    ...
    for (i = 0; i < modifier_n; ++i) {
      if (modifiers[i].indicator != NULL && modifiers[i].mask) {
        if (locked_mods & modifiers[i].mask) {
          ...
          accessx_status_applet_set_state_icon(sapplet, &modifiers[i],
  ...
  if ((notify_type & ACCESSX_STATUS_SLOWKEYS) && (event != NULL)) {
    cairo_surface_t* surface =
        accessx_status_applet_slowkeys_image(sapplet, &event->accessx);
    gtk_image_set_from_surface(GTK_IMAGE(sapplet->slowfoo), surface);
    cairo_surface_destroy(surface);
  }
  ...
  if (notify_type & ACCESSX_STATUS_ENABLED) {
    ...
    /* XkbMouseKeysMask | XkbStickyKeysMask | XkbSlowKeysMask |
     * XkbBounceKeysMask */
    XkbGetControls(GDK_WINDOW_XDISPLAY(window), XkbAllControlsMask,
                   sapplet->xkb);
...
static void accessx_status_applet_notify_xkb_ax(AccessxStatusApplet* sapplet,
...
static void accessx_status_applet_notify_xkb_state(AccessxStatusApplet* sapplet,
...
static void accessx_status_applet_notify_xkb_device(
    AccessxStatusApplet* sapplet, XkbExtensionDeviceNotifyEvent* event) {
  if (event->reason == XkbXI_IndicatorStateMask) {
    if (event->led_state &= ALT_GRAPH_LED_MASK) {
      ...
      accessx_status_applet_set_state_icon(sapplet, &modifiers[modifier_Mod5],
                                           GTK_STATE_FLAG_NORMAL);
    } else {
...
static void accessx_status_applet_notify_xkb_controls(
    AccessxStatusApplet* sapplet, XkbControlsNotifyEvent* event) {
  unsigned int mask = XkbStickyKeysMask | XkbSlowKeysMask | XkbBounceKeysMask |
                      XkbMouseKeysMask;
...
static void accessx_status_applet_notify_xkb_event(AccessxStatusApplet* sapplet,
                                                   XkbEvent* event) {
  switch (event->any.xkb_type) {
    ...
    case XkbExtensionDeviceNotify:
      /* This is a hack around the fact that XFree86's XKB doesn't give AltGr
       * notifications */
...
static GdkFilterReturn accessx_status_xkb_filter(GdkXEvent* gdk_xevent,
                                                 GdkEvent* event,
                                                 gpointer user_data) {
...
static void accessx_status_applet_reparent_widget(GtkWidget* widget,
...
static void accessx_status_applet_layout_box(AccessxStatusApplet* sapplet,
                                             GtkWidget* box,
                                             GtkWidget* stickyfoo) {
  AtkObject* atko;
  ...
  atk_object_set_description(
      atko, _("Shows keyboard status when accessibility features are used."));
...
static AccessxStatusApplet* create_applet(MatePanelApplet* applet) {
  ...
  surface = gtk_icon_theme_load_surface(icon_theme, SHIFT_KEY_ICON, icon_size,
                                        icon_scale, NULL, 0, NULL);

  sapplet->shift_indicator = gtk_image_new_from_surface(surface);
  cairo_surface_destroy(surface);

  surface = gtk_icon_theme_load_surface(icon_theme, CONTROL_KEY_ICON, icon_size,
                                        icon_scale, NULL, 0, NULL);

  sapplet->ctrl_indicator = gtk_image_new_from_surface(surface);
  cairo_surface_destroy(surface);

  surface = gtk_icon_theme_load_surface(icon_theme, ALT_KEY_ICON, icon_size,
                                        icon_scale, NULL, 0, NULL);

  sapplet->alt_indicator = gtk_image_new_from_surface(surface);
  cairo_surface_destroy(surface);

  surface = gtk_icon_theme_load_surface(icon_theme, META_KEY_ICON, icon_size,
                                        icon_scale, NULL, 0, NULL);
  ...
  surface = gtk_icon_theme_load_surface(icon_theme, HYPER_KEY_ICON, icon_size,
                                        icon_scale, NULL, 0, NULL);
  ...
  surface = gtk_icon_theme_load_surface(icon_theme, SUPER_KEY_ICON, icon_size,
                                        icon_scale, NULL, 0, NULL);
  ...
  surface =
      accessx_status_applet_altgraph_image(sapplet, GTK_STATE_FLAG_NORMAL);
  ...
  surface = gtk_icon_theme_load_surface(icon_theme, ACCESSX_APPLET, icon_size,
                                        icon_scale, NULL, 0, NULL);
...
static void accessx_status_applet_destroy(GtkWidget* widget,
                                          gpointer user_data) {
  AccessxStatusApplet* sapplet = user_data;
...
static void accessx_status_applet_reorient(GtkWidget* widget,
...
static void accessx_status_applet_resize(GtkWidget* widget, int size,
                                         gpointer user_data) {
  cairo_surface_t* surface;
  ...
  surface = gtk_icon_theme_load_surface(icon_theme, ACCESSX_APPLET,
                                        size - ICON_PADDING, icon_scale, NULL,
                                        0, NULL);
...
static void accessx_status_applet_realize(GtkWidget* widget,
                                          gpointer user_data) {
  AccessxStatusApplet* sapplet = user_data;
...
static gboolean accessx_status_applet_fill(MatePanelApplet* applet) {
  ...
  if (!gtk_widget_get_realized(sapplet->box)) {
    g_signal_connect_after(sapplet->box, "realize",
                           G_CALLBACK(accessx_status_applet_realize), sapplet);
  } else {
    accessx_status_applet_initialize(sapplet);
    was_realized = TRUE;
  }

  g_object_connect(sapplet->applet, "destroy", accessx_status_applet_destroy,
                   sapplet, "change-orient", accessx_status_applet_reorient,
                   sapplet, "change-size", accessx_status_applet_resize,
                   sapplet, NULL);

  g_signal_connect(sapplet->applet, "button-press-event",
                   G_CALLBACK(button_press_cb), sapplet);

  g_signal_connect(sapplet->applet, "key-press-event", G_CALLBACK(key_press_cb),
                   sapplet);

  action_group = gtk_action_group_new("Accessx Applet Actions");
  gtk_action_group_set_translation_domain(action_group, GETTEXT_PACKAGE);
  gtk_action_group_add_actions(action_group, accessx_status_applet_menu_actions,
                               G_N_ELEMENTS(accessx_status_applet_menu_actions),
                               sapplet);

  mate_panel_applet_setup_menu_from_resource(
      sapplet->applet, ACCESSX_RESOURCE_PATH "accessx-status-applet-menu.xml",
      action_group);
...
static gboolean accessx_status_applet_factory(MatePanelApplet* applet,
...
MATE_PANEL_APPLET_OUT_PROCESS_FACTORY("AccessxStatusAppletFactory",
                                      PANEL_TYPE_APPLET, "accessx-status",
                                      accessx_status_applet_factory, NULL)
