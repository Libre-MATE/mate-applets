...
static void about_cb(GtkAction *action, MultiloadApplet *ma) {
  const gchar *const authors[] = {"Martin Baulig <martin@home-of-linux.org>",
                                  "Todd Kulesza <fflewddur@dropline.net>",
                                  "Benoît Dejean <TazForEver@dlfp.org>",
                                  "Davyd Madeley <davyd@madeley.id.au>", NULL};
  ...
  gtk_show_about_dialog(
      ...
      _("translator-credits"), "logo-icon-name", "utilities-system-monitor",
      NULL);
}

static void help_cb(GtkAction *action, MultiloadApplet *ma) {
  GError *error = NULL;

  gtk_show_uri_on_window(NULL, "help:mate-multiload",
...
static void start_procman(MultiloadApplet *ma) {
  GError *error = NULL;
  ...
  if (monitor == NULL) monitor = g_strdup("mate-system-monitor.desktop");
  ...
  if (appinfo) {
    ...
    g_app_info_launch(G_APP_INFO(appinfo), NULL, G_APP_LAUNCH_CONTEXT(context),
                      &error);
    g_object_unref(context);
    g_object_unref(appinfo);
  } else {
    app_info = g_app_info_create_from_commandline(
        "mate-system-monitor", _("Start system-monitor"),
  ...
  if (error) {
    ...
    dialog = gtk_message_dialog_new(NULL, GTK_DIALOG_DESTROY_WITH_PARENT,
                                    ...
                                    "mate-system-monitor", error->message);
...
static void start_procman_cb(GtkAction *action, MultiloadApplet *ma) {
  start_procman(ma);
}

static void multiload_change_size_cb(MatePanelApplet *applet, gint size,
                                     gpointer data) {
...
static void multiload_change_orient_cb(MatePanelApplet *applet, gint arg1,
                                       gpointer data) {
...
static void multiload_destroy_cb(GtkWidget *widget, gpointer data) {
  guint i;
  MultiloadApplet *ma = data;
  ...
  if (ma->about_dialog) gtk_widget_destroy(ma->about_dialog);

  if (ma->prop_dialog) gtk_widget_destroy(ma->prop_dialog);
...
static gboolean multiload_button_press_event_cb(GtkWidget *widget,
...
static gboolean multiload_key_press_event_cb(GtkWidget *widget,
                                             GdkEventKey *event,
                                             MultiloadApplet *ma) {
  g_return_val_if_fail(event != NULL, FALSE);
  g_return_val_if_fail(ma != NULL, FALSE);
...
void multiload_applet_tooltip_update(LoadGraph *g) {
  gchar *tooltip_text;
  MultiloadApplet *multiload;
  const char *tooltip_label[graph_n] = {
      [graph_cpuload] = N_("Processor"),    [graph_memload] = N_("Memory"),
      [graph_netload2] = N_("Network"),     [graph_swapload] = N_("Swap Space"),
      [graph_loadavg] = N_("Load Average"), [graph_diskload] = N_("Disk")};
  const char *name;
  ...
  switch (g->id) {
    case graph_memload: {
      ...
      user_percent = MIN((float)(100 * multiload->memload_user) /
                             (float)(multiload->memload_total),
                         100.0f);
      cache_percent = MIN((float)(100 * multiload->memload_cache) /
                              (float)(multiload->memload_total),
                          100.0f);
      tooltip_text = g_strdup_printf(_("%s:\n"
                                       "%.01f%% in use by programs\n"
                                       "%.01f%% in use as cache"),
                                     name, user_percent, cache_percent);
      break;
    }
    case graph_loadavg: {
...
static void multiload_create_graphs(MultiloadApplet *ma) {
  struct {
    const char *label;
    ...
    LoadGraphDataFunc callback;
      ...
      [graph_cpuload] = {_("CPU Load"), VIEW_CPULOAD_KEY, "cpuload", cpuload_n,
                         GetLoad},
      [graph_memload] = {_("Memory Load"), VIEW_MEMLOAD_KEY, "memload",
                         memload_n, GetMemory},
      [graph_netload2] = {_("Net Load"), VIEW_NETLOAD_KEY, "netload2", 6,
                          GetNet},
      [graph_swapload] = {_("Swap Load"), VIEW_SWAPLOAD_KEY, "swapload",
                          swapload_n, GetSwap},
      [graph_loadavg] = {_("Load Average"), VIEW_LOADAVG_KEY, "loadavg", 3,
                         GetLoadAvg},
      [graph_diskload] = {_("Disk Load"), VIEW_DISKLOAD_KEY, "diskload",
                          diskload_n, GetDiskLoad}};
  ...
  speed = CLAMP(g_settings_get_uint(ma->settings, REFRESH_RATE_KEY),
                REFRESH_RATE_MIN, REFRESH_RATE_MAX);
  size = CLAMP(g_settings_get_uint(ma->settings, GRAPH_SIZE_KEY),
               GRAPH_SIZE_MIN, GRAPH_SIZE_MAX);
  net_threshold1 =
      CLAMP(g_settings_get_uint64(ma->settings, KEY_NET_THRESHOLD1),
            MIN_NET_THRESHOLD1, MAX_NET_THRESHOLD1);
  net_threshold2 =
      CLAMP(g_settings_get_uint64(ma->settings, KEY_NET_THRESHOLD2),
            MIN_NET_THRESHOLD2, MAX_NET_THRESHOLD2);
  net_threshold3 =
      CLAMP(g_settings_get_uint64(ma->settings, KEY_NET_THRESHOLD3),
            MIN_NET_THRESHOLD3, MAX_NET_THRESHOLD3);
  ...
  for (i = 0; i < graph_n; i++) {
    ma->graphs[i] = load_graph_new(
        ma, graph_types[i].num_colours, graph_types[i].label, i, speed, size,
        g_settings_get_boolean(ma->settings, graph_types[i].visibility_key),
        graph_types[i].name, graph_types[i].callback);
  }
  ma->nvme_diskstats =
      g_settings_get_boolean(ma->settings, "diskload-nvme-diskstats");
  /* for Network graph, colors[4] is grid line color, it should not be used in
   * loop in load-graph.c */
  /* for Network graph, colors[5] is indicator color, it should not be used in
   * loop in load-graph.c */
  ...
  /* for Load graph, colors[2] is grid line color, it should not be used in loop
   * in load-graph.c */
  ma->graphs[graph_loadavg]->n = 2;
}

/* remove the old graphs and rebuild them */
void multiload_applet_refresh(MultiloadApplet *ma) {
  guint i;
  MatePanelAppletOrient orientation;

  /* stop and free the old graphs */
  for (i = 0; i < graph_n; i++) {
    if (!ma->graphs[i]) continue;
  ...
  if (ma->box) gtk_widget_destroy(ma->box);
  ...
  /* create the N graphs, passing in their user-configurable properties with
   * gsettings. */
  ...
  for (i = 0; i < graph_n; i++) {
    gtk_box_pack_start(GTK_BOX(ma->box), ma->graphs[i]->main_widget, TRUE, TRUE,
                       1);
...
static const GtkActionEntry multiload_menu_actions[] = {
    {"MultiLoadProperties", "document-properties", N_("_Preferences"), NULL,
     NULL, G_CALLBACK(multiload_properties_cb)},
    {"MultiLoadRunProcman", "system-run", N_("_Open System Monitor"), NULL,
     NULL, G_CALLBACK(start_procman_cb)},
    {"MultiLoadHelp", "help-browser", N_("_Help"), NULL, NULL,
     G_CALLBACK(help_cb)},
    {"MultiLoadAbout", "help-about", N_("_About"), NULL, NULL,
     G_CALLBACK(about_cb)}};

/* create a box and stuff the load graphs inside of it */
static gboolean multiload_applet_new(MatePanelApplet *applet, const gchar *iid,
                                     gpointer data) {
  ...
  ma->settings =
      mate_panel_applet_settings_new(applet, "org.mate.panel.applet.multiload");
  mate_panel_applet_set_flags(applet, MATE_PANEL_APPLET_EXPAND_MINOR);

  action_group = gtk_action_group_new("Multiload Applet Actions");
  gtk_action_group_set_translation_domain(action_group, GETTEXT_PACKAGE);
  gtk_action_group_add_actions(action_group, multiload_menu_actions,
                               G_N_ELEMENTS(multiload_menu_actions), ma);
  mate_panel_applet_setup_menu_from_resource(
      applet, MULTILOAD_RESOURCE_PATH "multiload-applet-menu.xml",
      action_group);
  ...
  g_signal_connect(applet, "change-size", G_CALLBACK(multiload_change_size_cb),
                   ma);
  g_signal_connect(applet, "change-orient",
                   G_CALLBACK(multiload_change_orient_cb), ma);
  g_signal_connect(applet, "destroy", G_CALLBACK(multiload_destroy_cb), ma);
  ...
  if (GTK_IS_ACCESSIBLE(atk_obj)) {
    ...
    atk_object_set_description(
        atk_obj, _("A system load monitor capable of displaying graphs "
                   "for CPU, ram, and swap space use, plus network "
                   "traffic."));
  }
...
static gboolean multiload_factory(MatePanelApplet *applet, const gchar *iid,
                                  gpointer data) {
