...
static void shift_right(LoadGraph *g) {
  guint64 *last_data;
  gsize i;
  ...
  /* data[i+1] = data[i] */
  for (i = g->draw_width - 1; i != 0; --i) g->data[i] = g->data[i - 1];
...
static void load_graph_draw(LoadGraph *g) {
  ...
  if (!g->surface)
    g->surface = gdk_window_create_similar_surface(
        gtk_widget_get_window(g->disp), CAIRO_CONTENT_COLOR, (int)g->draw_width,
        (int)g->draw_height);
  ...
  switch (g->id) {
    ...
    case graph_netload2: {
      ...
      // ratio, multiload->net_threshold1, multiload->net_threshold2,
      // multiload->net_threshold3, multiload->net_threshold);
      ...
      for (j = 0; j < g->n - 1; j++) {
        ...
        for (i = 0; i < g->draw_width; i++) {
          ...
          cairo_line_to(
              cr, x,
              (double)g->pos[i] - 0.5 - (((double)g->data[i][j] * ratio)));
      ...
      if (level > 0) {
        ...
        for (k = 0; k < level; k++)
          cairo_rectangle(cr, 0.5,
                          (double)k * 2.0 * (double)g->draw_height / 5.0, 5.0,
                          (double)g->draw_height / 5.0);
        cairo_fill(cr);
      }
      cairo_stroke(cr);
      break;
    ...
    case graph_loadavg: {
      ...
      for (i = 0; i < g->draw_width; i++) {
        g->pos[i] = g->draw_height - 1;
        /* find maximum value */
        if (g->data[i][0] > maxload) maxload = g->data[i][0];
      }
      ...
      for (j = 0; j < g->n; j++) {
        ...
        for (i = 0; i < g->draw_width; i++) {
          ...
          if (j == 0) {
            cairo_line_to(
                cr, x,
                (double)g->pos[i] - (((double)g->data[i][j] - 0.5) / load));
          } else {
            cairo_line_to(cr, x, 0.5);
          }
    ...
    default:
      for (i = 0; i < g->draw_width; i++) g->pos[i] = g->draw_height - 1;
      ...
      for (j = 0; j < g->n; j++) {
        ...
        for (i = 0; i < g->draw_width; i++) {
          if (g->data[i][j] != 0) {
            ...
            cairo_move_to(cr, x, (double)g->pos[i] + 0.5);
            cairo_line_to(cr, x,
                          (double)g->pos[i] - (double)g->data[i][j] - 0.5);
          }
          g->pos[i] -= g->data[i][j];
        }
        cairo_stroke(cr);
...
static gboolean load_graph_update(LoadGraph *g) {
  if (g->data == NULL) return TRUE;

  shift_right(g);

  if (g->tooltip_update) multiload_applet_tooltip_update(g);
...
void load_graph_unalloc(LoadGraph *g) {
  gsize i;
...
static gint load_graph_configure(GtkWidget *widget, GdkEventConfigure *event,
                                 gpointer data_ptr) {
  ...
  if (!c->surface)
    c->surface = gdk_window_create_similar_surface(
        gtk_widget_get_window(c->disp), CAIRO_CONTENT_COLOR, (int)c->draw_width,
        (int)c->draw_height);
...
static gint load_graph_expose(GtkWidget *widget, cairo_t *cr,
                              gpointer data_ptr) {
...
static void load_graph_destroy(GtkWidget *widget, gpointer data_ptr) {
  LoadGraph *g = (LoadGraph *)data_ptr;
...
static gboolean load_graph_clicked(GtkWidget *widget, GdkEventButton *event,
                                   LoadGraph *load) {
...
static gboolean load_graph_enter_cb(GtkWidget *widget, GdkEventCrossing *event,
                                    gpointer data) {
  LoadGraph *graph;
...
static gboolean load_graph_leave_cb(GtkWidget *widget, GdkEventCrossing *event,
                                    gpointer data) {
  LoadGraph *graph;
  graph = (LoadGraph *)data;
...
static void load_graph_load_config(LoadGraph *g) {
  gchar *name, *temp;
  guint i;

  if (!g->colors) g->colors = g_new0(GdkRGBA, g->n);

  for (i = 0; i < g->n; i++) {
    name = g_strdup_printf("%s-color%u", g->name, i);
    temp = g_settings_get_string(g->multiload->settings, name);
    if (!temp) temp = g_strdup("#000000");
...
LoadGraph *load_graph_new(MultiloadApplet *ma, guint n, const gchar *label,
  ...
  g_signal_connect(g->disp, "draw", G_CALLBACK(load_graph_expose), g);
  g_signal_connect(g->disp, "configure-event", G_CALLBACK(load_graph_configure),
                   g);
  g_signal_connect(g->disp, "destroy", G_CALLBACK(load_graph_destroy), g);
...
void load_graph_start(LoadGraph *g) {
  ...
  if (g->timer_index != -1) g_source_remove((guint)g->timer_index);

  event_source_id = g_timeout_add(g->speed, (GSourceFunc)load_graph_update, g);
