...
static const GtkActionEntry battstat_menu_actions[] = {
    {"BattstatProperties", "document-properties", N_("_Preferences"), NULL,
     NULL, G_CALLBACK(prop_cb)},
    {"BattstatHelp", "help-browser", N_("_Help"), NULL, NULL,
     G_CALLBACK(help_cb)},
    {"BattstatAbout", "help-about", N_("_About"), NULL, NULL,
     G_CALLBACK(about_cb)}};
...
static void status_change_callback(void) {
  GSList *instance;
...
/* The following two functions keep track of how many instances of the applet
   ...
   it does.)
*/
static const char *static_global_initialisation(ProgressData *battstat) {
  gboolean first_time;
  const char *err;
...
static void static_global_teardown(ProgressData *battstat) {
 ...
 */
static void battstat_error_dialog(GtkWidget *applet, const char *msg) {
  GtkWidget *dialog;
...
*/
static char *get_remaining(BatteryStatus *info) {
  int hours;
  int mins;
  ...
  if (info->on_ac_power && !info->charging)
    ...
    return g_strdup_printf(_("Unknown time (%d%%) until charged"),
                           info->percent);
  else if (hours == 0)
    if (!info->on_ac_power)
      return g_strdup_printf(ngettext("%d minute (%d%%) remaining",
                                      "%d minutes (%d%%) remaining", mins),
                             mins, info->percent);
    else
      return g_strdup_printf(ngettext("%d minute until charged (%d%%)",
                                      "%d minutes until charged (%d%%)", mins),
                             mins, info->percent);
  else if (mins == 0)
    if (!info->on_ac_power)
      return g_strdup_printf(ngettext("%d hour (%d%%) remaining",
                                      "%d hours (%d%%) remaining", hours),
                             hours, info->percent);
    else
      return g_strdup_printf(ngettext("%d hour until charged (%d%%)",
                                      "%d hours until charged (%d%%)", hours),
                             hours, info->percent);
  else if (!info->on_ac_power)
    /* TRANSLATOR: "%d %s %d %s" are "%d hours %d minutes"
     * Swap order with "%2$s %2$d %1$s %1$d if needed */
    return g_strdup_printf(_("%d %s %d %s (%d%%) remaining"), hours,
                           ngettext("hour", "hours", hours), mins,
                           ngettext("minute", "minutes", mins), info->percent);
  else
    /* TRANSLATOR: "%d %s %d %s" are "%d hours %d minutes"
     * Swap order with "%2$s %2$d %1$s %1$d if needed */
    return g_strdup_printf(_("%d %s %d %s until charged (%d%%)"), hours,
                           ngettext("hour", "hours", hours), mins,
                           ngettext("minute", "minutes", mins), info->percent);
}

static gboolean battery_full_notify(GtkWidget *applet) {
#ifdef HAVE_LIBNOTIFY
  GError *error = NULL;
  GdkPixbuf *icon;
  gboolean result;

  if (!notify_is_initted() && !notify_init(_("Battery Monitor"))) return FALSE;

  icon = gtk_icon_theme_load_icon_for_scale(
      gtk_icon_theme_get_default(), "battery", 48,
      gtk_widget_get_scale_factor(applet), GTK_ICON_LOOKUP_USE_BUILTIN, NULL);

  NotifyNotification *n = notify_notification_new(
      _("Your battery is now fully recharged"), "", /* "battery" */ NULL);
...
static void battery_full_dialog(GtkWidget *applet) {
  /* first attempt to use libnotify */
  if (battery_full_notify(applet)) return;

  GtkWidget *dialog, *hbox, *image, *label;
  cairo_surface_t *surface;

  gchar *new_label;
  dialog = gtk_dialog_new_with_buttons(_("Battery Notice"), NULL,
                                       GTK_DIALOG_DESTROY_WITH_PARENT, "gtk-ok",
  ...
  surface =
      gtk_icon_theme_load_surface(gtk_icon_theme_get_default(), "battery", 48,
                                  gtk_widget_get_scale_factor(applet), NULL,
 ...
 */
static void battery_low_dialog_destroy(ProgressData *battstat) {
 ...
 */
/* #define HAVE_PMI */
static gboolean is_suspend_unavailable(void) {
#ifdef HAVE_PMI
  int status;
 ...
 */
static void battery_low_update_text(ProgressData *battstat,
                                    BatteryStatus *info) {
  const char *suggest;
  ...
  gtk_widget_get_preferred_size(GTK_WIDGET(battstat->battery_low_label), NULL,
                                &size);
  ...
  if (info->minutes < 0 && !info->on_ac_power) {
    ...
    remaining =
        g_strdup_printf(ngettext("You have %d minute of battery power "
 ...
 */
static void battery_low_dialog(ProgressData *battery, BatteryStatus *info) {
  GtkWidget *hbox, *image, *label;
  GtkWidget *vbox;
  cairo_surface_t *surface;

  /* If the dialog is already displayed then don't display it again. */
  if (battery->battery_low_dialog != NULL) return;

  battery->battery_low_dialog = gtk_dialog_new_with_buttons(
      _("Battery Notice"), NULL, GTK_DIALOG_DESTROY_WITH_PARENT, "gtk-ok",
      GTK_RESPONSE_ACCEPT, NULL);
  ...
  g_signal_connect_swapped(battery->battery_low_dialog, "response",
                           G_CALLBACK(battery_low_dialog_destroy), battery);
  ...
  gtk_container_add(GTK_CONTAINER(gtk_dialog_get_content_area(
                        GTK_DIALOG(battery->battery_low_dialog))),
                    hbox);
  ...
  gtk_window_set_focus_on_map(GTK_WINDOW(battery->battery_low_dialog), FALSE);
  gtk_window_set_skip_pager_hint(GTK_WINDOW(battery->battery_low_dialog), TRUE);
 ...
 */
static void update_tooltip(ProgressData *battstat, BatteryStatus *info) {
  gchar *powerstring;
  gchar *remaining;
  gchar *tiptext;
  ...
  if (info->present) {
    ...
    if (info->on_ac_power)
      tiptext =
          g_strdup_printf("%s\n%s", AC_POWER_STRING, _("No battery present"));
    else
      tiptext = g_strdup_printf("%s\n%s", DC_POWER_STRING,
 ...
 */
static void update_percent_label(ProgressData *battstat, BatteryStatus *info) {
  gchar *new_label;
...
/* Determine what status icon we ought to be displaying and change the
   ...
   showing.
 */
static void possibly_update_status_icon(ProgressData *battstat,
                                        BatteryStatus *info) {
  GtkIconTheme *theme;
  cairo_surface_t *surface;
...
static gboolean check_for_updates(gpointer data) {
  ...
  if (!event_driven) {
    ...
    if (timeout != battstat->timeout) {
      ...
      if (battstat->timeout_id) g_source_remove(battstat->timeout_id);

      battstat->timeout_id =
          g_timeout_add_seconds(battstat->timeout, check_for_updates, battstat);
    }
  }

  possibly_update_status_icon(battstat, &info);

  if (!info.on_ac_power && battstat->last_batt_life != 1000 &&
      (
    ...
    if (battstat->lowbattnotification) {
      ...
      if (battstat->beep) gdk_display_beep(gdk_display_get_default());
    }
  }

  if (battstat->last_charging &&
      battstat->last_acline_status == POWER_STATUS_ON && !info.charging &&
      info.on_ac_power && info.present && info.percent > 99) {
 ...
 */
static void destroy_applet(GtkWidget *widget, ProgressData *battstat) {
  if (DEBUG) g_print("destroy_applet ()\n");

  if (battstat->prop_win) gtk_widget_destroy(GTK_WIDGET(battstat->prop_win));

  if (battstat->battery_low_dialog) battery_low_dialog_destroy(battstat);
 ...
 */
static void help_cb(GtkAction *action, ProgressData *battstat) {
  battstat_show_help(battstat, NULL);
}

/* Called when the user selects the 'about' menu item.
 */
static void about_cb(GtkAction *action, ProgressData *battstat) {
  const gchar *authors[] = {
  ...
  const gchar *documenters[] = {"J\xC3\xB6rgen Pehrson <jp@spektr.eu.org>",
                                "Trevor Curtis <tcurtis@somaradio.ca>",
                                "Davyd Madeley <davyd@madeley.id.au>",
                                N_("MATE Documentation Team"), NULL};

  char *comments = g_strdup_printf(
      "%s\n\n%s", _("This utility shows the status of your laptop battery."),
  ...
  gtk_show_about_dialog(
      NULL, "title", _("About Battery Charge Monitor"), "version", VERSION,
      "copyright",
 ...
 * another panel).
 */
static void setup_text_orientation(ProgressData *battstat) {
...
*/
static void change_orient(MatePanelApplet *applet, MatePanelAppletOrient orient,
...
*/
static void size_allocate(MatePanelApplet *applet, GtkAllocation *allocation,
                          ProgressData *battstat) {
 ...
 */
static void load_preferences(ProgressData *battstat) {
  ...
  battstat->red_value_is_time =
      g_settings_get_boolean(settings, "red-value-is-time");
  ...
  battstat->lowbattnotification =
      g_settings_get_boolean(settings, "low-battery-notification");
  battstat->fullbattnot =
      g_settings_get_boolean(settings, "full-battery-notification");
 ...
 */
static void grid_layout_attach(GtkGrid *grid, LayoutLocation loc,
                               GtkWidget *child) {
  switch (loc) {
    case LAYOUT_LONG:
...
void reconfigure_layout(ProgressData *battstat) {
  ...
  switch (battstat->orienttype) {
    ...
    case MATE_PANEL_APPLET_ORIENT_DOWN:
      ...
      if (battstat->showtext) c.text = LAYOUT_RIGHT;
      break;
    ...
    case MATE_PANEL_APPLET_ORIENT_RIGHT:
      ...
      c.status = LAYOUT_TOP;
      if (battstat->showtext) c.text = LAYOUT_BOTTOM;
      break;
  }
 ...
 */
static gint create_layout(ProgressData *battstat) {
  ...
  */
  ...
  g_signal_connect(battstat->applet, "destroy", G_CALLBACK(destroy_applet),
                   battstat);

  g_signal_connect(battstat->applet, "change-orient", G_CALLBACK(change_orient),
                   battstat);

  g_signal_connect(battstat->applet, "size-allocate", G_CALLBACK(size_allocate),
                   battstat);

  return FALSE;
}

void prop_cb(GtkAction *action, ProgressData *battstat) {
 ...
 */
static gboolean battstat_applet_fill(MatePanelApplet *applet) {
  ProgressData *battstat;
  ...
  gtk_action_group_add_actions(action_group, battstat_menu_actions,
                               G_N_ELEMENTS(battstat_menu_actions), battstat);

  mate_panel_applet_setup_menu_from_resource(
      MATE_PANEL_APPLET(battstat->applet),
      BATTSTAT_RESOURCE_PATH "battstat-applet-menu.xml", action_group);
...
/* Boilerplate... */
static gboolean battstat_applet_factory(MatePanelApplet *applet,
                                        const gchar *iid, gpointer data) {
  gboolean retval = FALSE;

  if (!strcmp(iid, "BattstatApplet")) retval = battstat_applet_fill(applet);
