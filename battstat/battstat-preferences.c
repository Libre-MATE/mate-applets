...
enum { PROP_0, PROP_PROGRESS_DATA };
...
static void combo_ptr_cb(GtkWidget *combo_ptr, gpointer data) {
  ...
  g_settings_set_boolean(dialog->battstat->settings, "red-value-is-time",
                         dialog->battstat->red_value_is_time);
}

static void spin_ptr_cb(GtkWidget *spin_ptr, gpointer data) {
  BattstatPreferences *dialog = data;
  ...
  g_settings_set_int(dialog->battstat->settings, "red-value",
                     dialog->battstat->red_val);
}

static void show_text_toggled(GtkToggleButton *button, gpointer data) {
  BattstatPreferences *dialog = data;
  gboolean show_text_active;

  show_text_active =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(dialog->check_text));
  if (show_text_active) {
    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(dialog->radio_text_1)))
      dialog->battstat->showtext = APPLET_SHOW_TIME;
    else if (gtk_toggle_button_get_active(
                 GTK_TOGGLE_BUTTON(dialog->radio_text_2)))
  ...
  g_settings_set_int(dialog->battstat->settings, "show-text",
                     dialog->battstat->showtext);
}

static void lowbatt_toggled(GtkToggleButton *button, gpointer data) {
  BattstatPreferences *dialog = data;

  dialog->battstat->lowbattnotification = gtk_toggle_button_get_active(button);
}

static void full_toggled(GtkToggleButton *button, gpointer data) {
  BattstatPreferences *dialog = data;
...
static void battstat_preferences_update(BattstatPreferences *dialog) {
  gboolean check_text_active = TRUE;

  if (!g_settings_is_writable(dialog->battstat->settings,
                              "low-battery-notification")) {
    gtk_widget_set_sensitive(dialog->lowbatt_toggle, FALSE);
  }

  gtk_widget_set_sensitive(dialog->hbox_ptr,
                           dialog->battstat->lowbattnotification);

  gtk_spin_button_set_value(GTK_SPIN_BUTTON(dialog->spin_ptr),
                            dialog->battstat->red_val);
  ...
  if (dialog->battstat->lowbattnotification)
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dialog->lowbatt_toggle),
                                 TRUE);
  ...
  switch (dialog->battstat->showtext) {
    case APPLET_SHOW_PERCENT:
      ...
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dialog->radio_text_2),
                                   TRUE);
      break;
    case APPLET_SHOW_TIME:
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dialog->check_text), TRUE);
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dialog->radio_text_1),
                                   TRUE);
      break;
    case APPLET_SHOW_NONE:
    default:
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dialog->check_text),
                                   FALSE);
      check_text_active = FALSE;
  }
...
static void battstat_preferences_set_property(GObject *object, guint prop_id,
                                              const GValue *value,
                                              GParamSpec *pspec) {
...
static void battstat_preferences_get_property(GObject *object, guint prop_id,
                                              GValue *value,
                                              GParamSpec *pspec) {
  BattstatPreferences *dialog;
...
static void battstat_preferences_init(BattstatPreferences *dialog) {
  gtk_widget_init_template(GTK_WIDGET(dialog));

  g_object_bind_property(dialog->check_text, "active", dialog->check_text_ptr,
                         "sensitive", G_BINDING_DEFAULT);

  g_object_bind_property(dialog->lowbatt_toggle, "active", dialog->hbox_ptr,
                         "sensitive", G_BINDING_DEFAULT);
}

static GObject *battstat_preferences_constructor(
  ...
  g_settings_bind(self->battstat->settings, "low-battery-notification",
                  self->lowbatt_toggle, "active", G_SETTINGS_BIND_DEFAULT);

  g_settings_bind(self->battstat->settings, "full-battery-notification",
                  self->full_toggle, "active", G_SETTINGS_BIND_DEFAULT);

  return object;
}

GtkWidget *battstat_preferences_new(ProgressData *battstat) {
  return g_object_new(BATTSTAT_TYPE_PREFERENCES, "progress-data", battstat,
                      NULL);
}

static void battstat_preferences_class_init(BattstatPreferencesClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_PROGRESS_DATA,
      g_param_spec_pointer("progress-data", "Progress Data",
                           "The Progress Data",
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  gtk_widget_class_set_template_from_resource(
      widget_class, BATTSTAT_RESOURCE_PATH "battstat-preferences.ui");

  gtk_widget_class_bind_template_child(widget_class, BattstatPreferences,
                                       check_text);
  gtk_widget_class_bind_template_child(widget_class, BattstatPreferences,
                                       check_text_ptr);
  gtk_widget_class_bind_template_child(widget_class, BattstatPreferences,
