...
#define ERR_NO_SUPPORT                               \
  _("Your platform is not supported!  The battery\n" \
    "monitor applet will not work on your system.\n")

#define ERR_FREEBSD_ACPI                           \
  _("There was an error reading information from " \
...
static void test_update_boolean(GtkToggleButton *button, gboolean *value) {
  *value = gtk_toggle_button_get_active(button);
}

static void test_update_integer(GtkSpinButton *spin, gint *value) {
  *value = gtk_spin_button_get_value_as_int(spin);
}

static void initialise_test(void) {
  GtkWidget *w;
  GtkBox *box;
  ...
  g_signal_connect(w, "value-changed", G_CALLBACK(test_update_integer),
                   &test_status.percent);
  ...
  g_signal_connect(w, "toggled", G_CALLBACK(test_update_boolean),
                   &test_status.on_ac_power);
  gtk_box_pack_start(box, w, TRUE, TRUE, 0);

  w = gtk_toggle_button_new_with_label("charging");
  g_signal_connect(w, "toggled", G_CALLBACK(test_update_boolean),
                   &test_status.charging);
  gtk_box_pack_start(box, w, TRUE, TRUE, 0);
...
static const char *apm_readinfo(BatteryStatus *status) {
  ...
  if (!test_initialised) initialise_test();
...
static gboolean acpi_callback(GIOChannel *chan, GIOCondition cond,
                              gpointer data) {
  ...
  */
  ...
  if (using_acpi && acpiinfo.event_fd >= 0) {
    if (acpi_count <= 0) {
       ...
       * seconds) since reading the ACPI system takes CPU cycles. */
      acpi_count = 30;
      acpi_linux_read(&apminfo, &acpiinfo);
    }
    acpi_count--;
  ...
  /* If we lost the file descriptor with ACPI events, try to get it back. */
    ...
    if (acpi_linux_init(&acpiinfo)) {
      acpiwatch = g_io_add_watch(
          acpiinfo.channel, G_IO_IN | G_IO_ERR | G_IO_HUP, acpi_callback, NULL);
      acpi_linux_read(&apminfo, &acpiinfo);
    }
  } else
    apm_read(&apminfo);
...
const char *power_management_getinfo(BatteryStatus *status) {
  ...
  if (status->percent == 100) status->charging = FALSE;

  if (!status->on_ac_power) status->charging = FALSE;
...
const char *power_management_initialise(void (*callback)(void)) {
  ...
  if (using_acpi && acpiinfo.event_fd >= 0) {
    acpiwatch = g_io_add_watch(acpiinfo.channel, G_IO_IN | G_IO_ERR | G_IO_HUP,
                               acpi_callback, NULL);
  }
#elif defined(__FreeBSD__) || defined(__FreeBSD_kernel__)
...
#ifdef __linux__
  if (using_acpi) {
    if (acpiwatch != 0) g_source_remove(acpiwatch);
    acpiwatch = 0;
    acpi_linux_cleanup(&acpiinfo);
  }
#elif defined(__FreeBSD__) || defined(__FreeBSD_kernel__)
