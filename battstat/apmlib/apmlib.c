...
int apm_read(apm_info *i) {
  ...
  sscanf(buffer, "%s %d.%d %hx %hx %hx %hx %d%% %d %s\n",
         (char *)i->driver_version, &i->apm_version_major,
         &i->apm_version_minor, &i->apm_flags, &i->ac_line_status,
         &i->battery_status, &i->battery_flags, &i->battery_percentage,
         &i->battery_time, units);
  ...
  if (i->driver_version[0] == 'B') { /* old style.  argh. */
    ...
    sscanf(buffer, "BIOS version: %d.%d", &i->apm_version_major,
           &i->apm_version_minor);
...
dev_t apm_dev(void) {
  ...
  while (fgets(buf, sizeof(buf) - 1, str)) {
    buf[sizeof(buf) - 1] = '\0';
    for (pt = buf; *pt && isspace(*pt); ++pt)
      ; /* skip leading spaces */
    for (; *pt && !isspace(*pt); ++pt)
      ; /* find next space */
    if (isspace(*pt)) {
      *pt++ = '\0';
...
int apm_close(int fd) { return close(fd); }
...
unsigned int apm_last_error(int fd) {
  ...
  int ierr = 0;
  if ((ierr = ioctl(fd, APM_IOC_LAST_ERROR, &err))) return ierr;
#endif
  return err;
}
...
const char *apm_time(time_t t) {
  ...
  if (d)
    sprintf(buffer, "%lu day%s, %02lu:%02lu:%02lu", d, d > 1 ? "s" : "", h, m,
            s);
  else
    sprintf(buffer, "%02lu:%02lu:%02lu", h, m, s);

  if (t == -1) sprintf(buffer, "unknown");
...
const char *apm_time_nosec(time_t t) {
  ...
  if (d)
    sprintf(buffer, "%lu day%s, %lu:%02lu", d, d > 1 ? "s" : "", h, m);
  else
