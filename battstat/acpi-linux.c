...
static gulong read_ulong(GHashTable *hash, const char *key) {
  char *s;
...
static const char *read_string(GHashTable *hash, const char *key) {
...
static gboolean update_ac_info(struct acpi_info *acpiinfo) {
  ...
  while ((procdirentry = readdir(procdir))) {
    if (procdirentry->d_name[0] != '.') {
      have_adaptor = TRUE;
      ac_state = g_strconcat("/proc/acpi/ac_adapter/", procdirentry->d_name,
                             "/", acpiinfo->ac_state_state, NULL);
   ...
   */
  if (have_adaptor == FALSE) acpiinfo->ac_online = 1;
...
static gboolean update_battery_info(struct acpi_info *acpiinfo) {
  ...
  while ((procdirentry = readdir(procdir))) {
    if (procdirentry->d_name[0] != '.') {
      batt_info = g_strconcat("/proc/acpi/battery/", procdirentry->d_name,
                              "/info", NULL);
...
gboolean acpi_linux_init(struct acpi_info *acpiinfo) {
  ...
  if (g_file_get_contents("/sys/module/acpi/parameters/acpica_version", &pbuf,
                          NULL, NULL)) {
  ...
  if (!update_battery_info(acpiinfo) || !update_ac_info(acpiinfo)) return FALSE;
...
static int parse_acpi_event(GString *buffer) {
  if (strstr(buffer->str, "ac_adapter")) return ACPI_EVENT_AC;
  if (strstr(buffer->str, "battery")) return ACPI_EVENT_BATTERY_INFO;
...
gboolean acpi_linux_read(struct apm_info *apminfo, struct acpi_info *acpiinfo) {
   ...
   * apminfo.battery_status must be 0 for high, 1 for low, 2 for critical, 3 for
   * charging apminfo.battery_percentage must contain batter charge percentage
  ...
  while ((procdirentry = readdir(procdir))) {
    if (procdirentry->d_name[0] != '.') {
      batt_state = g_strconcat("/proc/acpi/battery/", procdirentry->d_name, "/",
                               acpiinfo->batt_state_state, NULL);
  ...
  apminfo->battery_status = remain < acpiinfo->low_capacity        ? 1
  ...
  if (!acpiinfo->max_capacity)
    ...
    apminfo->battery_percentage =
        (int)(remain / (float)acpiinfo->max_capacity * 100);
  ...
  if (rate && !charging)
    ...
    apminfo->battery_time =
        (int)((acpiinfo->max_capacity - remain) / (float)rate * 60);
  else
